...
- **套裝共鳴(`itemSets.ts`,共 66 套)**:集齊一套於同一武將,**按效果軸生效**——**攻**(power,×戰力)/**守**(guard,甲冑套·減自損)/**水**(naval,水戰才加成);`itemSetBonuses`,戰鬥/單挑皆吃。**多套疊加封頂**(攻 ≤+25% / 守 ≤−20% / 水 ≤+30%),防滾雪球。涵蓋三國·楚漢·隋唐·戰國·唐,跨七類角色,玩家與 AI 對等。**名品面板(ItemsBrowser)有「套裝」分頁**:逐套顯示成員 ✓己有/⌕可尋/⚔他方/⚒待鍛 + x/n 進度。主要分組:
