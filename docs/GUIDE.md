...
| 軍團都督 | 方略 攻略/蠶食(動態攻最軟敵)/略地/固守;都督統率抬動員(出兵0.65→0.77)、智力主擇敵;每旬出戰報;可改編(§4.3) |
...
- **固守**:最強城增援危急的瘦弱鄰城。
- **都督做活(`legion.ts`)**:都督不再只是掛名 —— **統率**抬動員(出兵比例 0.65→~0.77、餘力門檻 6000→~5100 @統100,名帥動員更多更早)、**智力**主**方略**(動態擇敵/重選陷落目標)。派誰當都督是真決策。
- **軍團戰報**:軍團不再黑箱 —— 每旬出一條摘要「X軍團〔攻略→Y〕:N 路發兵(兵數)、M 城補軍」。
