...
> 現在新增 `hostileFrontCount`(數的是**能打的鄰國**,簽了互不侵犯的邊界不算戰線),戰線每多一條,**非主攻方向**的攻擊門檻乘 `DISPERSAL_PER_FRONT`(0.85),下限 0.55 —— 五面受敵時偏師要有近兩倍於平時的把握才動手,而主攻那一路完全不受影響。全庫中位:城留存 86%→93%、兵留存 63%→70%,而**易手次數 43→37 沒有塌**(那是防呆線:任何讓 AI 變保守的改動都能靠「讓誰都不打」把留存推到 100%,那是把盤面凍住,不是修好)。
> ⚠ 下限 0.55 存在的理由是同一件事:再嚴下去,四面受敵的一家連自衛反擊都不打。量測工具與逐盤數字見 `docs/CAMPAIGN-CHECKS.md` §4b-2 —— 那裡也記了**五國攻秦那一盤 n=6 分不出來**(兩組都橫跨 50–250k),因為它的壓力是打進來的,不是自己攤出去的。
