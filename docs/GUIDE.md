...
| 連環計・計多必失(§5.3) | 智≥90 疊第二計(效果相乘),但疊計看破率+22%易雙重反噬;美人計對好色/反間對低忠更烈;forcedStratagem 支援玩家選計 |
...
- **軍師獻策(`forcedStratagem`)**——引擎支援**玩家指定用計**(過門檻且合條件則採之,否則回退主將自擇);玩家選計面板待接(UI 留作後續)。
