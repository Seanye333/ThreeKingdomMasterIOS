...
| 守方用計・鬥智(§5.3) | 守方抽守城計(鐵壁/以逸待勞/反間…)同場結算(mirrorDefenderEffect);看破→計不售且反噬(守+8%·攻損×1.12) |
| 連環計・計接戰場(§5.3) | 智≥90 疊第二計(效果相乘);速決之計轉戰術圖化開局劣勢(火攻→地火·埋伏→伏兵·夜襲→入夜·斷糧→乏食) |
...
- **八陣**(諸葛亮,智 ≥90):陣中每回合回兵 4%、鄰接兩個陣中單位的敵軍 −1 AP。**藤甲**抗常規但**火傷 ×2**(赤壁/南蠻)。
- **臨陣變陣**(見 §5.1):開戰前定陣外,戰中亦可 `changeFormation` 改陣(代價:全軍暫陷亂一回合)。
...
- **攻守鬥智・看破(將計就計)**——智深/`神算`之將可**看破**敵計:成則敵計**不售且反受其害**(將計就計,守方反得 +8% 戰力、攻方損失 ×1.12);攻方亦能識破守方之計。看破機率隨**智差**遞增。
- **連環計**——主將**智 ≥90** 可**疊第二計**(連環+火攻=赤壁成局),兩計效果**相乘**。
