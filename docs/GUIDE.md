...
- **戰役落幕(`scenarioVerdicts.ts`,2026-08)**:上表那五段敗亡輓歌是**全域**的(只看君主是死是囚、年份過沒過 263)。分盤分家的落幕文本另放一張表,`checkEndings` 取 `scenarioId + playerForceId`:
  ...
  - 沒寫過的盤照舊走通用結局 —— 這條退路仍在,但**現在用不到了**:69 張歷史盤(`kind !== 'whatif'`)已全部寫完,`content-audit.ts` 的覆蓋列印為 `0/69`。什麼假設盤(whatif)不計,走通用結局。開場白那一端(`scenarioPrologues.ts`)早就是分勢力的,這是補上對稱的另一端。
