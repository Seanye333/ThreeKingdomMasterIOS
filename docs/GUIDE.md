...
- **地方風物志(cityLore.ts,城市面板「總覽」頁)**:歷史名城各附一則風物志(山川形勝 + 掌故),讀之如翻《方輿》—— 洛陽銅駝荊棘、成都天府沃野、合肥逍遙津張遼威震、下邳白門樓飛將終殞、天水姜維故里街亭、交趾士燮綏南、新城司馬懿八日千二百里……現覆蓋 33 座要城。純展示 flavor,**不入 City 型別/存檔**;僅該城有 lore 時渲染一方引文卡,`cityLore(id)` 查無即不顯。逐城可續補,鍵以城 id。
...
- **劇本目標 objectives.ts**:部分劇本有**逐勢力專屬目標**(`findObjectiveFor(scenarioId, forceId)` 按玩家所選勢力查,ObjectivePanel 顯示進度;`primary` 主目標 + 可選 `secondary` 次目標)。2026-07 由 6 個劇本(184/190/200/208/220/234)擴至 **9 個** —— 新增**合肥 215**(孫權攻合肥 / 張遼七千死守 / 劉備爭漢中 / 張魯五斗自守)、**漢中 219**(劉備定軍山進位漢中王 / 曹操拒秦川 / 孫權白衣渡江襲荊州)、**夷陵 222**(劉備為關羽復仇奪江陵 / 陸遜火燒連營破蜀 / 曹丕坐收漁利南取荊襄)。forceId 逐一對照各劇本 `FORCES_*` 常量,絕不臆測;其餘 77 劇本仍吃通用結局判定。
