...
- **一夜的總帳(2026-08-07)**:全庫三輪掃描 **死目標 188 → 108,完全乾淨的盤 4 → 19**(69 張歷史盤)。剩下的一百來條裡有一批是**設計如此** —— 呂布「白門樓之前」、公孫瓚「易京不焚」、袁術「仲氏不亡」、袁尚「鄴城固守」,史書上他們就是輸了,那正是該盤要玩家改寫的東西。**動手改之前先分清楚「達不到」與「本來就該達不到」。**
  - ⚠ **三輪的解析度大約是 ±1 張盤**,別把個位數的差當訊號。最後一批(收窄四條守成型)前後同段比較:死目標 38 → 35,而乾淨盤 13 → 13 —— 兩張轉好、兩張轉壞,且兩張轉壞的都是 1~2/3 掉到 0/3 的擲骰。要判斷一個改動是不是真的有效,要嘛提高輪數,要嘛看**同一條目標**跨多次掃描的走勢,不要看總數的小幅波動。
