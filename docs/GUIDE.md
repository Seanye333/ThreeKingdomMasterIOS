...
>
> 動戰役內容(序章/目標/事件/開局外交)之前,先讀
> **[docs/CAMPAIGN-CHECKS.md](CAMPAIGN-CHECKS.md)** —— 那裡是全庫體檢清單、
> 每一條怎麼跑、現在的數字,以及踩過的坑。
...
| 5 | [戰術戰鬥](#第五章-戰術戰鬥) | tactical, tacticalAi, combat, formations, stratagems, weather(區域天候), battlefieldTerrain, worldScars(戰場烙印), fieldworks(築壘), columnReinforcements(會戰), wallTier城郭分層, 攻城動作(破城/搶修/雲梯), 入城三選, battleSpoils(戰場繳獲), **campDisease(軍中疫疾/頓兵/不習水土)**, **siegeWorks(攻城器械營造/焚其攻具)**, **nightRaid(夜襲劫營)**, **navalWarfare(水軍熟練/暈船/艦隊編成/淺灘擱淺/搶灘登陸)**, **tacticalNarration(逐擊戰報)**, **comboBondMul(連携合擊認因緣)**, **tacticalSurrender(陣前招降)**, **十面埋伏入林部署**, personalTactics, weaponTypes, namedMaps, damagePredict, battleRecap, fogOfWar | ✅ |
...
| 一代記階梯(11 級) | 白身→部曲→九品…→一方諸侯;功績門檻 0/6/18/40/75/130/210/320/460/620/820;**品階決定看得見哪些命令**;私兵上限 10/50/100/+1000/+3000/+6000 | career, careerAuthority |
| 投效(一代記) | 每季擲投效之望(功績.44+人望.28+威望.12+**在家鄉.16**);鄉勇進私兵、收不下會明說;部曲+ 偶有義士來投,**同鄉優先**(忠誠起手 78) | careerFollowers |
| 差事(一代記) | 白身掙功績的路;**從城的處境長出來**(民心<66 剿匪/商業≥55 押鏢/糧<9000 護糧/護院保底);兵力不足會折兵傷身;功績進 deeds,白身 ×1.6 | careerErrands |
| 人情・薦舉(一代記) | 差事有雇主(同城且身價不超品階上限);辦成記人情、大敗倒扣更凶;人情過門檻(8/14/22)且雇主份量夠,每季 18–55% 替你薦舉、折功績並耗掉人情 | careerPatronage |
...
| 旬與季 | `endSeason` 一**旬**一次;**一季九旬**(240 旬 ≈ 10 遊戲年) | store / resolution |
| 戰役隨機源 | `campaignRng(state)` = 純函式 (rngSeed, date);同狀態同流、日期推進換流 | campaignRng |
...
| 堤工(§1.21) | 0–3 級,「治水」+1;夏汛免疫門檻 = min(3, 堤防建築 + 治水),每級 −1/3 機率;久雨成澇 ×(1−min(0.8, 治水×0.22))。城池面板寫「再 N 級可免」 |
| 京師(§1.21) | 天子所在城 `citySize` 恆為最高城格「京」(不看戶口),各項上限最高;城池面板「☀京師」徽記 |
| 驛傳(§1.19) | 文書自首都腳程 3 跳,入己城 −1,抵驛站/驛傳回滿;≤1 跳 貪腐×0.8·民心+0.5 / ≤4 跳 ×0.95 / >4 跳 ×1.1·隱戶+0.15 / 斷驛 ×1.35·民心−1·隱戶+0.5 |
| 兵制(§4.8) | 更卒 全 ×1 / 世兵 徵兵×0.82·抽丁×0.55·耗糧×0.85·練度+0.5·逃亡×0.6 / 募兵 徵兵×1.28·抽丁×0.35·練度+1·逃亡×2.4·餉 14 金/千兵/季 |
| 欠餉(§4.8) | 由駐軍城府庫支付;欠餉比例×1/10 當季逃亡,民心 −1(欠半以上 −2) |
| 糧秣簿(§4.9) | 隨軍糧需求=兵×0.25×行程季;可撐季數=floor(糧/(兵×0.25));判詞 短(缺糧/不足行程)/緊(本城餘糧≤1季 或 僅夠1季)/足 |
| 軍功(§4.10) | 破陣×3+下城×8+單挑×2+罵倒×2+生擒×2+諜報×2+斬首/2000+內政×0.5,減已賞;未賞 ≥12 忠誠−1/季、≥26 −2;行賞費金=未賞×55,忠誠+min(18,未賞×0.8) |
...
| 整編(§4.12) | 質量=加權平均;新徵之卒素質 0(稀釋練度與老兵度)、傷癒歸伍素質 70(反而拉高);併軍依人數加權 |
...
| **元服之年** | **未滿 15 歲(含尚未出生)者不入人才池** —— 搜訪/薦舉/訪賢/遊俠現身/招降蠻王/求賢祭一體適用(`comingOfAge.ts`) |
...
| **名分之於隱士** | 訪賢再減 `clamp((天命−60)/70, −0.30, 0)` —— 名分低者請不動高士(黃巾 42、董卓 47;皇甫嵩/朱儁 61 為平) |
...
| 潰走與追擊(§5.1) | 士氣歸零=潰走(不能攻、自動逃向己方邊緣);追擊潰兵 步×1.5/騎×1.8 且潰兵不反擊;鄰主將可收攏回25;三軍盡潰即敗;**逃抵己方邊緣即離場**(一成計為離散,主將不離場) |
| 士氣可達性(§5.23) | **死傷奪氣 `BLOOD_MORALE_COST = 130`**(全軍覆沒 = 扣 130 士氣;舊值 50 使潰走結構性不可達);**被動回復上限 `MORALE_RALLY_CAP = 75`** —— 將旗/旗令/眾寡/順勢只能把人扶到 75,已在其上者不動,**鼓舞計略不受限**;AI 追亡:騎兵 ×0.5(優先追)、其餘 ×1.6(不為追亡而破陣) |
...
| 逐擊戰報(§5.18) | 每一擊挑主導因素寫進戰報:伏兵→擱淺→背刺→側擊→相剋→三面/兩軍夾攻→攻方所踞→守方所憑→精銳→師老兵疲→夜戰→天候;戰果分級 ≥30% 陣列摧折 / ≥18% 大挫 / ≥8% 陣腳一鬆 / >0 略挫;**輪替動詞不耗 rng**(取 damage+turn+col,否則每擊位移戰局隨機流);抽屜「逐擊」可關(kind `blow` 與戲劇節拍 `event` 分流,不搶跑馬燈) |
| 連携合擊(§5.19) | 取最親一條:**宿怨 ×1.0**(優先於一切)→ 正典名對 ×1.30 → 結義按深度 義交 ×1.15 / 金蘭 ×1.25 / 生死之交 ×1.35 → 骨肉同陣 ×1.18;執行期 `runtimeBonds`/`family` 於 `startTacticalBattle` 篩本盤後蓋章到戰局 |
| 陣前招降(§5.20) | 目標須 潰走 / 士氣≤25 / 殘部≤25%;2 格內 · 1 AP · **一戰一次**;忠誠≥95 或與你有宿怨=不聽;成算 0.10 +(魅−60)/200 +(60−忠)/200 + 殘破(潰0.20/士≤15 0.12/其餘0.05)+ 殘部≤20% 0.10 + 故主0.18 + 義兄弟0.22/骨肉0.14 − 主將0.18 ± 氣勢0.06,夾 [0,0.75];**拒絕者士氣 +10**;歸降者鄰兩格同袍 −8 士氣,你**守住戰場**才算俘虜;AI 成算≥0.30 亦會招降你 |
| 十面埋伏(§5.21) | 用該陣形者伏兵翼**自行走入 4 格內最近森林**並隱蔽;上限該方半數、主將不藏;AI 候選需 有林+守勢,智力門檻用陣形自身 `minIntelligence 95`,且**必須排在因軍制宜預設之前**(find 取第一個命中) |
...
- 下半:**府庫沿革**(八季 sparkline,升綠降赭,讀既有 `treasuryHistory` 快照,無新狀態)+ **制度批計量**(全境平均軍器 / 在營傷兵 / 平均練度)+ **全境民政帳** —— 全境均值(積案/隱戶與實收租賦%/囤積/文教/祠數/詩數),以及**最需要管的 12 城**逐行列出四個計量(逾閾轉赭色,懸停顯示階級名),**點城名直接跳到該城**。
...
**制度抉擇事件五則(2026-07-21)** —— 讓今夜的新制度也長出「要你親自選」的戲(同樣走 behaviorEvents 管線,一局各至多一次):

| 事件 | 觸發 | 三條路 |
|---|---|---|
| **大錢之議**(§1.17) | 行五銖 + 府庫見底(<城數×700) | 鑄大錢(+2600 金,民心 −3)/ 錢法不可輕改(民心 +3,能吏 +忠)/ 罷錢用穀帛 |
| **欠餉**(§4.8) | 行募兵 + 府庫不足三期餉 | 傾府庫發餉(將 +10 忠)/ 許以破城之財(民心 −4)/ 汰其冗卒(兵 ×0.88,民心 +3) |
| **商賈請榷**(§1.16) | 行平糴 + 有商業 ≥60 之城 | 開關通糴抽商稅 / **官榷其利**(+1500 金但商城 −6 民心)/ 寸粟不出境 |
| **傷卒滿營**(§4.11) | 在營傷兵 ≥ max(2000, 全軍 6%) | 出金購藥(−1200 金,民心 +4)/ 悉數遣還 / 軍中無暇顧此(民心 −5,醫者寒心) |
| **甲兵不修**(§1.18) | 全境平均軍器 <12 且兵 ≥2 萬 | 大開工官(−1600 金,城防 +6)/ 括民間之鐵徵匠戶(民心 −4,城防 +5)/ 兵在人不在器 |

坑(這次踩到):事件門檻用 `x < 閾值` 判定時,**City 缺欄會讓閾值變成 NaN,而 `x < NaN` 恆為 false** —— 於是事件在第一回合就開火。所有讀 City 選填欄的比較一律 `?? 0`。
...
- **大地圖資訊層「米價」**(資源疊圖組,**點選切換、無數字快捷鍵**),**並疊上上一季實際跑過的商旅路線** —— 虛線沿陸路畫,粗細隨運量,境內琥珀、跨境淡金。刻意只在此層顯示:糧往紅處流,線與色是同一件事。原句::全境城池按金/百石著色 —— 綠為穀賤傷農、赤為米珠薪桂,標籤直接寫價。**貴的地方就是商旅已經在路上的地方,也是圍城會更快咬下去的地方**。
...
### §1.21 堤工與京師 — 兩個只活在存檔裡的城市數值

**堤工 `floodWorks`**(0–3 級,「治水」指令每次 +1)。兩套獨立機制在讀它,兩套都沒上過畫面:

- **夏季洪災**(events.ts):`免疫門檻 = min(3, 堤防建築等級 + 治水級數)`,每級降 1/3 機率,**合計 3 級完全免疫**
- **久雨成澇**(weather.ts):雨季災害機率 ×`(1 − min(0.8, 治水×0.22))`

玩家真正要決定的是「還要不要再派一季」,所以面板寫的是**「再 N 級可免」**而不只是現值;無堤工且農業 <25 的城不畫這條(不值得占位)。

**京師 `imperialSeat`**。天子所在城被 `citySize` 硬覆蓋為最高城格「京」,不論戶口多少 —— 各項發展上限跟著拉到最高。城格本來看得見,**成因看不見**,於是「迎天子/遷都」的實際收益無從估計。現在城池面板加「☀京師」徽記並在懸停說明這條覆蓋規則。
...
- **史官年鑑(chronicle.ts / YearbookModal)**:每年開春,史官就**剛結束的一年**呈一頁文言綜述 —— 大勢(城數消長/霸主/最盛/最衰)、兵事(引年鑑實錄)、災異、天下武評前三、對主公收語;全由 annals/城帳/武評榜現成數據撰成,「閱畢歸檔」即收。
  - **史官的褒貶「論曰」(2026-07-25)**:記事之後另起一段,以陳壽「評曰」的筆法**評價你怎麼治國**。刻意**不新增隱藏德行分數、不動存檔** —— 每一句都是對當下存檔的直接讀數,只是換成一個必須當著在世君主的面下筆的史官會用的措辭:①**民**(全境民忠均值五檔)「道不拾遺,耕者讓畔」→「民不聊生,流亡載道」;②**士**(麾下忠誠均值)「皆願效死」→「離心已見,宜早圖之」;③**賢**(在野一流人才數)「天下遺賢尚 N 人,散在草澤,未見旌招」↔「海內英彥,略已收攬」;④**兵**(本年戰事 ≥5)「兵連禍結…窮兵者未有能久者也」;⑤**名分**(已稱帝而據城未過半)「遽正大位,而輿地未半」;⑥**災**(災年看民忠)「賑貸及時,民不甚困」↔「賑貸不繼,此有司之責也」。入城三選只有安民/犒軍/搜捕、沒有屠城殺降可記,故史論不建立在暴行清單上而建立在**治理實況**上 —— 苛政會從民忠、離心、遺賢三處同時顯形。YearbookModal 以分隔線+斜體把論曰與上方記事隔開(那是意見,上面是記錄)。
...
- **絕命詩**(`deathPoems.ts`,75 首):名將身故時,季報附上其絕命詩(史實或演義)。三國 51 首(關羽/諸葛亮/曹操等既有 22 + 2026-07 補荀彧/趙雲/太史慈/張遼/馬超/孫堅/孫權/陸遜/曹丕/董卓/袁術/郭嘉/許褚/夏侯惇/龐德/王允/魯肅 17 首,再補甘寧/張郃/于禁/張昭/周泰/徐晃/顏良/文醜/華佗/貂蟬/司馬昭/鍾會 12 首);另 2026-07 補**歷代名將** 24 首 —— 一線 14:項羽(天之亡我)、岳飛(天日昭昭)、文天祥(讀聖賢書所學何事)、于謙(石灰吟)、韓信/李斯/白起/荊軻/陸游(示兒)/譚嗣同/袁崇煥/檀道濟/蒙恬/田橫;二線 10:屈原(寧赴湘流)、商鞅(為法之敝)、韓非、冉閔、方孝孺(滅我十族)、史可法(頭可斷身不可屈)、陸秀夫(負帝投海)、鄭成功(何面目見先帝)、顏真卿、張巡(睢陽死守)。
...
- **性別資料修復**(2026-07):三國 `OfficerTemplate` 沒有 `female` 欄位(不同於 historical 模板),於是 802 名三國武將 `female:true` 為 **0** —— 貂蟬、二喬、孫尚香、蔡文姬、祝融、黃月英、星彩等 40 名女性一直被婚配(異性擇配 `aiArrangeMarriages`/`proposeMarriagePair`)、繼承序、立繪(`OfficerPortrait`)、性別顯示、生育「千金/麟兒」判定**當成男性**。現以 `THREE_KINGDOMS_FEMALE_IDS` 集合在 `officers.ts` 物化 Officer 時統一打標(`stampFemale`),不逐條改模板;historical 女性各帶 `t.female`,不受影響。
...
> **初始靜態關係**(`relationships.ts` 的 `OFFICER_RELATIONSHIPS`,2026-07 由 173→194):補錄演義名場面的宿敵/血仇/主僕/師徒 —— 關羽×黃忠(長沙)/龐德(抬櫬)/顏良(白馬)、趙雲×文醜(磐河)/張郃(長坂)、張飛×馬超(葭萌)、馬超×許褚(潼關)、太史慈×孫策(神亭)、黃忠×夏侯淵(定軍山)、姜維×鄧艾/郭淮,甘寧×凌統(殺父)/馬超×曹操(滅族)血仇,劉備×趙雲/諸葛(魚水)主僕,呂布×陳宮、司馬徽薦龐統、董卓×貂蟬。**再補一批(→207)**:鄧艾×鍾會(滅蜀爭功)、曹操×陳宮(捉放曹→白門樓)/華佗(刳腦下獄)/楊修(雞肋)/馬騰(衣帶詔滅族)血仇、甘寧×黃祖(反殺故主)、孫策×于吉(鏡中顯聖)、諸葛亮×孟獲(七擒)、王允×貂蟬(義父女)、曹丕×甄姬(洛水)、夏侯惇×曹操(宗族心腹)、呂蒙繼魯肅、曹操×荀彧(終賜空函)。讓這些名場面關係**一開局就激活**羈絆同陣戰力、宿敵對決 +10%、招募修正、AI 攻擊猶豫(見 §5.1 / relationshipEffects.ts)。
...
- **列傳金句**(`biographies.ts` 的 `quote`,2026-07 由 7→50):列傳/圖鑑另呈一句該人的史實或演義名言。原僅曹操「寧我負人」等 7 條,補入司馬懿評諸葛、周瑜「遇知己之主」、魯肅榻上策、呂蒙「刮目相待」、趙雲「國賊是曹操」、張飛「燕人張翼德」、魏延「舉天下而來,請為大王拒之」、賈詡「離之而已」、鍾會「不失為劉備」等 28 條(含貂蟬/甄姬/蔡文姬/孫尚香/黃月英 5 位女性),再補二線 15 條 —— 張遼「賊至乃發何遽也」、太史慈「大丈夫一諾死生不易」、鄧艾「鳳兮鳳兮故是一鳳」、姜維「但有遠志不在當歸」、華佗、龐德「良將不怯死以苟免」、陳宮、馬良「白眉最良」、蔣琬、費禕、鄧芝、荀彧、郭嘉、孫策。以集中補充表 + 合併迴圈注入,只補「已有列傳且尚無金句」者,不覆蓋既有。**歷代名將**(跨界名冊)的列傳金句(`historicalBiographies.ts`)同步由 **36→66**:補入項羽「力拔山兮氣蓋世」、韓信「狡兔死走狗烹」、岳飛「三十功名塵與土」、文天祥「留取丹心照汗青」、霍去病「匈奴未滅何以家為」、荊軻「風蕭蕭兮易水寒」、勾踐「臥薪嘗膽」、于謙「要留清白在人間」、李白/杜甫/蘇軾/辛棄疾/王安石、陳勝「王侯將相寧有種乎」、林則徐/戚繼光/鄭成功/花木蘭/左宗棠等 30 條,**再補二線 17 條**(66→83):廉頗/藺相如「先國家之急而後私讎」、吳起「在德不在險」、伍子胥「抉吾眼懸東門」、管仲「倉廩實而知禮節」、李牧、王翦、謝安「小兒輩已破賊」、陳慶之「千兵萬馬避白袍」、王猛「捫蝨而談」、桓溫「遺臭萬載」、劉琨「枕戈待旦」、田忌賽馬、趙奢「狹路勇者勝」、龐涓「遂成豎子之名」、謝玄、范蠡「飛鳥盡良弓藏」(共 30+17,17/18 命中,曹劌 id 缺跳過)。
...
- **身份階梯(11 級)**:由 deeds 推導功績,再換算階梯。九品之下另設兩級 —— **白身**(布衣,無官身)與**部曲**(豪族親隨),因為從「已經是武官」開頭會跳過整段最該玩的人生。功績門檻 `0 / 6 / 18 / 40 / 75 / 130 / 210 / 320 / 460 / 620 / 820`(白身→一品):前幾級刻意便宜,一場勝仗加一場單挑就離得開白身;都督以上才拉長,那是要花一輩子的。
- **權限閘門(careerAuthority.ts)**:一代記下**品階決定看得見哪些命令** —— 一道命令要多大的官才發得出,就要多大的官才看得見。個人的事(習武、比試、游歷、翻書、鍛造)白身即可;`部曲` 開私兵與陣形、`九品` 開軍團、`大臣` 開任官/考課/書信、`太守` 開內政/州牧/度支/賑災、`都督` 開計略/密偵/說客、`一方諸侯` 開邦交/朝廷。閘門設在 UI 的命令構造處(命令面板與頂欄下拉是同一份宣告),store 只在私兵等關鍵 action 補斷言。城池分頁另按 `cityAuthority` 判斷:內政要太守,軍務九品即可但**限自己駐紮的那座城**。
- **私兵上限由品階決定**:一代記下不再是「統率 ×100」—— 白身 **0**(不得聚眾)、部曲 **20**、九品/八品 **100**、大臣起才 `統率×100 + 1000/3000/6000`。品階是天花板,統率只決定你能不能摸到它。
- **投效(careerFollowers.ts)**:主角靠自己的作為招人,不是花錢買兵 —— 募兵用錢、投效用名。每季按**投效之望**(功績 0.44 + 人望 0.28 + 威望 0.12,**在自己家鄉再 +0.16**)擲一次;白身也擲得到,只是少(基礎機率 白身 .16 / 部曲 .24 / 有官身 .30,上限 .62)。
  - **鄉勇**進私兵,人數隨品階放大(白身 ~3 / 部曲 ~9 / 八九品 ~24 / 更高 ~60,再乘投效之望)。
  - **收不下的人會明說**:「另有 N 人願附,惜以身份所限,養不得也」—— **升品的動機來自人來了卻收不下**,而不是一條看不見的數值。
  - **義士來投**:部曲以上偶有在野武將追隨,**同鄉優先**(用武將表的籍貫),同鄉來投忠誠起手 78、非同鄉 68。名將不會跟著小官走(能力總和上限隨品階放寬:八九品 165 / 大臣 210 / 太守以上不限)。
  - 亂數走 `subRng(state, 'career-followers')`,同一顆種子重跑得到同一個世界。
- **差事(careerErrands.ts)**:白身唯一掙得到功績的路 —— 前面的階梯、閘門、投效都做了,但白身能做的只有習武比試,而那些不記戰功,於是階梯爬不動。差事補的就是這個缺口。
  - **從城的處境長出來,不是憑空刷的**:民心 <66 才有**剿匪**(民心越低難度越高,20 以下四級)、商業 ≥55 才有**押鏢**、糧 <9000 才有**護糧**、人口 >12 萬才有**緝拿**;**護院**哪裡都有,錢少但穩,是白身的保底。同城同季恆定,刷新刷不出更好的活。
  - **會死人**:勝算 = 本事(剿匪押鏢看武力統率、緝拿看智力)+ 帶的人夠不夠 − 難度。**兵力不足最致命** —— 帶三個賓客去剿四級匪就是送死,大敗會折兵、掉威望,還可能讓主角自己**負傷一至三季**。
  - **報酬能換品階**:功績直接進 deeds(剿匪記 battlesWon/killsTroops、緝拿記 espionageSuccess、其餘記 civicWorks),所以做活就是在爬階梯。**品階越低同一件差事給的功績越重**(白身 ×1.6、部曲 ×1.3)—— 第一場仗該有份量。
  - 太守以上不再接私活(`takesErrands`),那時候你該去打仗,不是替人看家。
  - **差事面板**(命令 `差事`,一代記下白身即可開):列出所在城此刻的活,每件標**難度**(易/尋常/難/凶險/死地)、**需人**、**酬金**與**勝算**(綠/黃/紅量尺)。人手不足會紅字明示。風險設計成會死人,就得把賭注攤開講 —— 藏起來的風險不叫風險。孤身一人時另提示:「要剿匪,先得有人跟你走。」
- **雇主與人情(careerPatronage.ts)**:差事有**有名有姓的委託人** —— 這是 797 個武將真正進入玩家生活的地方。
  - **託得起你的人跟著你的身份走**:雇主須與主角同城,且「政治+統率+魅力」不超過品階上限(白身 170 / 部曲 200 / 有官身 260)—— 白身接不到太守的委託。
  - **人情**記在 `careerMode.favors`:辦得漂亮 +(3+難度)、辦成 +(2+難度/2)、沒辦成 −2、**大敗 −(3+難度)**。搞砸一次抵得上辦成兩次。
  - **薦舉**:漢末仕進靠察舉不靠考試。人情過門檻(白身 8 / 有官身 14 / 大臣以上 22)且那人**說話有份量**(政治×.6+魅力×.4 ≥ 52/62/74/86,隨你的品階水漲船高——縣吏薦得動白身,薦不動都督),每季有 18–55% 機率替你說話,折算約門檻 ×0.9 的功績,並**消耗掉那筆人情**。
  - 只挑**人情最厚的那一個**,一季不會來三個;死者與囚者不會開口。
  - 設計上刻意**比自己掙功績快、但不受你控制** —— 獎勵的是「你替誰辦過事」而非「你刷了多少活」。把全縣得罪光的人,武藝再高也升不上去。
  - 面板顯示每件差事的**委託人與現有人情**,以及「最厚的人情 N/門檻」。
- **品階特權**與**年譜**(自動記錄威名/晉升等里程碑)照舊;義士來投與薦舉都記入年譜。
...
- **上書(info)4→20 條(2026-07-25)**:info 不需准駁,是純陳情;但它每季對每個武將都在滾,原本只有 3 條條件報告 + 一句 fallback「上書問安」,實際玩起來絕大多數奏摺都是那句問安。現擴至 20 條,每條掛在**當下為真的條件**上 —— ①**城務**:糧倉太空/太滿、民忠低/高、兵少而城大、貪腐(corruption ≥40)吏胥舞文、隱戶(hiddenHouseholds ≥20)版籍不及半、城堞頹圮(defense <30)、末富本貧(商 >農×1.6)、物賤傷農(農 >商×1.6)、焦土、天子駐蹕;②**時令**:春耕乞緩征 / 夏雨葺堤 / 秋熟野有遺秉 / 歲暮戍卒衣薄;③**其人**:年逾六十猶思鞍馬、創處未合、疏遠之身久居閒任(忠 <60)、智高武低者論廟算、猛將閒久「髀肉復生」、政高者論安民足食。「上書問安」降為真正的 fallback(人不在城中且自身無事可陳)。池抽成 export 的 `composeInfoLetters` 純函數,generateWish 只從池裡隨機取一封,池本身直接可測。
...
### 2.14 潛能與血仇上畫面(2026-07-27)

兩個引擎在讀、畫面從不顯示的武將欄位。找法是**比對型別欄位在 UI 被讀幾次**(與 §5.18 的 `facing` 同一招):`latentStats` 引擎 13 處/畫面 0 處,`killedRelativesBy`+`killedSwornBy` 合計 16 處/畫面 0 處。

**潛能 `latentStats`** —— 每次升級擲骰都受它管(`growth.grantXp` 按 latent 上限決定能不能漲;資質 S/A/B/C 決定 +2 機率與被抽中權重),但只活在存檔裡。能力條後面加**虛線鬼影**畫到上限、末端一根刻度線、數字旁 `/上限`,tooltip 補資質。**已達上限者不畫** —— 否則五條都拖著一條無意義的尾巴。

**血仇 `killedRelativesBy` / `killedSwornBy`** —— 這是全批最要緊的一個:`estimateRecruitChance` 對仇家扣 **50 點**,`aiCaptiveFate.aiRecruitChance` 對仇家直接**回 0**(誓不事仇)。玩家可以對一個永遠不會降的俘虜連試三種勸法,而畫面上**沒有任何一個字**說明原因。現在武將面板出現血仇徽記(仇家是自己時轉紅並標「誓不事仇」);同時把 `freedByForceId`(義釋報恩 **+20%**)也做成徽記 —— 那是同一個決策的另一半。
...
**元服之年(`comingOfAge.ts`,2026-08)**:上列每一條入口都**先過年齡這一關** —— 未滿 15 歲(含尚未出生)者不在人才池裡。

這條線本來就有,但**只裝在玩家親自「搜訪」那一條路上**(`handleSearch` 的 `MIN_RECRUIT_AGE`),薦舉、訪賢、遊俠現身、招降蠻王、求賢祭抽卡都沒有。184 年的盤上八百名武將裡有 292 人尚未出生、206 人未滿十五 —— 六成二的人不該在池裡而只有一個入口擋著,自走體檢於是跑出「朱儁軍於隆中臥龍崗延攬諸葛亮入幕」(時年三歲)、「月旦評 — 盧植品姜維為上品令器」(十八年後才出生),而張角一死,黃巾的繼位者是那個三歲的諸葛亮。現在常數收在 `comingOfAge.ts`,各路徑共用同一條判斷。

實作上是**純函式**:年齡由 `birthYear` 與當前年份現算,不新增狀態、存檔不必遷移,時間一到人自己就進池子。(試過開局抽掉未成年者的 `locationCityId`,那是錯的 —— `handleSearch` 把「無本貫者」當成四海皆可遇的後備池,抽掉反而更容易被撿走。)
...
- **名分之於隱士**(2026-08-04):三顧的成算另減 `clamp((招賢之主的天命 − 60) / 70, −0.30, 0)` —— 天命 60 為平(皇甫嵩/朱儁 61),低於此逐步難請(黃巾 42、董卓 47),高於此**不另加成**(加成是三顧之誠的事,名分只決定人家肯不肯開門)。
  加這一項之前,自走體檢四輪裡三次出現「黃巾於水鏡莊延攬司馬徽入幕」「黃巾於高密鄭玄延攬鄭玄入幕」—— 漢末經學宗師與清流名士,投了太平道。追下去發現這條路**只看魅力**,而張角魅力 95 是全盤最高:純魅力模型裡,他請隱士比任何人都容易。缺的那一項是名分,而史書恰恰記了反面:「黃巾過高密,相約不敢入玄鄉」—— 賊眾敬鄭玄敬到繞道,而不是請他入幕。
- **名所擴充**(2026-07,`scenicSites.ts` 8→21):新增九處**訪賢隱士居**(博陵崔州平/沔南黃承彥/峴山龐德公/高密鄭玄/平原管輅/天柱山左慈/曲阿于吉/徐無山田疇/譙郡華佗)與四處**尋寶古蹟**(泰山封禪壇→太公兵法/汨羅屈子祠→離騷/會稽禹穴→春秋/磻溪釣台→六韜),為世界多添十三處游歷目標。coords/guards 城/hermit·item id 皆逐一對照存在。
...
### 3.7 鄉論・清議(publicOpinion.ts,2026-07)

月旦評(§3.5)評的是**人**,文名(§6.15)量的是**一個士人的聲望**。兩者都沒說到**地方**。在一個靠**察舉**取士的時代 —— 地方賢達推舉地方人才 —— 一個郡的**鄉論**就是人才願不願意出頭的那套機制。潁川出了一整代公卿,不是因為那裡的水特別,而是因為它的鄉論是個**運作中的制度**。

**鄉論刻意不落存檔**,它就是「這座城當下治理得如何」的和:

`鄉論 = 34 + (民心−50)×0.28 + min(18, 文教×0.22) − min(16, 貪腐×0.16) − min(10, 積案×0.1) + 律令(峻法 −6 / 寬刑 +2) + 名士坐鎮 min(14, (最高(智+魅)/2 − 60)×0.35)`

—— 沒有存檔欄就不會走樣,而且**你治理一改善,它當季就變**。

| 鄉論 | 稱謂 | 買到什麼 |
|---|---|---|
| ≥78 | **衣冠淵藪** | 訪賢成功率 ×1.35、招攬 +10%、察舉品第 +6 |
| ≥60 | 鄉論稱美 | 依比例遞減 |
| ≥40 | 鄉里尋常 | 50 為中性點,全 ×1.0 |
| ≥22 | 士人不至 | 反向 |
| <22 | **為鄉里所鄙** | 訪賢 ×0.65、招攬 −10% |

- 接線三處:**搜索人才**(resolution 的 `successMul`)、**勸降**與**在野招攬**(`esteemBonus`)。
- 意義:文教、律令、貪腐、積案這些內政軸,第一次直接換算成**人才**。峻法收得齊稅,但清議出不來 —— 這正是諸葛亮治蜀與潁川士風的分野。
- 入口:城市面板「鄉論」條(<30 轉警示色,附白話後果)。**無新存檔欄**。

---

### 3.8 門生故吏(patronage.ts,2026-07)

舉薦(§3.1)本來就在跑 —— 徐庶薦諸葛亮、荀彧薦郭嘉。遊戲忘掉的是這項制度**真正危險的那一半**:**舉主的關係是終身的**。「門生故吏遍天下」是史書用來形容袁氏的話,而那不是一句稱讚,是在描述**一項軍事資產** —— 四代人的舉薦意味著袁紹起兵時,別人家的部下跟著他走。

現在一次舉薦會留下印記(`Officer.patronId`):

- **主辱臣憂**:舉主與門生同朝時,**舉主忠誠 ≤45 → 門生每季 −1**;**≥80 → 每季 +0.5**。中間不動 —— 只在兩端有感。舉主不在同一國(或已死)則無此效。
- **故吏相隨**:舉主**割據自立**時,同城的門生**優先於**一般「忠誠動搖的同僚」被帶走,而且門檻更寬 —— **忠誠 <60 就跟著走**(一般同情者是 <45)。義務比意見走得遠。帶「忠義」性格者永不隨行。
- 反過來:一個被你**善待且滿意**的舉主,等於在他舉薦過的每一個人身上都掛了一筆常設忠誠加成。這與 §4.10 行賞直接串起來 —— 賞一個人,可能同時穩住五個。
- 入口:武將卡顯示「舉主:某某(其心已離 —— 主辱臣憂…)」與「門生故吏遍天下 —— 所舉 N 人(在朝 M)」。存檔欄 `patronId`,舊檔缺欄 = 無舉主。

### 3.9 訪賢勝算與唯一的機率接縫(2026-07-27)

俘虜面板早就逐策顯示勝算(`estimatePersuasion`),**在野招攬從來沒有**:招聘、賄賂 300 金、獻上名品、遣人舌戰,四種花費全是盲賭。

不能直接接既有的 `previewRecruitChance` —— 它是 `attemptFreeAgentRecruit` 的**手抄簡化版**,早已漂走:少算宿怨、舊部歸心、報恩、知遇、主義相合、舉薦作保、三顧之誠、賄賂**八項**。接上去等於在畫面上承諾引擎不認的數字。

改法是**把機率從 attempt 裡整段抽成 `freeAgentRecruitOdds`**,attempt 自己也呼叫它,`previewRecruitChance` 刪除;拒絕理由一併搬進去,於是「事前警告」與「事後解釋」共用一份判斷。store 側同理:`recruitFreeAgent` 組參數那三十行抽成 `freeAgentRecruitInput`,estimator 與它同源。

契約測試釘的就是這件事:**用預覽值 ±0.001 當 rng 餵給 attempt,低於必成、高於必敗**。日後誰在 attempt 裡加了預覽沒有的項,這條當場翻紅。
...
### 4.3b 都督之權:違令與擁兵自重(legion.ts,2026-07)

軍團都督不是一個副程式。他手上握著你一大部分的軍隊、在一條你看不到的戰線上、憑一年前你寫下的一句方略行事。歷史上這種安排規律地產出兩樣東西:**一個安靜地照自己想法做事的統帥**,和**一個認為不再需要你的統帥**。

兩者都由同兩個數字決定 —— 他有多忠,以及他身邊站著全國多少軍隊。**全無隨機**:你養得住又沒讓他坐大的都督永遠聽話,而面板上一眼就看得到。

- **違令**(將在外,君命有所不受):忠誠 **<58** 且**智力 ≥72** 的都督會把你的方略換成他自己的 —— 武 ≥ 統者改行**蠶食**,反之改行**固守**。智力不足者沒有自己的方略,照令而行。季報直說「都督不奉方略」。
- **擁兵自重**:軍團兵力佔全國 **≥25%** 且忠誠 **<70** 時,他每一路發兵都會「為前線安全計」**留下 (70−忠誠)/200(頂 35%)**的兵在城裡。軍團戰報附「留兵 N%」。
- **權臣坐大**:軍團兵力佔比 **≥30%** 且忠誠 **<60** 時,該都督在 §7.5 的謀反判定裡多吃一份權重(至多 +0.06,與朋黨/門閥加成同流匯入 `factionBoost`)。
- 對策就是你本來就該做的事:**行賞**(§4.10)、**分軍團**(別讓一個人握半個國家)、或**換都督**。
...
### 4.8 兵制:更卒・世兵・募兵(conscription.ts,2026-07)

這個遊戲裡的軍隊一直是同一種來歷:徵兵從人口裡拉人,他們吃糧,關係到此為止。三國在這件事上**沒有一項是一致的** —— 魏吳蜀之間最鋒利的制度差別就是**怎麼把兵弄到手**,而那決定了各自的軍隊能拿去做什麼。

| 兵制 | 徵兵 | 抽丁 | 民怨 | 軍餉 | 耗糧 | 練度/季 | 缺糧逃亡 |
|---|---|---|---|---|---|---|---|
| **更卒**(編戶皆兵,預設) | ×1.00 | ×1.00 | ×1.00 | — | ×1.00 | — | ×1.0 |
| **世兵**(士家世襲) | **×0.82** | ×0.55 | ×0.50 | — | **×0.85** | **+0.5** | **×0.6** |
| **募兵**(重賞之下) | **×1.28** | **×0.35** | **×0.30** | **每千兵 14 金/季** | ×1.00 | **+1.0** | **×2.4** |

- **世兵**:曹魏士家制 —— 幾乎不費錢養、不會跑、閒時自耕(耗糧 −15%),代價是**兵源封閉**(徵兵 −18%,士家就這麼多人)且**士家不入民籍**,這批人永遠從你的納稅人裡減掉。
- **募兵**:錢能找到徵兵找不到的人 —— 流民、遊俠、別人的逃兵。鄉里幾乎無感(抽丁 ×0.35、民怨 ×0.3),操練也好。**但每季都要發餉**。
- **有意思的破局點是「募兵打一場超出府庫的仗」**:欠餉直接變成逃亡 —— 欠餉比例的 **1/10** 當季就走人,城中民心再 −1~2,季報明說「重賞之下所聚,無賞則散」。史上就是這樣塌的。
- **軍餉由駐軍所在城的府庫支付**(不是總庫),且**以當季本身的帳算** —— 本季收入在發餉日之前已入庫,所以一座賺得夠的城不會只因為「月初箱子是空的」就被判欠餉;真正的欠餉是**收入加起來也不夠**—— 於是「前線屯重兵而該城貧」本身就是一個要解的問題。
- **AI 亦有兵制**:富而好戰(每城均金 ≥2200)者募兵;窮而守成/暴虐(<700)者行世兵;其餘更卒。
- 入口:**國政面板 ▸ 兵制**(行募兵時直接顯示全境兵數與每季軍餉總額)。存檔欄 `serviceSystem`,舊檔缺欄 = 更卒(即舊行為)。


### 4.9 糧秣簿:出師之前先問糧(campaignLedger.ts,2026-07)

**隨軍糧本來就在跑** —— 縱隊出發時自源城支糧、每季吃一份、糧盡當季逃亡一成(convoy.ts)。缺的是**玩家看不到**:你拉個兵力滑桿、按下出陣,然後在四季之後的山那頭才知道結果。

**出陣面板**(MarchPicker)兵力滑桿下方現在直接算給你看:

| 欄 | 意思 |
|---|---|
| **隨軍糧支 N 季** | 這支軍能走幾季(行程幾季就在旁邊) |
| **自本城支糧 X 石** | 從源城實際搬走多少 |
| **本城餘糧支 M 季** | **沒人記得檢查的那個數** —— 你把糧帶走之後,留守的兵還能吃幾季 |

三檔判詞(顏色同步):**糧秣充足** / **僅堪一行**(本城餘糧 ≤1 季,或隨軍糧只夠一季)/ **糧不足行**(直接標出缺幾石,途中必乏食)。

- **順手拆掉一處雙算**:軍隊面板原本自己寫 `food / (troops×0.25)` 算餘糧季數;現在面板、出陣簿與季度結算共用 `armyEndurance()` 一個模型,`FOOD_PER_TROOP_PER_SEASON` 改了不會有兩處各說各話。
- 純讀取,不動任何狀態 —— 它只是提前把實話講出來。


### 4.10 軍法・賞罰:賞不逾時,罰不遷列(militaryLaw.ts,2026-07)

從第一場戰役起,遊戲就一直在記每個武將的戰功:破陣幾次、下城幾座、單挑、生擒、斬首。它從來沒問過那位主公真正被史書評判的那個問題 —— **你賞了沒有?**

「賞不逾時」不是一句講慷慨的成語,它講的是**不賞會怎樣**。一個下了三座城而寸賞未得的人不會就這麼算了:**帳目未清的每一季,他的忠誠都在掉**,而 §7.5 就在那道斜坡的底下等著。

- **軍功**由**既有的武功簿推導**,不新增任何累積掛鉤:`破陣×3 + 下城×8 + 單挑×2 + 罵倒×2 + 生擒×2 + 諜報×2 + 斬首/2000 + 內政×0.5`,減去**已賞**(`Officer.meritRewarded`)即為**未賞**。
- **功高不賞則怨**:未賞 ≥12 每季忠誠 **−1**,≥26 **−2**。
- **行賞**(武將卡):費金 = 未賞 ×55,忠誠 +min(18, 未賞×0.8),另給經驗。錢由**首都府庫**出。
- **軍過**= 敗績×3,減去已罰。**軍法四等**:

| 軍法 | 清過 | 其人忠誠 | 眾將忠誠 | 失威 | 他去之險 |
|---|---|---|---|---|---|
| **申飭**(責而不辱) | 3 | −1 | — | — | — |
| **杖責**(明法而後戰) | 6 | −6 | **+2** | 1 季 | 5% |
| **降職**(奪其兵柄) | 12 | −12 | **+3** | 2 季 | **15%** |
| **斬首**(揮淚斬馬謖) | 全清 | 死 | **+5** | — | — |

  —— 罰得越重,越能服眾,也越可能把人逼走或直接損失一員大將。這正是諸葛亮那一刀的兩難。
- **AI 亦賞不逾時**:每季挑欠得最多的一位,首都出得起就付掉。**不必給 AI 模擬感恩,它只是照付帳單** —— 於是機制天然對稱,而這筆帳實際上只有玩家會賴。
- 入口:武將卡「軍功簿:未賞 N · 未罰 M」+「🎖 行賞」與「⚖ 軍法…」;未賞 ≥12 時直接寫明「功高不賞則怨」。存檔欄 `meritRewarded` / `faultPunished`,舊檔缺欄 = 0(即全部未賞 —— 第一次打開武將卡就會看到一筆該付的帳)。


### 4.11 傷兵與撫恤(veterans.ts,2026-07)

戰損一直是一個「離開棋盤」的數字。所有記載都說不是這樣:一支敗軍的「損失」裡,更大的一部分是傷者、散卒、以及半個月後自己走回來的人 —— 而主將**怎麼處理他們**,決定了他多快能再打一仗。

**傷兵營**這棟建築從第一版就在,它治的是**武將**,對軍隊什麼也沒做。現在它做它名字上寫的事。

- **傷亡之分**:守方**守住陣地**時,傷者(而非死者)佔戰損 **30%+12%**(有醫館再 +8%,上限 55%);**沒守住的一方把傷者留在原地**(城破則無傷兵可收)。
- **每季療傷**(`City.wounded`):
  - 歸伍率 = min(0.82, **0.34** + 醫館/傷兵營等級×0.09 + min(0.12, 最高智力/700) + 藥材足額×0.14);
  - 死於創 = max(0.03, **0.20** − 照護×0.8);餘者六成**廢疾還籍**(回到人口,而非兵)。
  - **藥材按人頭消耗**(每傷兵 0.4),不夠就只有部分人吃得到藥 —— 藥材從一項商品變成「你預期會被圍的那座城裡該屯的東西」。
  - 無人照料:約三成自己好、兩成死;滿級傷兵營 + 藥材 + 智將坐鎮:**大半回得來**。
- **意義**:守城戰從純消耗變成一道**投資題** —— 醫館、藥材、留一個智力高的人在城裡,都是能把兵拿回來的錢。
- 入口:城市面板「傷兵」條(有傷才顯示,≥駐軍 8% 轉警示);季報逐城報「傷卒愈而歸伍 N 人(廢疾還籍 M,死於創 K)」。存檔欄 `City.wounded`,舊檔缺欄 = 0。


### 4.12 整編:新兵稀釋與傷癒歸伍(reorganization.ts,2026-07)

**練度**與**老兵度**掛在城上,不掛在人身上 —— 於是一座練到 80 的要塞可以吸收兩萬名新徵之卒,帳面上**還是 80**。所有記載都說反話:**新兵是會稀釋的**。這正是「一季重建起來的敗軍從來不是那支敗軍」的原因,也是曹操的**青州兵**之所以重要 —— 一整批已經打過仗的人,整建制併入。

算式只是加權平均,但它改變了玩法:

- **徵兵**(§4.8):新徵之卒素質為 **0**,`練度' = (練度×原兵 + 0×新兵)/(原兵+新兵)`;**老兵度同理**。一次爆兵買到的是數量,付出的是質量 —— 而 §1.18 的軍器與 §4.8 的兵制決定你一次能買多少。
- **傷癒歸伍**(§4.11):傷卒素質記為 **70**,**高於一般駐軍** —— 他們是已經挨過打的人。於是一座經營良好的傷兵營,價值**高於它還回來的人頭數**:它同時在還練度。
- **併軍**:`mergeQuality` 依人數加權,供部隊合流使用。
...
#### 5.1.1 戰場的「機制可見化」(2026-07-26)

戰場的表現層本來就厚(受擊 flinch/血霧、屍橫焦痕、箭雨落點、運鏡震屏、陣形可視化、天候粒子…),但深挖後發現真缺口全在同一處:**機制在跑,畫面沒說出來**。

- **向背**(`battle3d/facing.ts`):`facing` 一直決定背擊 ×1.25、側擊 ×1.12,以及正面槍陣 braced 把騎兵衝鋒壓到 ×0.7 —— 而 3D 場景與 HUD **一處都沒讀它**,每個單位在畫面上永遠朝同一方向,玩家因此**打不出**這些加成。現在模型按 `facing` 轉向(最短弧插值),選中/游標所指的單位腳下畫三色扇區(正面藍=braced 弧、兩側琥珀、背後紅=×1.25 窗口),戰鬥預判卡點名「擊其背後 ×1.25」。角度不硬編方向表而是幾何推導(走到 `hexDirection` 同意的鄰格取方位),避免和引擎漂移;`hitArc` 複製 `dirGap` 判定,測試逐一驗六向 × 五種入射並釘住三個倍率。
- **狀態徽記**(`battle3d/statusBadges.ts`):九種 `TacticalStatus` 棋盤上原本只畫 🔥 與「糧」,其餘七種只在側欄 chip —— 而看不見的幾種**直接改數值**(陷亂打得更弱受得更重、據守減免、詐敗是你自己設的陷阱)。現在全部上名牌(最要緊優先、上限三個);`chained`/`revealed` 刻意不上(已有連線與爆發演出)。側欄改吃同一張表,兩處不可能對不上。**`Record<TacticalStatus['kind'], …>` 是窮舉的**,以後新增狀態忘記給徽記會編譯失敗。
- **非 effects 的狀態**:**衝**衝鋒勢(奔 ≥2 格,騎兵至 +32%)、**矢**矢盡(無齊射可用,此前照樣拉弓)、**疲**疲憊(freshMul 最多 −30%,模型同時站低、呼吸放慢)、**驍**驍勇(`kills` 破敵部數 / `damageDealt` 陣斬人數,此前完全沒顯示)。**注意 `kills` 計的是擊潰敵單位數(每潰一部 +1)不是斬殺人數** —— 用兵力數當閾值永遠不會觸發。
- **地利**:高地優勢早就是實質機制且雙向按兵種細分(弓兵 hill ×1.25、騎兵 ×1.30;守方 hill 0.9/mountain 0.85),但只在預判卡可讀。名牌加 **地** 得地利 / **陷** 地形不利,倍率**直接呼叫引擎的 `terrainDamageMod`/`defenderTerrainShield`**,不複製表。
- **轉身接戰 + 揮擊**:此前 lunge 只做位移,單位朝著舊方向側身平移過去打人,讀起來像推擠;現在攻擊時朝向目標並全身順勢前傾扭轉(槍兵短刺、刀馬撲上)。
- **順手修真 bug**:主檔 `if (!battle) return null` **之後**還呼叫 useEffect(敵將叫陣),`battle` 為 null 的渲染會整個跳過該 hook 而使 hook 順序不一致 —— 與 CityMapScreen3D 踩過的同一個坑。已把 effect 提到 guard 之前。
...
- **大地圖可見**:頓兵 ≥3 季的圍城 token 亮 **頓N** 徽章(≥5 季轉紅),營寨升起漸濃的炊煙 —— **看得見冒煙的營,就是已經站到會生疫的營**。
...
- **大地圖可見**:圍城營的 3D 營寨外圈環繞小小的器械剪影(數量隨 park),token 上另有 **⚒N** 徽章;軍隊面板同顯 ⚒N(懸停「器械如林/攻具略備/始造攻具」)。季報報「守軍縱火焚其攻具 —— 器械毀 N 具」。存檔欄 `MarchCommand.siegeEngines` / `Army.siegeEngines`,舊檔缺欄 = 0。
...
- **戰果是持久的**:夜襲跑在圍城營的季結算「之後」,所以它同時改寫 `keptCommands` 與已導出的 `Army`(兵/器械/糧/疲勞/士氣)—— 否則整場劫營會在季界蒸發。
- **一季一劫**:夜襲是**每季**判定一次(在季界),不是每旬 —— endSeason 每旬跑九次,不設閘會一季觸發九回;而上方的**突圍**刻意每旬(圍城糧耗本就每旬,故守軍每旬都有機會傾城而出)。
- **它贏的從來不是圍城本身,是圍城的裝備** —— 天亮之後敵軍還在,只是沒有東西可以攻城了。

### 5.18 逐擊戰報(tacticalNarration.ts,2026-07-26)

戰場每一擊背後算了十幾件事 —— 兵種相剋、地形、朝向側背、夾擊人數、精銳、兵裝、疲勞、擱淺、天候 —— 然後全部折成**一個傷害數字**,消失。

戰報**確實會敘事**,但只寫**例外的一擊**(合擊/立防/衝鋒/甕中捉鱉/銜尾掩殺/詐敗/困獸猶鬥/腹背受敵/會心/軍心崩潰/主將陣亡)。**平凡的一擊什麼都不寫**,唯一的聲音是 `pickVoiceLine`,而全庫 **2,218 名武將中只有 39 人有戰場語音**(1.8%)。實測:一場九回合分出勝負的野戰,全場戰報 **5 條**,沒有一條講誰打了誰;120 場 AI 對 AI 觀測,近戰關鍵詞 **4 次**,而天候 915 次。

- **主導因素**:每一擊挑**最能解釋這個結果的那一件事**寫出來,優先序為 伏兵 → 擱淺 → 背刺 → 側擊 → 相剋(正克/最忌兩向都說)→ 三面夾攻 → 兩軍夾攻 → 攻方所踞之地 → 守方所憑之險 → 精銳 → 師老兵疲 → 夜戰 → 天候。
- **戰果分級**:按傷亡佔敵**滿編**之比 —— ≥30% 陣列摧折 / ≥18% 大挫其鋒 / ≥8% 陣腳一鬆 / >0 略挫。另報**還擊**折損與**箭矢已空**。
- **敘事不耗隨機**:輪替動詞用的是 `damage + turn + col` 導出的整數,**不是 `rng()`**。第一版從戰局共用亂數流抽同義詞,等於每一擊多走一格隨機 —— 120 場觀測當場看到整個戰役位移(破牆 24/40 → 17/40)。**風味文字必須是免費的**。
- **戰報抽屜**新增「逐擊」開關(預設開)。逐擊行 `kind: 'blow'`,與戲劇節拍 `'event'` 分開,所以可以關掉;也因此不會搶走跑馬燈(跑馬燈只吃 `voice`/`arrival`)。
- 實測:同一場戰鬥 5 條 → **25 條**;120 場觀測近戰 4 → **982**。

### 5.19 連携合擊認你在遊戲裡結的義(tactical.comboBondMul,2026-07-26)

戰場合擊本來只認**一份寫死的 11 對名單**(劉關張、孫策周瑜、夏侯兄弟…)。而 `teamDuel` 與單挑面板**早就**是 `areBonded(...) || areSwornBrothers(...)` —— 也就是說:**你在遊戲裡桃園結義、聯姻、收義子,單挑認,戰場不認。**

- **合擊倍率**(`comboBondMul`,取最親的一條):**宿怨 ×1.0**(有仇的人不會配合,**優先於一切**)→ 正典名對 **×1.30**(維持原值,不動平衡)→ 結義按**義結深化**深度 **義交 ×1.15 / 義結金蘭 ×1.25 / 生死之交 ×1.35** → **骨肉同陣 ×1.18**(靜態宗譜 + 執行期婚姻/收養)→ 無 ×1.0。
- **戰報點名那條緣分**:「張飛 × 關羽 合擊 —— 生死之交,如左右手!」
- **因緣入陣**:執行期的 `runtimeBonds` / `family` 在 `store.startTacticalBattle` **開戰那一刻**篩出「本盤上場者」再蓋章到 `TacticalBattle.oathBonds` / `familyTies`。選這個接縫是因為它是**五個 setupTacticalBattle 呼叫點唯一都會經過的地方**(其中一個在彈窗裡);只在新戰局蓋章,因為這個 setter 每回合都會跑。存檔只帶本盤相關的幾條,不是整個關係圖。

### 5.20 陣前招降(tacticalSurrender.ts,2026-07-26)

戰場上原本**完全無法受降**:士氣歸零的敵將只會潰走,你唯一能做的是追上去砍死他;要收人只能等戰後擲骰。而勸降需要的輸入**全都現成** —— 魅力、忠誠、故主之義、關係網、他有多殘破。

- **誰肯聽**:潰走中、或士氣 ≤25、或殘部 ≤ 滿編 25%。**忠誠 ≥95 者聽都不聽**;與你有**宿怨**者寧死不降。
- **喊得到多遠**:2 格。耗 **1 AP**。
- **一戰一次**:每名敵將**只能勸一次**(`surrenderCalls`),拒絕不能重擲。
- **成算**:`0.10` + (魅力−60)/200 + (60−忠誠)/200 + 殘破度(潰走 +0.20 / 士氣≤15 +0.12 / 其餘 +0.05)+ 殘部 ≤20% 再 +0.10 + **故主之義 +0.18**(他曾在你麾下)+ **義兄弟 +0.22 / 骨肉 +0.14** − **主將 0.18**(統帥不棄其軍)+ 戰局氣勢 ±0.06,夾在 **0 ~ 0.75**。
- **拒絕有代價**:對方**士氣 +10**(厲聲拒之,殘部反為之一振),你白費一個行動。所以這是個決定,不是免費按鈕。
- **歸降之後**:該部即刻離場(兵力計入其本方損失),**兩格內同袍士氣 −8**(一將歸降,左右奪氣);歸降者於**你守住戰場**時成為俘虜 —— 輸掉當天,俘虜被自己人接走。
- **AI 對稱運用**:AI 亦會對**你**的殘部喊話(成算 ≥0.30 才喊,免得白費一個活化又反而堅其心)。

### 5.21 十面埋伏真的有人埋伏(tacticalSetup,2026-07-26)

`ten-ambush` 的舊規則是「**剛好**被部署在森林格的部隊開局隱蔽」。部署線按行列排,不按掩蔽排 —— 所以 120 場觀測**一次都沒發生**。一個叫十面埋伏、卻沒有人埋伏的陣形,只是個名字。

- **入林設伏**:用該陣形的一方,伏兵翼**在第一回合前自己走進最近的林子**(至多 4 格內、無人佔用的森林格)。
- **有界**:只出動**該方半數**(伏兵翼,不是整軍消失),**主將留在明處**,且掩蔽必須近到「他們本來就可能行軍過去」—— 不會瞬移到敵後。
- **AI 選得到了**:`pickAiFormation` 從來沒把 `ten-ambush` 放進候選,所以 AI 根本不可能設伏。現在有林可藏 + 守勢才推入候選,而且**必須推在「因軍制宜」預設之前** —— `cands.find(usable)` 取第一個命中,否則守勢的魚鱗永遠先贏。智力門檻用陣形**自己的** `minIntelligence: 95`,不另設一道較低的門(那會讓程式讀起來像是中人之才也能設伏)。
- **伏兵驟起會寫進戰報了**:引擎給 ×1.3(夜 ×1.5)並使敵陷亂,卻**一句話都不寫** —— 觀測到的唯一一次「伏兵」其實是斥候識破了一個從沒發動的埋伏。現已列為 §5.18 敘事的最高優先項。實測 0 → 3(120 場)。

### 5.22 水戰與糧車終於看得見(2026-07-26)

`shipClass` 在 `src/game` 底下被讀 **49 處**(船身戰力、深舟擱淺 ×0.55、暈船、火燒脆弱度、連環/接舷/撞角/火船),在 `src/ui` 底下被讀 **1 處** —— 而那一處是**港口面板,不是戰場**。赤壁盤上玩家分不出樓船和走舸,也看不出哪艘船下一步會擱淺。`isSupply` 同理:引擎 26 處(燒掉糧車讓**整方**陷入 `starving`),UI 1 處,而那 1 處只是個 3D 模型。

- **艦徽記**:名牌上顯示**船級名**與船身戰力倍率;**淺**(擱淺,附實際倍率)、**暈**(暈船,附倍率與「連環可穩」)。
- **糧徽記**:自家糧車金色「護住它」,敵方糧車紅色「燒了它 —— 烏巢之計」。
- 與 §5.8 地利徽記同一條契約:**數字由引擎自己的函式算出後傳進來**(`shipPowerMul` / `groundingMul` / `isGrounded` / `seasickness`),徽記永遠不會承諾戰鬥模型不給的東西。

### 5.23 士氣的下半段本來到不了(2026-07-26)

上一批把「AI 幾乎不追擊」列為待辦。追下去發現那只是症狀 —— **整個士氣系統是裝飾性的**。

80 場 AI 對 AI 實測(修前):

| 量測 | 修前 |
|---|---|
| 一個部隊整場的**最低**士氣(平均) | **81.7**(開局就 80) |
| 曾跌到 ≤25 的部隊 | 3% |
| 被全殲時的士氣(平均) | **90.3** —— 滿懷鬥志地被殺光 |
| 潰走事件 | 5 次 |
| 潰走中被追殺 | **0** |

也就是說 §5.1 的 動搖(<40)/ ≤15 / 潰走(0)/ 追擊掩殺 ×1.5·×1.8 / 收攏 / 督戰壓陣,以及新做的 §5.20 陣前招降,**全部鎖在一扇打不開的門後面**。兩個原因:

- **旗鼓可定,不可復其銳(`MORALE_RALLY_CAP = 75`)**:每回合的被動士氣回復(將旗 +3、旗令 aura、眾寡懸殊 +3、順勢 +1~2)原本**無條件漲到 100**,包含接敵前那五六回合的行軍 —— 於是所有人是**在 100 士氣的狀態下**才開始打,而不是開局的 80。現在被動回復只能把人**扶到 75**;已在 75 以上者不動(旗子從不使人洩氣)。**鼓舞計略不受此限** —— 這正是重點:把殘部拉回鬥志,得花一張牌。
- **死傷奪氣(`BLOOD_MORALE_COST` 50 → 130)**:原本打光**整支**部隊只扣 50 士氣。實測「打光一支部隊期間士氣淨掉約 35 點」—— 兵先流乾,氣才會盡。130 之下,傷亡過半的部隊已在崩潰邊緣,約六成即潰。
- **潰不成軍,遂去**:潰兵跑回己方邊緣後原本**停在那裡站一整場**(22 次潰走裡有 9 次到終局還杵在邊緣上)—— 既沒損失也沒用處,等於「打崩敵人」幾乎不用付代價。現在跑到家就以 `retreatUnit` 離場(一成計為離散)。**追擊追的就是這個時間差。**
- **追亡逐北是騎兵的活**:`pickAiTarget` 完全沒看潰走 —— AI 只會順手打掉**剛好貼著**的潰兵(`pickAdjacentTarget` +300),從不去追。現在騎兵權重 ×0.5(優先追),其餘兵種 ×1.6(不為追亡而破陣)。
...
| 追擊關鍵詞(120 場) | 0 → **20** |
| 士氣潰關鍵詞(120 場) | 70 → **141** |
| 陣前招降(120 場) | 5 → **13**(機制互相餵食:有人崩潰,才有人可招降) |
| 陣亡(120 場) | 98 → **79**(部隊改為潰散而非被殲) |

**平衡 A/B**(把兩個常數暫時調回舊值對照):槍剋騎 79%→79%、騎vs槍 29%→29%、弓vs槍 67%→67% 完全不動;騎vs弓 38%→**46%**(追擊本來就是騎兵的長處);騎vs步 46%→38%;鏡像先手 81%→**75%**(離 85% 上限更遠)。兵種三角未受損。

### 5.24 烏巢之火在自己的機制裡到不了(2026-07-26)

**燒糧**(糧車被毀 → 整方 `starving`:逃亡 + 戰力衰減)的檢查讀的是 `b.units` —— **這一旬開始時**的棋盤。可是糧車最常見的死法是那個 tick **稍後**才發生的火燒(糧車燃燒率 ×3,正是烏巢那一幕),而回合結束時 0 兵單位一律被濾掉,下一旬再也看不到那具殘骸。實測:120 場裡糧車被毀 **17 次**,挨餓的軍隊 **0 支**。**為烏巢寫的機制,烏巢到不了。**

改結算在 `removed` —— 那是「本回合離場者」的**唯一定義**。於是被打爆、被燒毀、被城上滾木砸掉、被計略帶走,全部算數;以後任何新的傷害路徑把糧車清零,都會落進同一個清單而自動生效,不必再接一次線。0 → **17**。

### 5.25 觀測工具說實話(scripts/tactical-watch.ts,2026-07-26)

這支腳本連續三次把**自己取樣的缺陷**報成「機制是死的」。修法不是一次次補樣本,而是改變量測方式:

- **從戰局狀態量,不從戰報字串猜**:`移動` 與 `變陣` 永遠是 0,因為位移**刻意不寫戰報**(會淹掉抽屜)—— 那一列在量一個不存在的東西,而旁邊那個永遠亮著的 ⚠ 只會訓練人忽略警告。改為直接比對每回合單位座標與陣形欄位:**機動 30% 的單位回合有換格、臨陣變陣 6 場**。
- **`糧盡` 那列的正則從來對不上引擎真正寫的那句**(`糧車被焚!三軍乏食…`)—— 量了半天量的是別的字。
- **取樣不可自我矛盾**:harness 傳給 `pickAiFormation` 的 int 是一個數,主將**實際**的 int 是另一個數(62),而臨陣變陣的門檻是 75 —— 於是分支結構性不可能觸發。
- **取樣不可反相關**:「攻方不反制選陣」只出現在 `n%4===3`(全奇數),而「攻方主將夠聰明」是 `n%2===0` —— 兩個必要條件被我自己排成互斥。**反相關的取樣藏得住活分支,和藏得住壞分支一樣好。**
- **糧車是逐圖 opt-in 的**(18 張預設戰場只有 3 張有),純程序化棋盤一具都不會有 —— 現在每七場派一場官渡/長坂。
- **車輪戰是玩家專屬動作,AI 無呼叫路徑** —— 直接印在報表末尾,不讓那個 0 看起來像發現。

### 5.26 脫離戰場 — 單部隊撤出(2026-07-27)

`retreatUnit` 在引擎裡存在很久,**全庫只有 `tacticalAi.ts` 呼叫**:AI 能把打殘的一部拉出戰場保住殘兵,玩家唯一的退路卻是「撤退」——整場認輸。

規則(引擎原有,未改):**主將不可脫離**;單位須退至**我方陣後兩格內**(攻方 col ≤ 2、守方 col ≥ 寬−3);脫離時 **10% 計為潰散折損**,其餘保全,單位離場。

補的是玩家入口。做法沿用攻城三動作的教訓:引擎側原本以「回傳原戰局」表示不合法,對 AI 夠用,對玩家等於**一顆按了沒反應的按鈕**;因此另立 `canRetreatUnit` 判定,按鈕只在引擎說可行時浮出。`tacticalRetreat.test.ts` 成對釘住 —— 說可以就必須改變戰局,說不可以就必須原封不動。
...
- **名將台詞**(officerLines.ts,2026-07 由 128→138 位):3D 單挑中名將以**自己的**挑釁+必殺台詞開口(未收錄者回落 battleLines.ts 的 persona 通用詞;台詞亦作卡面 flavor,見 §2.13)。補入有單挑戲份卻此前無專屬台詞的顏良、文醜、華雄、紀靈、張任、魏延、關興、丁奉、廖化、王雙 10 位,再補李典、于禁、曹仁、曹洪、潘璋、關平、王平、蔣欽、朱然 9 位二線名將(共 128→147)。
...
- **鄰近補位會把城送給錯的人(2026-08)**:`buildInitialCities` 對劇本沒明列的城,採「最近的已列城之主」補位(60px 內)。這條規則本身是對的 —— 後來加進地圖的關隘與衛星城才不會變成無主的空洞 —— 但它會**跨過山川與州界**:
  - 207 盤張魯只明列漢中一座,補位就把**關中的陳倉、散關**也給了他,於是一個四城小國直接與曹操本土接壤;實測他**第 1 旬丟陳倉、第 12 旬丟漢中**,一年之內亡國,而他的主目標是「師君治漢中」。
  - 208/211 盤:益州北部的陰平、葭萌、梓潼、劍閣、白水關離武都(馬騰/韓遂)比離成都近,於是涼州軍閥憑空得了劉璋的門戶。213/215 盤:天水、上邽離漢中最近,張魯憑空得了隴右。
  - 修法是把這些城**明列進歸屬表**。⚠ 補位是**連鎖的**:明列葭萌之後,巴西的「最近已列城」從漢中變成葭萌,張魯的縱深又沒了 —— 改一座要回頭看旁邊那座。
  - 另兩條配套:張魯自建安五年即受漢廷鎮民中郎將之號,曹操西征在 215 年 —— 204/207/213 補上曹張互不侵犯;張魯姿態改為兵 ×1.30、城防 +26、民忠 +24(劉璋伐之二十年不能克)。
  - **第二批補位錯誤(2026-08-07 晚)**:**西陵**在 252/263/264/265 四張盤上被補位給了**魏**,而它是吳的上游門戶 —— 陸抗說「西陵、建平,國之藩表…若非其人,萬里長江,難以恃也」,它要到 272 年步闡叛降才易主(那正是 scn-272 的前提)。四張盤明列為吳,272 保持不動,兩邊都釘進 `proximityFill.test.ts`。
  - **把它寫成腳本了**(`scripts/fill-audit.ts`)。判準:一座**補位來的**城,其主在該城所屬的州裡**一座明列的城都沒有**,而那個州的多數是別人的 → 可疑。它報的是可疑不是錯(邊界城本來就會跨州),要人去看那一句話:**這一家在那個州有沒有根?** 第一次跑就撈出四類:易水給了全圖最南的楚(荊軻「風蕭蕭兮易水寒」的那個易縣)、壺關給了楚、譙給了齊/燕、陳倉散關給了蜀漢。共明列 21 座修正,可疑數 280 → 228;剩下的多半是**州別本身**掛錯(新城/武昌歷史上屬荊州而圖上掛在益州/揚州),那是另一件事。
  - ⚠ 腳本第一版的鍵正則漏掉帶引號的形式(`'yi-county': …`),把**明列**的城誤報成補位來的 —— 我照著它加了重覆鍵,`tsc -b` 當場 TS1117。修 audit 的同時要記得:資料檔的鍵有兩種寫法。
  - **張魯這一格追到最後(2026-08-07 晚)**:他在 204/207/207bl 仍守不住,而追的過程有一課 —— **我一直盯著漢中,而丟掉的一直是陽平關**。逐旬 diff 只印「城數」時他始終是四城(因為同時從馬騰那裡拿了安定),看起來一切正常;把 `evaluateGoal` 的 progress 逐旬印出來才看見 `1/2`。**數城數會蓋掉「丟的是哪一座」。**
    陽平關人口只有五萬五,守軍基數小,姿態乘數乘在小基數上還是小 —— 張衛「橫山築城十餘里」不是一支守關小隊,所以直接給守軍(`troops: 14000`)。另補劉表—張魯互不侵犯(荊州與漢中之間隔著秦嶺與東三郡,劉表一生沒有向漢中用過兵)、張魯兵力 ×2.00(閻圃:「漢川之民,戶出十萬」)。
    結果:204 由 0/3 到 1/3、208 由 0 到 3/3;**207 與白狼山仍是 0** —— 四城之國夾在劉璋(九至十三城)與曹操之間,史書上他活下來是因為鄰居都騰不出手,而那件事這個模擬表達不了。到此為止,不再硬掰。
- **三國線的開局姿態(2026-08,37 張)**:44 張三國盤裡只有 184/190/195/200/208 五張寫過姿態,其餘 39 張一律 `buildInitialCities(ownership)` 沒有第二個參數 —— 序章寫「你缺糧、你後院要反、你被圍在易京樓上」,數字上你和別人一模一樣。現在每張盤都有,220 之後諸盤另有一個共用底 `TK_LATE`(魏大而富 / 蜀小而險 / 吳據江而守),各盤再疊自己那一年的例外:
  - 194 曹操 民忠 −18(張邈陳宮迎呂布,郡縣皆應,只剩三城)、197 袁術 糧 ×0.55 民忠 −25(稱帝而淮南人相食)、197/199 公孫瓚 城防 +22~24 而民忠 −10~18(易京樓積穀三百萬斛,樓高十丈而人心已散)、198 呂布 城防 +10 糧 ×0.55(城牆還在,糧沒了)、204 袁譚/袁尚 民忠 −14/−18(兄弟鬩牆)、207 劉表 兵 ×0.80 糧 ×1.25(最富而最不設防)、211 馬超 民忠 −14(關中十部貌合)、213 劉璋 糧 ×1.30 民忠 −22(殷富而人心已貳)、244 曹爽 糧 ×0.75(牛馬騾驢多死於道)、249 曹爽 城防 −8 民忠 −20(挾帝在外而洛陽已閉城)、253 孫吳 糧 ×0.85 民忠 −8(暑月疫作,士卒大半病亡)、263 蜀漢 民忠 −14(黃皓用事)、280 孫吳 民忠 −28(金陵王氣黯然收)。
  - 立竿見影的三格:198 呂布主目標 0→2/3、袁術 0→2/3(兩人**都該亡**,而先前是連亡都亡不掉);199 公孫瓚「易京樓不焚」0→2/3;280 晉 83→94 城而吳 28→9(先前兩邊都不動)。
- **外傳三線的開局姿態(2026-08)**:戰國/楚漢/隋唐 **27 張盤此前一張姿態都沒有** —— 城的數值只剩三國地理,於是「戰國七雄」盤上秦據關中隴蜀三十八城卻一路掉到十五,坐擁江南的楚穩收三十六(這張圖的南方本來就富)。秦因此在**每一張**戰國盤的主目標都是 0。現補上三組姿態:秦耕戰而關中險、齊富而臨淄甲天下、趙胡服騎射而乏糧、韓地小而弩利、燕僻遠而貧、楚地大而政散、魏先霸而衰;楚漢是「項羽兵強而失人心 vs 劉邦約法三章、蕭何轉漕」;隋唐是「唐開倉賑民、鄭城堅而糧絕、夏得河北人心、瓦崗兵盛而內裂」。各盤再疊自己的例外(樂毅下齊七十城只餘莒與即墨、垓下四面楚歌兵少食盡、邯鄲圍三年不下…)。修後秦 40→29 城、兵四十七萬,居七雄之首。
- **時代漏法(2026-08 修)**:外傳三線借三國曆法軸(`startDate.year = 178`),於是**任何拿絕對年份當觸發的漢末內容都會照演一遍**。此前已補過兩次(林邑國早四百年建國 → `tribesOnBoard`;流民叛亂長出太平道 → `sectsAvailable`),而**黃巾總爆發**的 call site 沒接上守衛 —— 全 AI 觀察跑戰國盤,太平道從秦手裡拿走了**長安**。現由 `isLaterHanBoard` 擋掉,判準另有 `eraLeaks.test.ts` 釘住。同理,AI 即位的「220 年」門檻對外傳線是死鎖(跑不到 220 年),已改為只在三國線生效 —— 否則齊湣王「南面稱帝」這種以稱帝為主目標的盤,機制上永遠 0。
- **開局爵位(2026-08)**:各劇本的勢力現在**開局即帶史實名分**,不再一律從庶民爬起 —— 184 漢室即位為帝、190 董卓為太師(公)、197/198 袁術**稱帝**(仲氏)、曹操 侯(196–212)→ 公(213 魏公)→ 王(216 魏王)→ 曹丕 帝(220)、劉備 219 漢中王 → 221 帝、孫權 220 吳王 → **229 帝**;戰國七雄相王之後皆王(前 353 之前的三張盤仍是侯)、秦二世/唐高祖/薛舉/王世充/安祿山皆帝。爵位帶內政與招募加成(帝 ×1.35 / +25%),故此舉同時是**盤面平衡的一部分**,不只是稱謂。
- **AI 也走得完這條階梯(2026-08 修)**:此前 AI 卡在「侯」上不去 —— 進「公」須先拜丞相,而 `aiAppointments` 依表順序**先派滿各城太守**、一人一職,城愈多的勢力朝廷愈空(229 盤實測:魏 60 城、吳 29 城,非太守職位**掛零**)。現改為**朝官先於太守、丞相又先於軍師**,AI 因此拜得出丞相、進得了公王。
- **割據稱帝**:AI 即位原本要求「天下無帝」,而史實正相反 —— 劉備 221、孫權 229 稱帝都**因為別人先稱了**。現改為:天下無帝時照舊(每季 40%);已有他人稱帝時,只要與**其中一位**皇帝關係 ≤0 即可僭號自立(每季 15%)。用「其中一位」而非「全部」,是因為孫權踐阼時與蜀漢是盟友(蜀還遣陳震往賀),他要否認的只有洛陽那一位。
- **敕令冷卻改為每家一份**:`edictCooldowns` 原本只以敕令種類為鍵而玩家與全部 AI 共用 —— 蜀漢一下大赦,魏、吳、和**玩家**的大赦一起進冷卻。AI 側改用 `forceId::kind`(玩家側不變,舊存檔照讀)。
...
> **⑤ AI 背盟(§7.1,2026-08-07 新增)** —— 撕毀互不侵犯此前**只有玩家做得到**:`isHostilePermitted` 只在關係為 `neutral` 時放行,而 `breakAlliance` 是 store 上的玩家動作。後果不是「AI 比較守信」,是**好幾張盤的前提在 AI 手上演不出來** —— 211 渭南盤劉備與劉璋是互不侵犯(史實上他正是被請進去的),於是 AI 劉備永遠打不了益州,而他的主目標就叫「西取益州」。
> 現在 AI 每季有機會撕約,代價與玩家那一側同數(信譽 −25、對方積怨 +30、四鄰各 −8)。門檻刻意收緊:只撕 `non-aggression`(結盟另有聯姻/會盟系統管)、要真的接壤且對面有一座壓得下的城(壓境兵力 ≥ 守軍 1.3 倍)、我方總兵力 ≥ 對方 1.6 倍(**打不過的人不會背盟,只會守約**)、關係 ≤ +40、基礎 8%/季再乘君主性格的攻擊傾向。
> ⚠ 實測記一筆:這條**沒有**讓 211 盤的 AI 劉備拿下成都 —— 那張盤他 11 城 5.7 萬兵對劉璋 13 城 8.5 萬兵,本來就過不了 1.6 倍那一關。史實上他贏在張松法正的內應而不在兵力,那是另一個問題。

> **⑥ 守勢對人不對事(2026-08-07)** —— `forcePosture` 原本只回傳 aggressive/defensive,而守勢把攻擊門檻**對所有目標**打對折:一家被巨鄰壓住,就對誰都不敢動手。實測後果是小勢力在大國旁邊全部僵住 —— 211 渭南盤的劉備與曹操接壤,於是他連弱得多的劉璋也不打,而那張盤他的主目標叫「西取益州」。史書上被壓著的人做的正好相反:劉備西入益州、孫策東取江東,都是趁強鄰顧不上時往另一邊咬一口。新增 `overshadowingForce` 回傳**壓我的那一家**,守勢只對它生效;討伐霸主仍是例外。
> ⚠ 這個改動第一次上時把「同種子同世界」的重播測試打紅了,而真兇不在 AI ——見 §10 的名器登記那一條。

> **⑦ 多線作戰:處處進攻就是處處不進攻(2026-08-16)** —— `pickForceTarget` 每季本來就替一家選出一個全力級目標,`focusRelax`(×1.3)給它放寬門檻;但在此之前**沒有任何東西讓其餘方向收手**,那個「集中」機制只會加、不會減。後果是史書上的強國在盤上一律會崩:城多 = 邊界長 = 戰線多,一家同時在五個方向各打各的,每一路只帶得動本城七成的兵,於是每一仗都卡在勉強線上 —— **城打不下來,兵一批批填進去**。實測(6 輪 × 240 旬)開局最大的那一家末旬城留存:3 條戰線 94%、4 條 84–87%、**5 條只剩 59%**。
> 現在新增 `hostileFrontCount`(數的是**能打的鄰國**,簽了互不侵犯的邊界不算戰線),戰線每多一條,**非主攻方向**的攻擊門檻乘 `DISPERSAL_PER_FRONT`(0.85),下限 0.55 —— 五面受敵時偏師要有近兩倍於平時的把握才動手,而主攻那一路完全不受影響。
> **只有握 12 座城以上的一家吃這條規則**(`DISPERSAL_MIN_CITIES`)。三座城開三條戰線的張魯不是「多線作戰」—— 他的城互相挨著,打的是內線,隨時能集中;對他收縮只會讓他變呆,而他的主目標偏偏是往南取葭萌。§4b 的病是**邊界長到 outrun 集結能力**,那是大國才有的處境。
> ⚠ **效果是誠實的邊際,不是漂亮的勝利**:城留存 86%→88%、兵留存 63%→62%,大概率落在噪聲裡。不設城數門檻的版本看起來好得多(93%/70%),但那個收益主要來自**讓小國不敢動手**而不是大國學會集中 —— 門檻只改變小勢力的行為,而量的是最大那一家,收益卻掉了五個百分點。用錯的理由換來的數字不算數。**§4b 因此仍然開著:病因(戰線數)釘住了,藥還沒找到。** 三個版本的逐盤數字與那條推論見 `docs/CAMPAIGN-CHECKS.md` §4b-2。
...
- **抉擇事件鏈**:玩家為當事勢力(chooserRulerId 之主)時彈選項,他人走史實線(第一選項)。已實裝:
  ...
  - **夷陵連營**(劉備:依林結營避暑〔史實,蜀×0.5、吳天命+8,觸發「火燒連營七百里」〕或納馬良之諫謹慎結寨〔蜀×0.85、吳×0.95〕)——原本單一結算(蜀×0.5)升格為抉擇分岔。
  - **田豐諫止南征**(袁紹:囚之傾國南下〔史實,自貶天命、田豐忠−10〕或納諫持重緩圖〔河北兵×1.1〕)——官渡曹操一方早有 evt-wuchao(劫烏巢 vs 按兵)抉擇,此為補上的**袁紹視角**分岔。
- **行為事件的門檻按「每城」算,不是絕對值**(2026-08-02):「有錢/沒錢」是相對於要養多少城說的。原本 `倉廩盈溢` 的門檻是 `totalGold >= 8000`、`府庫空虛` 是 `totalGold < 800`,於是十七座城共 14,735 金(每城 866,全盤最窮)的黃巾在開局第二回合就被告知「府庫充盈,金帛山積」,而三十八座城的朝廷永遠跌不到 800,那條事件對大勢力等於不存在。現改為每城 2,500 / 每城 250,選項的金額也隨城數縮放(否則大勢力擺一場宴只花 2,000,那不是抉擇是白拿民忠)。同檔的 §錢法 早就是這個寫法(`totalGold > cities.length * 700`)。
...
- **外傳三線的事件與時代守衛(2026-08-09)**:戰國/楚漢/隋唐三線原本**一條事件都不會演** —— 實測 `scn-ws-changping` 四輪兩百回合觸發清單完全是空的,21 張盤全空。成因是三國那兩百餘條事件的守衛全是 `officer-alive: cao-cao` 這一類,而戰國盤上沒有曹操(`findFiringEventIn` 查不到人就回 false)。**這個天然隔離是對的**,結果是外傳那一側從來沒有人往裡面放東西。補 45 條(戰國 17 / 楚漢 11 / 隋唐 13 + 安史 8 有重疊)之後,戰國中位 13 條、楚漢 12、隋唐 9,零事件的盤 0 張。寫這一線的事件前必須知道三件事:
  - **年份軸是借三國的**(`startDate.year = 178`)—— 窗口寫 178–192,不是 -260。**年份擋不住任何東西**,鎖住它們的是「只有那條線才有的人」。`eraLeaks.test.ts` 釘住這條:外傳事件的 requires 至少要有一條 `hist-` 人物條件。
  - **同一線的盤共用人物池**(14 張戰國盤上 154 個人全在場),所以人物守衛擋得住三國線、**擋不住戰國盤彼此**。通用場面這樣寫沒問題;某一戰自己的鏈要另綁 `chain-xxx`。
  - ⚠ **隋唐線的人物池不對稱,是唯一的例外**:隋末群雄(煬帝/竇建德/王世充/劉武周/宋金剛/李密/屈突通)**不在安史盤**,而安史那批(安祿山/楊貴妃/哥舒翰/顏真卿/平陽公主/李建成/李世勣)**在淺水原、柏壁、虎牢盤上都在**。所以唐初事件配一個隋末專屬的人就擋得住安史盤,而**安史事件靠人物擋不住,必須綁 `chain-anshi`**。
  - ⚠ **照史書挑名場面會偏向名場面多的那一家**:第一版戰國十三條裡趙佔四條(胡服騎射/完璧歸趙/負荊請罪/李牧守邊),實測長平盤趙從 13 城漲到 24、秦從 41 掉到 33 —— 跟那張盤要講的事情相反。補秦那一側(鄭國渠/白起/王翦)之後才平。另外**效果的作用面要對得上勢力規模**:秦在戰國盤有 41 座城,單城乘數等於沒加。
- **戰役專屬事件鏈**(2026-08,自 184 黃巾之亂起):替單一劇本寫、按史實季節上鎖的鏈。黃巾之亂 —— **解黨錮**(春,盧植)→ **長社之火**(夏,皇甫嵩:順風縱火夜襲 / 固守待援)→ **廣宗易帥**(秋,盧植:不賂左豐而檻車徵 / 送賄保帥)→ **張角病死**(秋,無選項,須 `yt-guangzong` 已設)→ **卓無功,嵩代之**(冬,董卓:持重不進 / 強攻廣宗)→ **宛城之圍**(冬,朱儁:聲東擊西強攻 / 納降)→ **收印綬,削戶邑**(185–187,皇甫嵩拒賄而被削)→ **黑山之聚**(185+)→ **餘燼**(186+)。季節上鎖是必要的:不鎖的話會擠在開局第 2–5 回合演完,史實上這是跨兩年的事。
  - **卓無功,嵩代之**(2026-08-04 補):鏈子上原本少了這一節 —— 代盧植去打廣宗、打不下來、坐減死罪一等的那個人整個不見,而他就在盤上,他自己的目標註解裡還寫著「史實上他這一仗打輸了」。補上之後兩件事同時說得通:①董卓的「擁兵自重」有了來由(這一趟是賠本的,回涼州保本才成為選擇);②皇甫嵩的北道之任有了本錢 —— 史實上他是**接了董卓的兵**才北上的(十月代卓擊廣宗斬張梁,十一月下曲陽斬張寶),而體檢實測他五城四萬兵去打黃巾兩座最硬的老巢本來就不夠。選「持重不進」則董卓天命 −12、皇甫嵩兵 ×1.28;選「強攻」則董卓兵 ×0.84、黃巾 ×0.93,交到皇甫嵩手上的軍隊也薄了一層(×1.14)。
- **測試突然變慢/偶發逾時:先數一數殘留的 chrome-headless-shell(2026-08-05)**:那一天 `src/game/state/*.integration.test.ts` 這一族開始零星紅,同一支計時腳本量到 54 次 `endSeason` 要 **8691ms**(健康時 **521ms**,十七倍)。真兇是 **Playwright 漏了 43 個 `chrome-headless-shell` 進程**,其中四個 GPU 進程各吃 200% CPU —— e2e 逾時被 force-kill 之後,worker 的 GPU 子進程活了下來。`pkill -f chrome-headless-shell` 之後 load average 從 41 掉回來,季結算立刻回到 521ms。
  - 排查順序:①數殘留進程並殺掉,重新量;②單獨跑那一檔(過了就是並行抖動);③都不是,才去看剛改的東西。我第一次遇到時跳過 ①②,直接懷疑自己剛改的資料,而那一檔用的劇本跟我改的那張盤毫無關係。
  - 逾時放寬與砍工作量的改動仍然保留 —— 它們讓測試對「機器很忙」有韌性,但**當時寫的診斷不完整**:那不是無來由的抖動,是有具體來源的。
- **一把尺:主目標寫他真正做到的事,次要寫他沒做到的(2026-08-05,190 盤全面套用)**:十一家裡曾有十家主目標 0/12。逐條查下去,原因幾乎都是同一個 —— 目標寫的是那個人物**最有名**的事,而不是他在這張盤的位置上**走得到**的事。
  - 曹操「於197年前取洛陽與許昌」→「至196年仍據許昌、陳留與官渡」:0/12 → 7/12 →(只查許昌陳留時 12/12,補上會真的丟的官渡)→ **6/12**。
  - 孫堅「攻取洛陽」→「攻取襄陽」:他在荊南,洛陽隔著袁術與劉表兩個盟友,而史實上他能到洛陽是因為隨袁術屯魯陽當前鋒 —— 盤上表達不了「借道盟友」。
  - 袁紹「盡取冀州」→「攻取平原」:冀州五城有三座在公孫瓚手裡,全取等於九年內打乾淨北方最厚的一家。
  - 劉焉「盡有益州」→「至197年仍據成都、江州、永安」:十五城裡十一座是走不到的南中。
  - 劉表「盡有荊州」→「至196年仍據襄陽、江陵、江夏」:荊州十六城裡孫堅佔三、袁術佔三。
  - **名場面與難目標一律降為次要** —— 那是熬過主線之後的獎賞,不是入場券。
  - 反過來也要防:**守成型目標若查的全是腹地,就是白送**(曹操只查許昌陳留時 12/12)。要挑他**真的會丟**的那一座。
- **守成目標查幾座城才對(2026-08-07,收窄 21 條)**:我這幾批新寫的守成目標一度列到**四、五座城**,而守成是「到期限那一年這幾座**全部**還在」—— 四座城撐十七年,丟一座就是 0。隋末盤的唐拿著全圖最強的三十四城,主目標仍然 0/3,就是這個原因。統一收窄到**兩到三座**(長安+潼關、鄴+渤海、洛陽+長安…),挑的是他真的會丟的那一兩座。這條與 §7.4 既有的反向提醒是一對:查得太少是白送,查得太多是白扣。
  - ⚠ **這個診斷後來自己推翻了一半。** 我一度想把它寫成 lint(守成的「城數 × 年數」不得超過預算),結果它把戰國七雄盤上**達成 3/3** 的趙(3 座 × 17 年)、韓(2 座 × 22 年)全部標紅 —— 長窗口的守成目標照樣成立。也就是說唐的 0/3 更可能是那張盤的**變異數**(500 回合三輪,終局 2–16 城),不是城數。收窄本身仍然對(挑真正要緊的那兩三座是好寫法),但別把它當成「4 座必失」的規律。lint 沒有加。
- **楚漢線(2026-08-07,8 條)**:最刺眼的一條是**目標指向了盟友的城** —— 鉅鹿之戰楚的主目標寫「取鄴」,而鄴是趙的治所,項羽是**去救它的**。改為 `defeat-force qin`(沉船破釜,持三日糧;在鉅鹿打碎的是秦)。其餘同型:項羽「力拔山兮」(defeat-force 漢,而他輸了)→「西楚霸王」守九郡;龍且「救齊之師」(東救齊而死於濰水囊沙)→「保彭城」;章邯「三秦拒漢」要長安**加陳倉**,而三秦另外兩家數週而降、他自己守了十個月 →「廢丘死守」只留長安;魏豹「魏地復國」要濮陽**加洛陽**,而洛陽從來不在魏地之內。
- **中後期十四盤的第三輪(2026-08-07,16 條)**:同一把尺掃完剩下的。取樣:馬超「奪取長安」(長安從來沒真正在他手上)→「關中十部之盟」;劉璋「驅逐客兵」(`defeat-force` 劉備)→「拒守成都」(鄭度勸他堅壁清野,他說「吾聞拒敵以安民,未聞動民以避敵」);孫權 213/214「索還荊州/湘水劃界」都要**江陵**,而湘水劃界分給吳的是長沙桂陽 → 改對城;曹操「得隴望蜀」要漢中**加成都**,而他說完那句話就回去了 → 只留漢中;孫權 215「攻取合肥」(十萬眾敗於八百人)→「保江東」;劉備 221「還於舊都」→「興復漢室」(守成都/漢中/江州:還於舊都是誓,手上這一塊是實);孟獲「北上成都」→「南中之主」;諸葛恪「拔新城」(圍三月,帶回來的是疫)→「圍新城」守成;鄧艾「功高見疑」(`defeat-force` 鍾會,而被殺的是他)→「偷渡陰平」。
  - ⚠ 這一批被 `scenarioAudit.test.ts` 當場擋下一條:公孫淵改寫成 `survive-until 238`,而那張盤就開始於 238 年 —— **開局即已達成**。改為「襄平不陷」(守到 240)。那條測試值回票價。
- **鼎立與隋唐諸盤的同一把尺(2026-08-07,17 條)**:220「魏之天下統一」與 221「受禪定鼎」都是 `unify-realm`(魏未曾一統)→ 改「守成之君」(洛陽/許昌/長安/鄴);220 劉備「漢室再興」要洛陽長安、而他次年做的是**稱帝** → 改 `declare-emperor`(實測 0/3 → 吳同型目標 3/3);222 吳「火燒連營」寫成 `defeat-force`(吳沒滅蜀)→ 改守江陵/夷陵/猇亭;228 石亭吳「詐降誘敵」要合肥壽春(周魴詐降要的是曹休那十萬人,不是一座城)→ 改「江表無虞」。隋唐五盤十二條同型:唐「太原起兵」要長安**加洛陽** → 改「居關中而制天下」(洛陽是三年後的事)、竇建德「河北夏王」要鄴**加洛陽**(去救洛陽的那一趟要了他的命)→ 改守河北三城、薛舉「西秦入關」(他死在入關之前)→ 改「隴右自立」。
  - ⚠ **兩輪的掃描對這幾張盤不夠**:隋末盤 500 回合三輪,唐的終局城數是 13(2–16)—— 同一份資料跑出 2 城與 16 城兩種結局。看到守成型目標 0/2 時,先確認那不是變異數。
- **`control-province` 當主目標的陷阱(2026-08-07,19 條)**:大州裡有 AI 永遠不會走到的邊城,於是「盡有某州」在盤上等於永遠達不到。實測的四個大坑:**揚州 16 城**(含合肥壽春,孫氏從來沒同時據有)、**荊州 19 城**、**涼州 12 城**(含敦煌酒泉張掖,河西盡頭)、**交州 8 城**(含珠崖日南)、**幽州 11 城**(含樂浪帶方)、**益州**(含南中雲南永昌)。
  - 一律改查該州的**核心幾座**,原「盡有某州」降為次要:孫策「江東六郡」(建業/吳/會稽/豫章)、劉表「荊襄之守」(襄陽/江陵/江夏/長沙/宛城)、士燮「交州王」(交趾/南海/合浦/蒼梧)、劉璋「守此蜀土」(成都/江州/雒城/涪城/綿竹)、馬騰「不入許都」(武威/金城/安定/隴西)、公孫瓚「白馬義從」(薊/北平/漁陽/易縣)。
  - 反過來說,**緊湊的州仍然適合 control-province** —— 冀州 9 城、徐州 5 城、豫州 5 城、兗州 3 城、青州 2 城都沒有出現在死目標裡,那幾條保留不動。
- **`defeat-force` 當主目標的兩個陷阱(2026-08-07)**:`defeat-force` 要對方**在盤上全滅**,而史書上的「翦滅」往往只是誅其身:
  - 249 高平陵盤,司馬與曹爽**互相**以 defeat-force 對方為主目標,兩邊永遠 0 —— 曹爽在盤上握著 29 城,不是史書裡那個三日而族滅的人。司馬改為「據武庫,屯浮橋」(守住那天早上奪來的洛陽/許昌/鄴);曹爽改為桓範真正勸他做的那件事 —— 取許昌。
  - 265 晉的主目標是 `unify-realm`,而他真正做到的是**滅吳**(280),一統是滅吳的結果 —— 兩者對調。吳的「存續至290年」是史書沒給他的十年 → 改為 280(不多不少就是那十五年),290 降為次要。
  - 配套把 280 盤的懸殊拉開(晉兵 ×1.50、吳兵 ×0.60 民忠 −35):吳 28→1 城、晉主目標 0/3→1/3。
- **主目標與開局外交自相矛盾(2026-08-08,33 條)**:`isHostilePermitted` 只在關係為 `neutral` 時放行,而互不侵犯/同盟在盤上是**永久的**。於是一條「取下 X 城」的主目標,只要 X 的開局之主與他有約,這條目標**從第 0 旬就是死的** —— 不必模擬也知道,而模擬要跑一個鐘頭才看得出來。最刺眼的是 189 盤:那張盤的描述自己寫著「董卓抵洛陽,少帝已落其手」,而董卓對漢廷寫的是 `non-aggression`,他**永遠**走不進洛陽,而他的主目標就叫「提兵入洛」。同型的還有官渡兩家(195/197 盤 cao-yuan-shao 是 `allied`)、湘水劃界、白衣渡江、劉備入蜀 —— **全庫最有名的幾手,機制上一手也出不來。**
  - 病根是語意誤用:寫盤的人用 `non-aggression` 表達的是「此刻還沒開打」,而引擎讀成「永不交兵」。要的幾乎總是 `neutral` —— **還沒開打,但可以打。** 已改 24 對關係(每一對都在盤檔裡註明理由),另 9 條改寫目標。
  - 判準寫成腳本 `scripts/objective-diplomacy-audit.ts`,並由 `objectiveDiplomacy.test.ts` **硬性歸零**。
- **五張盤在自己的年代裡一條事件都不會演(2026-08-08 補)**:量「每張盤窗口內有幾條事件」才發現的洞 —— **大澤鄉、濰水、垓下、虎牢、西陵** 全是 0,而垓下之戰沒有四面楚歌、虎牢之戰沒有一戰擒兩王,那正是那兩張盤存在的理由。補了六條鏈共 21 節(大澤鄉 3、濰水 3、垓下 4、虎牢 3、西陵 3、晉滅吳 3,外加兩條楚漢橋接);全庫「本盤年代內零事件」的盤 **5 → 0**。
  - **新增 `Scenario.eventFlags`(盤自己宣告要演哪幾條鏈)**。原因是年份鎖不住外傳線:戰國/楚漢/隋唐三線共用 `startDate.year = 178`,於是大澤鄉那張盤把垓下、濰水整套演一遍,順序還是亂的(十面埋伏演在魚腹丹書之前)。年份也錯不開 —— 垓下那條鏈要在垓下盤上**早早**演、在整線盤上**很晚**演,同一個窗口做不到兩件事。鏈上**每一節**都掛旗標,不只開頭那節(開頭鎖住而後續不鎖,等於留了一條從中間插進來的路)。
  - 「楚漢爭霸」那張整線盤只種 `chain-chuhan-era` + `chain-daze`,由兩條史實橋接(**章邯破陳·項梁起江東**、**鴻溝之約**)接力打開後兩段 —— 實測十二節照史書順序演完。
  - **`eventFlags` 的第二個用途:預先種下「開幕那天它已經發生」的前置(2026-08-09)**。通用的楚漢長鏈是線性的(蕭何月下追韓信 → 暗度陳倉 → 背水一陣 → 彭城 → 紀信誑楚 → 分我一杯羹),每一節都要等前一節先擲中;於是**「彭城之戰」那張盤跑滿十年演不到彭城之戰** —— 它排在鏈的第四節。修法是在盤上把前置直接寫進 `eventFlags`:還定三秦那一天韓信早已拜過大將(`ch-zhuihan`),井陘那一天他已定三秦、已並魏(`ch-zhuihan` + `ch-chencang`)。代價是那幾張盤不再演「蕭何月下追韓信」—— 那本來就是它們開幕之前的事。**判準:這一節在盤面文案裡是「已經發生」還是「即將發生」。**
  - **後三國一段同樣薄(2026-08-08 第二批)**:滅蜀之役只有 4 條、鍾會之亂 3 條、襄平 3 條 —— 蜀漢亡國那一年最該演的三幕(綿竹諸葛瞻父子、譙周之議、輿櫬出降)一幕都沒有。再補 8 節(滅蜀 3、鍾會之亂 3、襄平 2)。
- **假想盤的前提人物(2026-08-08,`Scenario.premiseOfficerIds`,16 張盤)**:十七張假想盤裡有六張,**賣點在開局頭幾旬就被自己的事件表取消掉**。實測 `observeScenario` 三輪全中:關羽守住荊州 —— 關羽**第 1 旬**死於「關羽,麥城死」;若關羽威震華夏 —— 第 1–2 旬死於「白衣渡江」;若周瑜不死 —— 第 1–5 旬死於「周瑜歸天」;若董卓未亡 —— 第 3–6 旬死於「呂布弒董」;若孫策不死 —— 第 3–6 旬死於「孫策死於刺客」;若郭嘉不死 —— 第 9–12 旬死於「郭嘉遺計定遼東」(窗口 207–208,正壓在開局年上)。
  - **兩個各自獨立的殺手,少修一個就白修**:①**事件** —— 這類事件的守衛問的是「此人還活著嗎」(`officer-alive`/`officer-active`),而這種盤的前提**正是他活著**,於是不但不被擋,還必然成立;②**壽命** —— `deathChance` 一旦「當前年 ≥ 史實卒年」就回 `0.3 + 0.15 ×(超出年數)`,而前提人物正是頂著自己的卒年開局:周瑜卒 210 而盤 211 開局 = 每年 45%,陸遜卒 245 而盤 249 開局 = **每年 90%**(實測 3/3 死在第一個冬季)。只修事件那一半,六張盤的人仍然全滅,只是晚幾旬。
  - 修法:盤宣告 `premiseOfficerIds`,則 ①**任何會把他設成 dead 的歷史事件在這張盤上都不演**(整條讓路 —— 攔效果會演出「關羽死於麥城」而人沒死,比不演更荒謬);②開局清掉他的 `deathYear`,改走年齡兜底(60 歲起每年 (age−60)×5%)。這正是「諸葛亮活到八十」那張盤的意思:不是不死,是活到該活的歲數。
  - 修完實測:郭嘉/孫策/周瑜/關羽(荊州)/龐統 **0/3 死**;董卓、關羽(北伐)各 1/3、呂布 2/3,查下去都是**戰場陣亡**(這幾位親自帶兵),不是機制。
  - **第二類矛盾:盤會把自己已經改寫掉的那一仗再演一次(`Scenario.blockedEventIds`,8 張盤)**。這一類**不死人**,所以前提保護擋不住 —— 「曹操贏赤壁」會把整套赤壁再演一遍,而它的開局盤面正是那一仗打完的樣子(曹七十六城、吳三城殘部)。逐盤逐條量下來,每張盤都有一到兩條**真的點得著**:許攸夜獻烏巢(若袁紹勝官渡 / 若孫策不死 —— 贏家的糧草再被燒一次)、決泗沂之水灌下邳(若呂布割據徐州 / 若袁術稱帝成,而前提正是那道水沒淹進來)、王允獻貂蟬(若董卓未亡,而「事泄而敗」是前提)、夷陵連營(關羽守住荊州 —— 關羽還活著,伐吳那個理由並不存在)、草船借箭(曹操贏赤壁)、水淹七軍(若關羽威震華夏 —— 那是開局,不是未來)。其餘同鏈節點一起擋,否則前置一演就從中間接上去。
  - ⚠ **這條的反向測試第一版是綠的,而那是因為它自己寫窄了**(只掃春季、只跑三年)—— 也就是說當時「被擋的事件點不著」那條是**空測試**。改成掃四季 × 八年、逐盤都要求至少一條真的點得著之後才立得住。
  - `whatifPremise.test.ts` 釘住四條(id 有效且活著、卒年已清、殺他的事件點不著、**反向**:拿掉保護那些事件確實點得著)。兩條保護都**逐一反向驗過會紅**。
  - ⚠ **綿竹那一節第一次寫成 `officer-alive: zhuge-zhan`,而諸葛瞻在盤上第 1 旬就戰死**(綿竹之戰正是這張盤的開局戰),於是它永遠等不到他。改問「綿竹還在蜀手裡沒有」(`city-owner-ruler`)—— 那才是這一幕的前提。與長社之火(皇甫嵩第 9 回合陣亡)是同一個坑:**事件的守衛要挑活得過那一刻的東西**。
  - ⚠ 補完之後 `seasonReplay` 立刻紅,揪出一個**既有**的決定性漏洞:`findFiringEvent` 的呼叫端沒有把戰役 rng 傳進去,歷史事件那 60% 的擲骰一直走裸 `Math.random`。它一直沒被抓到,是因為只有**條件全部成立**的事件才走得到那一擲。已修。
- **劇本開局的互不侵犯此前是永久的(2026-08-08 修)**:局中簽的互不侵犯是 **8 季**(`NAP_DURATION_SEASONS`),而劇本 `openingRelations` 寫的那些**一紙定終身** —— 沒有人是故意這樣寫的,`rel()` 單純沒帶 `expiresAt` 那個欄位。後果不是「劇本比較守信」,是**史書上撕得最快的幾紙盟約在盤上撕不掉**。現改為開局的互不侵犯 **20 季(五年)後期滿**(`SCENARIO_NAP_SEASONS`,比局中長是因為它陳述的是一段局勢而非一次交易);同盟 `allied` 不在此列(由聯姻/會盟/背盟各自管),真的不該期滿的可在盤上寫 `permanent: true`。
- **主目標的期限比那個人的命還長(2026-08-08,31 條)**:199 易京盤的袁術主目標是守壽春**到 205 年**,而袁術 199 年嘔血死於江亭;198 下邳盤的公孫瓚要守到 204,而他 199 年自焚於易京樓。這種目標在盤上不是難,是**要求一個死人繼續守城** —— 而掃描只會告訴你「這條 0/3」,看不出來為什麼。一律壓回君主的卒年:那正是「他活著的時候守住了」。張魯佔四條、韓遂三條、公孫瓚三條、馬騰兩條。
  - ⚠ **只查小勢力(開局 ≤8 城)**。大國有真正的繼統 —— 曹操卒於 220 而魏繼續、劉備卒於 223 而蜀繼續。不收這一刀,規則會報出 118 條而其中一百條是無害的。
  - ⚠ 另兩個具名豁免,都是我第一版漏掉、被測試逮到的:君主卒年**早於**開局年(「曹操贏赤壁」由孫翊當家,而史書上他 204 年就死了)、以及**等於**開局年(189 盤的何進在開場第一句就被斬,把期限壓到 189 等於給他一個零長度窗口 —— 那不是修正,是換一種壞法)。判準寫成 `scripts/objective-lifespan-audit.ts`,由 `objectiveLifespan.test.ts` 硬性歸零。
- **一座城都不剩的人,什麼目標都不算達成(2026-08-13)**:這條規則原本**只寫在挑戰系統**(`evaluateChallenge` 第一行:被抹掉即刻判敗),而劇本目標走的是 `evaluateGoal` 那一條路 —— 於是 `survive-until` 只看年份:「存續至 220 年」在勢力早已亡了之後,到了 220 年照樣判**成功**。掃描裡「曹操贏赤壁」的吳殘部 3/3、「若袁紹勝官渡」的曹操殘部 3/3,有一部分就是這麼來的,而那兩條主目標寫的正是「存續」。現把那一層搬進 `evaluateGoal` 本身。
  - 同一個形狀還蓋到另外三型:`defeat-force` / `break-force` / `protect-force` 問的都是**別人**的城數,不看自己還在不在 ——「我救了趙國」不能由一個已經亡了的國家來說。`hold-cities` 本來就會自然判失敗(城已經不是你的),多這一層不改變它。
  - `objectiveLiveness.test.ts` 釘住三型;反向驗過(把那層拿掉,三條全紅)。
- **新目標型別 `protect-force`(2026-08-08)**:竊符救趙、毛遂自薦、春申君北救、吳救壽春 —— 史書上都是同一件事:**出兵不為取地,而為那一家別亡**。系統裡原本只有 `hold-cities`,於是這幾條被寫成「取下你要救的那一家的城」,信陵君的主目標成了「攻取鄴城」,而鄴是趙的、兩家還是同盟。新型別問的是「到那一年他還在不在」(`minCities` 不填視為 1),與守成型同一口徑:期限之前一律進行中,**中途被打殘又收復仍算救成了** —— 救援看的是最後那一刻。
- **一夜的總帳(2026-08-07)**:掃描的**分母變了**,所以要用比例講。起點是 393 條主目標裡 **188 條從沒達成過(48%)**;補完 147 家缺目標的勢力之後,分母是 540 條,而死目標 **105 條(19%)**。乾淨的盤那個數字不能直接比 —— 一張盤上的目標多了,至少有一條 0 的機會自然變高。
  - 剩下的一百來條裡有一批是**設計如此** —— 呂布「白門樓之前」、公孫瓚「易京不焚」、袁術「仲氏不亡」、袁尚「鄴城固守」、英布叛楚、田榮為項羽所破。`objective-sweep.ts` 現在把它們分開列(要進去就得寫理由),頭條數字因此是「未解釋的」那些。**動手改之前先分清楚「達不到」與「本來就該達不到」。**
  - ⚠ **三輪的解析度大約是 ±1 張盤**,別把個位數的差當訊號:某一批前後同段比較死目標 38 → 35 而乾淨盤 13 → 13,兩張轉好、兩張轉壞,轉壞的都是 1~2/3 掉到 0/3 的擲骰。要判斷一個改動是不是真的有效,要嘛提高輪數,要嘛看**同一條目標**跨多次掃描的走勢。

- **外傳三線的目標窗口一律太長(2026-08-07,27 張盤)**:三國線的目標期限是開局後 **3–10 年**,而戰國/楚漢/隋唐三線寫的是 **12–30 年** —— 鉅鹿之戰打了幾個月,目標期限卻在二十年後。長窗口對守成型是致命的:**守一座關隘二十年幾乎必失一次**,而守成要的是「到期限那一年全部還在」。全庫掃描剩下的 45 條守成型死目標,絕大多數集中在這三線。
  - 按各盤**真正打了多久**壓回去:單一戰役的盤壓到 4–8 年(鉅鹿/彭城/井陘/濰水/淺水原/柏壁/虎牢…),跨世代的時代盤保留長窗口(七雄逐鹿 17–27、秦滅六國 10–17、隋末群雄 10–16、安史 6–12)。期限是等比重映的,描述文字裡的年份一併換。
  - 立刻可見:楚漢線八張盤的死目標 21 → 9,乾淨的盤 1 → 3;順帶掃描本身快了一倍(324 旬對 684 旬)。
  - ⚠ 這條與「守成該查幾座城」是同一件事的兩面:**曝險是窗口 × 城數**,而窗口那一邊我先前完全沒看。先前想把城數寫成 lint 被自己推翻,正是因為只看了一半。
- **同一把尺套到戰國諸盤的秦(2026-08-06,9 條)**:掃描顯示**秦在十四張戰國盤上主目標幾乎全 0**,而查下去有兩層:①「外傳三線沒有開局姿態」(見 §7.4,修完秦 40→29 城居七雄之首)②主目標寫的是他**失敗的那幾仗** —— 閼與之戰是秦東出以來唯一的敗仗,邯鄲圍三年不下而還,而這兩張盤的秦主目標就叫「取閼與」「圍邯鄲」。改成他真正做到的:閼與那年「關中之固」(未損分毫)、邯鄲那年「河東不失」(長平所得的上黨沒有丟)、長平「取上黨」(鄴是後話)、七雄「據崤函之固」(unify-realm 降為次要 —— 統一在八十年後,盤上的窗口只有二十八年)。
- **同一把尺套到北伐諸盤(2026-08-06,15 條)**:掃描顯示蜀漢在 228 街亭 / 228 石亭 / 229 三帝 / 231 鹵城 / 234 五丈原 / 238 遼東 / 249 高平陵 / 252 東興 / 253 合肥新城 / 255 淮南二叛 **十張盤主目標全 0**,而十條裡有八條寫的是同一件事:**取長安**。諸葛亮與姜維一次也沒取到長安 —— 那是他們沒做到的事,照這把尺就該是次要。
  - 主目標改寫成他們**真正做到**的:一出祁山所得的「隴右三郡」(天水/上邽/武都/陰平,盤上蜀漢開局就握著)、建興七年陳式取的「武都陰平」、五丈原的「屯田渭濱」、姜維九伐的前提「漢中之險」(漢中+陽平關)。都是守成型,而且挑的是**真的會丟**的邊城,不是成都。
  - 同一批連帶三條:魏「蜀漢殲滅」(直到 263 才做到)→「拒亮於渭南」;曹爽「取漢中」(興勢大敗而還)→「關中不搖」;魏「中原正統/unify-realm」(魏未曾一統)→「中原之固」。
  - 吳「全據長江」要的襄陽合肥兩座終其一世沒拿到 → 改「江表之守」(江陵/武昌/建業);公孫淵「燕王之號」寫成 `declare-emperor`,而他稱的是**王**不是帝 → 改 `survive-until 238`(司馬懿來的那一年)。
- **民忠的全盤下漂,與「被自己人拖垮」的一家(2026-08-05)**:190 盤上曹操五城掉到一城,追六輪十四次失城 —— **無主(民變)七次、黃天教四次、鄰國只有三次**。他不是被打垮的,是外圍城池自己爛掉:白馬民忠 79→50、延津 79→40(110 回合)。
  - 先猜是「無人牧守」(九將全擠在許昌),把人鋪開再量 —— **曲線一模一樣**,白馬仍在第 70 回合跌到 23。
  - ⚠ **接著我下了一個錯的結論**,寫成「民忠是全盤下漂的機制」。2026-08-06 用整盤平均重量了一次:官渡盤 30 回合後,**有官駐守的 21 座平均 +4.57、無官的 76 座平均 +1.57 —— 兩邊都在漲**。掉下去的是鄴(−35)、成都(−17)、金城(−9),全是當時正在挨打的城。也就是說**民忠是打輸的結果,不是打輸的原因**;我先前只盯著曹操(一個正在輸的勢力)的城看,把倖存者偏差當成了機制。
  - 那 曹操為什麼垮?200 盤是打不過鄰居(袁紹二十一城對他十一城),208 盤是**四十八城守不住**(追兇分佈:孫權 ×66、民變 ×57、異族 ×20)。民變是症狀,不是病灶。
  - **所以不修,改承認**:史實上曹操的後方真的反了(194 年張邈陳宮迎呂布,兗州郡縣皆應,只剩鄄城、范、東阿三城)。他的主目標因此從「取洛陽奉天子」改成「至196年仍據許昌與陳留」—— 守住核心才是他當時真正做到的事,奉天子降為次要。
  - 教訓:**AI 自走裡一家崩掉,先查兇手是誰**(逐回合 diff 城池歸屬並記下新主),別預設是鄰國打的。
- **郿與武關原本掛在益州(2026-08-05 修)**:郿縣屬右扶風、去長安三十里(董卓築郿塢之處),武關在弘農郡商縣、是關中通南陽的門戶 —— 兩座都在關中,而 `provinces.ts` 把它們列在益州底下。於是「盡取益州」這條目標**必須從據有長安的那一家手裡打下兩座關中城**。190 盤實測:劉焉終局十七城(全盤最大)而主目標 0/12,差的正是董卓手上的郿與武關。已移入司隸;益州 17 城 → 15 城,全境只剩劉焉與無主。**七張盤有益州目標,全部受影響**。
- **200 官渡的鏈(2026-08-05)**:節也都在,順序也全散 —— 體檢十二輪跑出**許攸夜獻烏巢第 1 回合、官渡之戰第 3、白馬斬顏良第 11**:決定勝負的那把火燒在開戰之前,而斬顏良發生在戰役結束之後。重排為 田豐諫止南征(秋)→ 許攸夜獻烏巢(冬)→ 官渡之戰(春,**須 `wuchao-burned`**),十輪實測 10/10,落在第 1–5 / 9–12 / 18–21 回合。
  - **官渡之戰改成烏巢那把火的結果,不是它的前提**:選「親率輕騎夜襲烏巢」才有大潰;選「疑有詐,按兵不動」則這一節不演 —— 相持下去,糧盡的是自己。那是這張盤唯一一個真正決定勝負的選擇。
  - ⚠ **季節鎖要對照劇本的開局季節看**:烏巢先鎖了「秋」,而**這張盤的開局就是 200 年秋**,於是秋鎖在第 1 回合就成立,十輪仍全落在第 1–2 回合。鎖冬才真的推到相持之後(十月,正是史書給的月份)。184、190 都是春天開局的盤,遇不到這個坑。
  - ⚠ **順序交給季節,不要交給一個可能永遠設不上的旗標**:一度讓烏巢要求 `baima-yanliang`,十輪**整條鏈 0/10** —— 白馬那一節要 `guan-yu-with-cao`,而官渡盤上關羽在劉備麾下(他 200 年二月為曹操斬顏良,而後掛印歸備,那是**這張盤開局之前**的事),旗標在這張盤根本設不起來。與 190 盤「討董鎖春導致整條鏈不演」是同一個教訓。
  - 另修一組同名重複:**白馬斬顏良有兩條**(`evt-baima-yanliang` / `evt-baima-yan-liang`),互斥條件各查各的旗標,誰先誰後全看列表順序。已併為一條,旗標一併設上。
- **190 反董卓聯軍的鏈(2026-08-05)**:節都在,順序與節奏全散 —— 體檢十二輪跑出討董第 2 回合、三英戰呂布第 4、孫堅得玉璽第 6、董卓焚洛陽第 9,兩年多的事十一個旬演完,而且**玉璽比洛陽起火還早三回合撈上來**。重排為:十八路諸侯討董(190,不鎖季節)→ 溫酒斬華雄(夏,須聯軍已成)→ 三英戰呂布(秋,須華雄已斬)→ 董卓焚洛陽(冬,須聯軍已成)→ 孫堅得玉璽(須洛陽已焚)。十輪實測 9/10,節奏落在第 1–4 / 9–10 / 18–20 / 27–32 / 28–33 回合。
  - **開關那一節刻意不鎖季節**:一度也給討董鎖了春(190 正月起兵),十輪有一輪整條鏈沒演 —— 春天只有九個旬,而每旬只放行一個事件,開關一旦沒開,底下四節全靜默(都要 `coalition-formed`)。節奏由**下游**的季節鎖扛,開關給它一整年。
  - 同批抓出三個真壞掉的:①「三英戰呂布」在事件表裡有**兩條**,名字一字不差,互斥只做了一半(前者要呂布 active、後者只要 alive —— 呂布負傷時後者先演、旗標沒設,傷癒後前者再演一次),已併為一條;②「董卓焚洛陽」的效果寫 `cityId: 'city-luoyang'`,而城 id 就叫 `luoyang`,查不到城靜默跳過 —— **這條效果從沒生效過**,全庫僅此一處;③焚洛陽原本無條件觸發,關東還沒起兵國都就先燒了。
  - ⚠ **把名場面綁在「某人據有某城」之前,先量**:玉璽一度綁「孫堅據有洛陽」(照 184 宛城之圍的做法),六輪 0/6 —— 他的地盤在荊南,自走十二輪一次也沒打到洛陽。而且史實上他也不曾據有洛陽(董卓西遷後他入的是空城,掃除宗廟而後引軍還)。只留「城已焚」那一道,順序就對了。
- **`force-cities-revolt-ruler`(城池反正,2026-08)**:按君主解析一個勢力,令其**守備最薄的 fraction 比例城池**改旗易幟 —— 歸給鄰城的主人(鄰城若都是自家的則變無主),城中兵留四成、民忠 −15。永遠留一座城給該勢力(崩解 ≠ 當場除名)。
  - 為什麼要有它:一場**運動**的崩解和一支軍隊的戰敗不一樣。張角一死,黃巾不是被逐城攻下的,是成批降的 ——「黃巾遂降」四個字底下是幾十座城一季之內換旗。`force-troops-multiplier` 只砍兵不動城,所以教主死後黃巾照樣滾雪球。現用於張角病死(0.30)與宛城之圍(強攻 0.45 / 納降 0.55)。
- **名場面補遺**(2026-07,歷史事件 113→123):補入侦查發現缺失的著名場面,各挂天命/兵力/忠誠後果 —— **上方谷天火**(誘司馬懿父子入谷縱火,忽天降大雨救之:魏軍×0.9、蜀天命+5)、**當陽橋斷喝**(張飛據橋喝退曹軍×0.95)、**八陣圖困陸遜**(魚腹浦亂石困吳追兵×0.93)、**磐河趙雲初陣**(常山子龍登場,忠+10)、**生子當如孫仲謀**(濡須對峙,吳天命+7)、**陳倉郝昭拒諸葛**(千餘守堅城,蜀北伐×0.95、郝昭忠+12)、**曹操屠徐州・陶謙三讓**(彭城民忠−15、曹天命−6)。另補**三分建號三式**:曹丕受禪代漢(魏天命+12)、劉備漢中王封五虎(蜀天命+8、五虎各忠+5)、孫權稱帝建吳(吳天命+10)。
- **觀賞事件補機制**(2026-07):原本純 flag、零後果的四則高知名度場面補上機制 —— 出師表(蜀天命+8、傾國北伐×1.06)、借東風(曹軍水寨×0.88、吳天命+6)、煮酒論英雄(劉備天命+5)、火燒博望坡(曹軍×0.92、孔明威望+10)。名場面從「過場文本」變「真有影響」。
- **後三國補遺**(2026-07,事件 123→129):侦查發現 234–280 這 46 年僅 ~15 事件,姜維九伐壓成 2 條、淮南三叛僅 1 條概括。補 6 則填空檔(effects 以**穩定命中的蜀方** `liu-shan` 天命 + 具體武將忠誠為主,因魏方 ruler 後期頻繁更替 曹芳/曹髦/曹奂/司馬氏,削魏 ruler-effect 易 no-op):**興勢之戰**(244 王平拒曹爽,蜀天命+4)、**費禕遇刺**(253 郭循席間行刺,蜀失柱石 −5)、**洮西大捷**(255 姜維破王經,天命+5、姜維忠+8)、**段谷之敗**(256 鄧艾破姜維,天命−4、姜維自貶 −6)、**諸葛誕壽春之叛**(257 淮南三叛之三,壽春民忠−12)、**沓中屯田避禍**(262 姜維避黃皓,君臣離心 −3)。後三國不再是空白期。
- **武將個人時刻**(2026-07,事件 131→134):給尚無專屬事件的名士忠勇補個人名場面 —— **禰衡擊鼓罵曹**(裸身擊鼓反辱曹操,操怒而不殺,曹天命−3)、**周泰九死護主**(濡須殺入重圍救孫權,身被數十創,孫權脫衣示創、一創一觴、賜青羅傘蓋,忠+12)、**孔融滿門受戮**(孔門之後力主尊漢,曹操以不孝謗訕之罪滅其族,士林寒心、曹天命−4)。
- **經典橋段補遺·二**(2026-07,事件 134→141):再補七則演義名場面 —— **馬躍檀溪**(的盧一躍三丈脫蔡瑁之追,劉備天命+3)、**張松獻西川地圖**(曹操慢士、劉備郊迎,蜀天命+5/曹−3,取蜀之基)、**周瑜歸天**(既生瑜何生亮,箭瘡迸裂卒於巴丘,吳天命−3)、**割髮代首**(制法自犯、割髮示眾,曹軍紀肅然 ×1.03)、**橫槊賦詩**(赤壁宴上刺殺劉馥,驕兵敗亡之兆)、**木門道射張郃**(諸葛設伏、萬弩射殺張郃,曹軍×0.96)、**隴上裝神割麥**(扮六丁六甲之神退魏兵、盡割隴麥充糧)。周瑜/張郃於**史實卒年** `officer-status dead`,與孫策遇刺、五丈原星墜等既有死亡事件同風格。
- **經典橋段補遺·三**(2026-07,事件 141→147):開局與奸雄六則 —— **孟德獻刀**(七星寶刀刺董卓,事敗矯詔起兵)、**陳宮捉放曹**(寧教我負天下人,夜半棄曹)、**戰長沙義釋黃忠**(黃忠魏延歸蜀,蜀天命+4、黃忠忠+8)、**濮陽之戰**(呂布火攻、典韋救主,曹軍×0.9)、**吉平下毒**(衣帶詔太醫弒曹事敗、觸階斷指而死)、**曹操夢中殺人**(奸雄多疑,楊修「君乃在夢中耳」)。
- **武將個人時刻·二**(2026-07,事件 147→151):**太史慈北海解圍**(報孔融養母之恩、單騎突圍求救於劉備,忠+8)、**于吉顯聖**(孫策斬道人、鏡中顯形驚懼迸創而亡)、**管輅神卜**(卜東吳西蜀奇中、預言鄧颺何晏死期)、**王粲過目不忘**(背碑成誦、復盤不誤,建安七子之冠)。
- **經典橋段補遺·四**(2026-07,事件 151→155):討董與河北四則 —— **十八路諸侯討董**(酸棗會盟推袁紹為盟主,董天命−4)、**三英戰呂布**(虎牢關桃園三英環戰溫侯)、**界橋之戰**(麴義八百強弩破白馬義從,公孫瓚×0.9,趙雲初隨公孫)、**孫堅跨江隕峴山**(黃祖竹林伏箭,孫策扶柩誓報父仇)。
...
- **時代門控(2026-07-26)**:教派原本只看 `date.year` 挑旗號(`year < 195` → 黃天教),而**戰國/楚漢/隋唐劇本共用三國地圖與紀年、開局一律 178 年** —— 於是那些盤上每一場民變都掛黃天教旗;實測楚漢爭霸跑 200 旬,`cult-huangtian-*` 一度佔到 10 座城,成為第五大勢力。現加時代中性的 **`folk`(流民軍,首領「流民渠帥」)**:`isLaterHanBoard(scenarioId)`(以 `scn-ws|ch|st-` 前綴判斷,不看年份)為 false 時一律用 folk。**文案同步分流** —— 教派聚「信眾」、流民軍聚饑民,起義文案作「流民作亂 — 某城為饑民所據」、蔓延作「饑荒蔓延」,只改 id 不改文案的話楚漢盤上仍會讀到「信眾雲集」。
...
### 8.4b 異族也要分時代(tribes.foundedYear,2026-07-27)

用全 AI 觀察器(`scripts/ai-watch.ts`)跑沒驗過的盤,戰國七雄 200 旬結束時場上有 `tribe-state-linyi` 佔兩座城 —— **林邑是 192 年才立的國**(這句話就寫在它自己的說明文字裡),比那個盤的故事晚約四百年。與「黃巾在楚漢盤起義」同一形狀:**所有劇本都開在遊戲紀年 178**,凡是拿 `date.year` 判時代的內容一律誤觸發。

`Tribe.foundedYear` 以**盤面紀年**(`boardHistoricalYear`:戰國 −300 / 楚漢 −205 / 隋唐 617 / 漢末 200)過濾:烏桓・鮮卑 −206、扶餘 −100、高句麗 −37、林邑 192;**匈奴/羌/氐/南蠻/山越不填** —— 它們比每一個盤都古老。

**缺席用「沒有 aggression 條目」表示,而不是把值設成 0**:四個消費點(擾邊、徵發義從、以夷制夷煽動、AI 征討)原本都寫著 `?? tribe.baseAggression`,設 0 會被 fallback 救回來。現在一律 `== null` 就 continue,`createInitialTribeState(scenarioId)` 成為唯一那個開關。
...
- **制度功業(2026-07-21,§1.16–§1.19/§4.8–§4.12/§5.16–§5.17)**:八項新勳功 —— **甲堅兵利**(某城軍器達 55)· **傷卒歸伍**(一季歸伍 ≥300 人)· **通商惠工**(商旅跨你的國境成交)· **器械如林**(圍城 park 達 9 具)· **百騎劫營**(夜襲得手)· **政令通達**(十城以上而無一斷驛,金)· **賞不逾時**(全境軍功帳一次結清,金)· **錢法不改**(大錢之議選「錢法不可輕改」,金,走 event-choice 旗標)。
...
- **制度警訊與軍師二患(2026-07-21)**:① **武庫將空**(軍器跌破 12 且駐軍 ≥2000 的那一季)· **傷卒滿營**(傷兵越過駐軍 8%)—— 皆只在**跨越門檻那一季**報一次;**斷驛是常態而非事件**,故改為**每年春季**一條全境提醒(通達 N/M 城 + 點名最嚴重三城)。② 軍師建議新增二患:**督造軍器**(只在**庫中有鐵 ≥300** 的城才提 —— 沒鐵時該指令必敗,而全圖開局軍器皆 0,不設閘會在第一回合對著玩家喊一件他做不到的事)與**傷卒待療**(無對應指令,直接說明宜屯藥材/建醫館/留智將坐鎮)。
...
- **氛圍與旁白(flavor 層)**:戰場語音(voiceLines.ts —— 出陣/斬將/危殆台詞;2026-07 由 20→39 人:先補三國陸遜/姜維/龐德/張郃/鄧艾/魏延/夏侯淵/徐晃,再補**歷代名將**項羽(力拔山兮氣蓋世)/李存孝/霍去病(匈奴未滅何以家為)/白起/韓信(背水列陣)/岳飛(還我河山)/常遇春/薛仁貴(三箭定天山)/蘭陵王/尉遲恭/李廣(飛將軍)—— 跨界名冊的頂級猛將原有決鬥台詞卻無戰場語音)、季度隨機對話事件(dialogues.ts / dialogueRoll,多為趣味、少數有實際效果;每季 8% 機率 roll 一則,followup 分支則於次季必觸發)——2026-07 起除泛化路人趣聞外,新增**麾下名將情境進言**一路:對話用 `speakerOfficerId` 掛真實武將立繪、以 `conditions.requiresOfficerInService`(**該將須正效力於你**,非僅「天下尚存」)為門檻,抉擇多牽動該將忠誠,故是「你的班底親自對你說話」而非路人貼紙(關羽請戰/張飛請為先鋒/趙雲諫還田/周瑜論勢/司馬懿劝韜晦/賈詡冷謀[含 followup 坐收漁利]/郭嘉病中十勝十敗/呂布驕橫索賞/黃忠老當益壯/姜維繼志北伐/陸遜少年持重/許褚宿衛/張遼請陷陣/典韋守夜);另有**重臣薦賢**三則(徐庶走馬薦諸葛/荀彧薦郭嘉/魯肅薦鳳雛)真正落實 `recruit` 效果——薦主在你麾下時觸發,所薦之才應召入仕(歸你勢力、駐都城、忠誠保底 60;若已歸你或身亡則 no-op)。又新增**讖緯・天象・童謠**一路(隨漢季天人感應底色):災異(熒惑守心/彗孛太微/白虹貫日/日食,可修德禳祭 gold)、祥瑞(五星聚東井/黃龍見/麟鳳至/甘露降,徵天命 set-flag)、謠讖(千里草童謠 189–193 隱「董卓」/帝非帝王非王上北邙 188–191/代漢者當塗高 200+/山石剖文之圖讖),抉擇分修德・自壯・斥妄三態,多以 set-flag 記其嚮應。再一路**異族風俗**(接 tribes.ts 諸族,各具其俗與馭法):南蠻(犀甲象兵,攻心/互市/威服;七擒心服則孟獲率眾內附 recruit)、羌(叛服無常尚血讎,罷貪吏羈縻/徙種/征討)、烏桓(突騎名天下,收其銳為用/白狼山掩襲)、鮮卑(控弦十萬三部,離間/和市/築塞)、山越(阻險出為兵,討定補精卒/羈縻)、氐(半耕牧為屏藩,撫用/徙實關中)、南匈奴(分五部立帥而監之/和親/贖歸沒胡漢女)、東夷高句麗扶餘(納貢通好/陳兵備詐)、五溪蠻沙摩柯(慕義來投 recruit)。再一路**檄文・表文・書信**(漢季文章即政治,「呈文待決」):討賊檄(傳檄/削峻辭/秘不發,minYear 190)、求賢令(唯才是舉 vs 才德並重,200)、出師表(允其專征/乞緩圖,226)、勸進表(三辭三讓/受之/斥非時,210)、自明本志令(頒令釋疑/不辯,205)、誡子家書(淡泊明志)、降表(納降示寬/削權)、勤王血詔(移檄倡義/藏詔待時)、遺表(厚恤旌節/從其薄葬)、抗疏切諫(納諫罷役/優容不從/怒黜)。派系事件(factionEvents.ts)、AI 朝廷請願旁白(aiWishesFlavor.ts —— 每季 0~2 條,且晉升/賞賜會真的調動軍階與忠誠)。
- **地方風物志(cityLore.ts,城市面板「總覽」頁)**:歷史名城各附一則風物志(山川形勝 + 掌故),讀之如翻《方輿》—— 洛陽銅駝荊棘、成都天府沃野、合肥逍遙津張遼威震、下邳白門樓飛將終殞、天水姜維故里街亭、交趾士燮綏南、新城司馬懿八日千二百里……**2026-07-25 補滿全圖 128/128**,分三類寫:①**名場面之地**(官渡/赤壁/長阪/街亭/麥城/猇亭/樊城/博望/白馬/延津/虎牢/潼關/函谷/劍閣/陰平/白水關/葭萌/雒城/綿竹/涪城/公安/濡須/巴丘/西陵)寫那件事本身與它為何發生在這塊地上;②**州郡重鎮**寫地理形勢與此地出過的人家(丹陽出精兵、常山出趙雲、琅琊出諸葛與王氏、譙縣是曹夏侯同鄉);③**江東嶺南與北疆西域**(臨海浮海求夷洲、合浦還珠、日南與大秦來使、朱崖罷郡、樂浪漆器銘「蜀郡西工造」、帶方與卑彌呼、張掖焉支山歌、犍為井鹽養半個蜀漢)。純展示 flavor,**不入 City 型別/存檔**;僅該城有 lore 時渲染一方引文卡,`cityLore(id)` 查無即不顯。`cityLore.test.ts` 守住:key 必須是真城 id(打錯只會靜默不顯示)、全圖不留空、中英皆備。
...
- **存檔**:IndexedDB 主存(idbStorage,localStorage 降級)+ 五槽(縮略圖)+ 存檔互傳(導出/導入 JSON)。
  導出走**系統分享面板優先**(`navigator.share({files})`),桌面瀏覽器退回 `<a download>`——
  因為 iOS WKWebView 會靜默忽略 `download` 屬性,打包成 App 後那顆按鈕原本是完全沒反應的,
  而那恰恰是最需要備份的平台。取消分享視為玩家主動放棄,不再補彈一次下載。
...
**2026-07-21 制度批的對稱**:本夜新增的每一項制度都**從設計上就是雙向的**,不是事後補的 ——

| 制度 | AI 的答案 |
|---|---|
| **糴政**(§1.16) | 商賈/縱橫/均衡之主行**通糴**、暴虐/守成行**閉糴**、其餘平糴 |
| **錢法**(§1.17) | 府庫見底時暴虐/征伐鑄**大錢**、其餘退**穀帛為市**;商賈之主永不離五銖 |
| **兵制**(§4.8) | 富而好戰(每城均金 ≥2200)**募兵**、窮而守成(<700)**世兵**、其餘更卒 |
| **流民之政**(§8.6) | 仁厚/縱橫**招撫**、暴虐/守成**閉關**、其餘安置 |
| **督造軍器**(§1.18) | 駐軍 ≥2500 且庫中鐵 ≥400 且軍器 <45 的邊城自己開工 |
| **賞不逾時**(§4.10) | 每季挑欠得最多的一位,首都出得起就付掉 —— 不必模擬感恩,照付帳單即可 |
| **驛傳/傷兵/軍器/疫疾/攻城器械/夜襲/整編** | 皆在 resolution 的全城/全軍迴圈裡,**天然對所有勢力生效** |
| **一騎定和**(§6.13) | **新增**:兩個交惡的 AI 國(關係 ≤−15、提議方城數不及對方九成)每季 6% 遣使以champion 定和,勝負只決定賠款與顏面,**兩國照樣締 NAP**;結果入季報 |

**對稱審計測試** `aiSymmetry.test.ts`:逐一斷言每項制度對**所有君主性格**(含未知性格)都解析得出合法答案、效果非 NaN,且**不會塌成單一選擇**(否則天下諸國會長得一模一樣)。這是防止「玩家白賺優勢」再度發生的鎖。
...
### 10.y 君主本性譜:讓曹操像曹操(rulerProfiles.ts,2026-07)

過去 86 個劇本的 `scenarios.ts` 從未在 `Force` 上寫過 `personality/statecraft/recruitmentStance`,於是 **~297 個 AI 勢力開局一律回落到預設 `opportunist` / 雜糅 / 並用** —— 曹操、袁紹、劉備的 AI 打法一模一樣,§7.8 門閥與 §7.9 學派兩套機制都在空轉。

`rulerProfiles.ts` 的 `RULER_PROFILES` 表以 **君主 id** 為鍵(這三軸是人物本性,跨劇本穩定),一張約 100 條的表**同時點亮全部 297 個勢力實例**:
- **性格**:曹操 攻擊、袁紹 慎重(多謀少決)、董卓/孫皓 暴虐、劉備/劉璋 守勢、孫權 機會、袁術/馬騰 擴張、孔融/劉表 學者、陶謙 守備……
- **治國理念**:曹魏 法家、蜀漢 儒、孫吳 儒、張魯 道(五斗米)、張角 道(太平)、諸武人之主 兵家。
- **門第政策**:曹操 唯才是舉、曹丕/袁紹 重門第(九品/四世三公)、孫吳 並用。
- 外傳戰國/楚漢/隋唐君主(秦孝公 法家唯才、劉邦 黃老善將將、項羽 兵家世家、燕昭王 招賢……)一併賦性。

回填點在 `gameState.ts` 物化 `scenario.forces` 處(profile 先展開、Force 顯式值後展開 → **劇本若已顯式設定則永遠勝出**),是純補數據、零機制新增。`imperialRank`/國號/年號**刻意不放進本譜**(隨劇本與時代變動,仍由劇本資料與天子/建國系統掌管)。2026-08 起 `imperialRank` 已逐盤寫進**劇本的勢力表**(見 §7.4 開局爵位),本譜仍不碰它。

### 名器登記是模組級鏡像,會跨 setState 殘留(2026-08-07)

`REFINE_REGISTRY` 那一族(精煉/突破/鑲嵌/典故/覺醒/進化/磨損/詞條)是 store 的**鏡像**,只在「精煉動作、開新局、讀檔」時由 `set*Registry` 推過去 —— 讓 combat / duel / 傷害預估這些純函數不必把整張表穿進每一層簽名。

代價是:**任何繞過那三個入口的 setState,鏡像就停在上一次的值。**

抓到它的過程值得記:改 AI 守勢讓它更積極之後,「同種子同世界」的重播測試從**第 1 旬**就分歧,分歧點在一場野戰的 `blendedStat`(134.1 對 135),而最終 officers 兩次一致。排除過的:①**不是新引入的隨機** —— patch 掉 `Math.random` 計數,endSeason 期間裸呼叫 **0 次**;② 不是跨 replay 殘留的 state 鍵(快照全帶到了);③ 把 AI 行為改回去、只留接線,分歧立刻消失。也就是說變的不是裝備 id,是它的 **live 效果** —— 第一次重播裡 AI 鍛出來的精煉留在模組變數裡,第二次重播從頭就用著上一輪的裝備數值。
...
修法一行:夾到 `cityStatCap(city)`。修後同一張盤的洛陽在 60 旬時是 70,不再一路爬。

⚠ **而修完之後才發現我自己也踩了同一個坑的鏡像:** `cityStatCap` 是**按人口**分級的
(邑 60 / 鎮 80 / 城 100 / 都 130 / 京 160),而**關本來就人少而城高** ——
劍閣民二萬五而城防 95、函谷關民一萬二而城防 88。全圖有十三座開局就超過自己
等級上限的城,**全部是關**(外加一座郿,那是董卓的塢)。

於是 `Math.min(cap, current + gain)` 對一座 95 防的邑會算出 60 —— **一鎮守就把
自家的劍閣拆矮了**。`commands.ts` 的大築城原本就有這個問題(`Math.min(cap - defense, …)`
在超額時取到負數),而**它同一行的預覽字串有 `Math.max(0, …)` —— 介面顯示 +0,
實際扣分**。兩處都改成只往上長、不往下砍;前提本身由 `passDefense.test.ts` 釘住。

⚠ **這條修正讓掃描的數字變差,而那是對的。** 修前 105 條死目標、修後 116 條,
增加的十一條**全是守成型**,而且散在十幾張不相干的盤上(戰國的韓、齊、燕,
楚漢的魏,三國的劉表、士燮、公孫瓚…)。原因很直接:城防不再無限長,
守方就真的守不住了。

也就是說 —— **先前的守成通過率有一部分是那個 bug 撐起來的**。等級上限
(京 160 / 都 130 / 城 100)本來就是設計說了算的數字,城池比以前容易易主
才是設計要的行為,先前的穩定是假的。所以不因為數字變差而回退,
也不急著補償;要補償得先想清楚是「上限太低」還是「AI 攻擊門檻太鬆」。

## 第十一章 核心流程・勝敗・培訓・其他模式

### 10.z 引擎算了就要說:軍師兩條新告警 + 君主性格上畫面(2026-07-27)

三處「引擎在讀、玩家看不到」的 AI 相關資訊:

- **謀反前兆**(`ambition.brewingRebels`)—— 它的註釋自己就寫著「rough risk for **display**」,卻沒有任何 display。既有的「忠誠告警」只看忠誠 <40,而謀反擲骰真正讀的是一整組門檻(忠誠、性格或積怨、手上有城、國有二城以上)。軍師現在點名那個**已經站在懸崖邊**的人,優先度高於單純的忠誠提醒。
- **賞罰之柄**(`militaryLaw.ledgerHealth`)—— 逐人的行賞行罰早在武將面板,缺的是**總帳**:欠賞每人每季扣 1~2 忠誠,二十個人沒結清就是整國慢性失血,而玩家只會一個一個地看到症狀。現在直接報「幾人欠賞、合計多少、每季共損多少」。
- **君主性格**(`Force.personality`)—— 引擎 **34 處**在讀(攻擊門檻 ×0.65~1.35、議和意願 ×0.3~1.4),畫面 **0 處**。外交一覽每列加鷹/鴿標記與 tooltip 數字,「誰會來打我」從此可讀。標籤表放 `rulerPersonality.ts` 與倍率同檔,免得日後改倍率忘了改說明。

### 10.z-2 引擎算了就要說(第二批,2026-07-27)

六處引擎一直在讀、畫面卻不說的資訊。其中三處是**決策代價**,以前只有在後果發生時才知道:

- **封爵的野心代價**(`Peerage.ambitionPressure`)—— 大國之封每季推高受封者野心,一路餵給謀反系統,而封爵面板清一色只顯示食邑與忠誠加成。爵位列現在多一欄「野心 +N/季」(警示色),無此代價者顯示「無患」;`loyaltyOnGrant`(受封當下的一次性忠誠)補進忠誠欄 tooltip。
- **官職互斥**(`CivicTitle.excludes`)—— 任命丞相會自動罷免太尉/司徒/大鴻臚,以前只能靠「那個名字怎麼不見了」發現。官職卡加一行條款列出會被罷免的職位,並補上 `loyaltyOnAppoint`。
- **背質子之盟會死人**(`MarriageAlliance.hostage`)—— `marriageAlliance.ts` 明寫:若為質子之盟,背盟直接**處死你自己那位聯姻武將**(蓋上卒年)。而「背盟」按鈕的 tooltip 只說「好感俱崩」。現在依實際盟約狀態改文案,質子之盟顯示「背盟 ⚠」並點名那位會死的武將;非質子之盟則說明忠誠 −25(這個也一直沒寫)。

另外三處是**資訊不對稱**:

- **技能的戰場數值**(`SkillCombatEffects` 七個乘數)—— 武將卡印技能的風味描述與熟練徽記,卻從不說這個技能值多少。新增 `skillCombatEffectLines()`,**從資料生成而非手寫映射**(`traitMechanicalEffects` 是手寫的,只能那樣做),所以日後調數值或加技能都不用動 UI。熟練度折算與 `combat.ts` 同源:平添值 ×m、乘數 `1+(v−1)×m` —— Lv.3 的「我軍損失 ×0.9」加深到 ×0.87 而非翻過 1。**這是照著 combat.ts:424 對過才寫的**,並有契約測試釘住(避免重蹈訪賢勝算「手抄一份預覽然後漂走」的覆轍)。
- **遠使部族的不對稱**—— 非部族目的地會列出確切金額、異域兵、奇珍名稱,部族卻只有一句「或招其酋」。`foreignRealm` 明明帶著酋長 id、也真的用 30–65% 去擲。現在寫出是誰、武統多少、以及他是否已有所屬。
- **游歷所得改成結構化生成**(`haulSummary()`)—— 歸來報告只印 `haul.note`,一行由各 errand 模式各自手寫的自由文字,於是哪個模式忘了提某項收穫,玩家就永遠不知道。而漏掉的三項全是永久改變:`statGain` 直接改能力值、`recruitOfficerId` 招來一整個武將、`homeLoyaltyDelta` 動了本城民心。改成從欄位生成,`note` 保留給風味。

戰場與外交另補三處:**戰場印記**(`terrainScars`/`fieldworksBurned` —— 火計燒林、斷橋、焚壘,斷橋尤其改變可通行路線)、**援軍時程**(`battle.reinforcements` 帶著完整的「第幾回合、從哪一邊、多少兵」,一個字都沒顯示過;敵援也一併預告,反正生成當下就看得見)、**討伐會盟**(`warCoalitions` 整個系統零 UI —— 玩家可以是四家聯軍的討伐對象、或自己拉起的盟主,而在任何畫面上都讀不到)。

> **方法論警告(這批最重要的產出)**:我用「型別欄位在 UI 被讀幾次」掃出 60 個候選,逐項查證後**約六成是誤報**。原因有二:①UI 常透過 systems 的計算函式讀而不碰欄位名(`legionBannerBonus()` / `martialXiuwei()` / `armProficiency()`);②UI 的本地 state 用不同命名(`additionalIds` vs 型別的 `additionalOfficerIds`)。確認為「其實早就做了」的有:軍團旗士氣、隨軍副將、武藝/文辯修為、坐騎羈絆、兵器熟練、家門、戰場繳獲、戰鬥目標、義結/親族(合擊時戰鬥日誌會寫)。**下次用這招掃描,一律以「查概念」(中文標籤、相關函式名)複驗,不要只 grep 欄位名。**
...
> **旬結算內的新增(2026-07-21)**:圍城營每旬推進時另有 —— **攻城器械營造/焚燬(§5.16)** → **夜襲劫營(§5.17,守方分支②)** → **軍中疫疾(§5.15,頓兵之日 +1,拔營歸零)**。

2. **季度結算**(每季,即每 9 旬一次,`seasonBoundary`)。實際鏈條頗長,分兩段:

   **甲・resolveSeason 內(resolution.ts)**,依序:
   - 經濟 tick(金/糧/糧耗/逃兵/人口 + 領土格收入)
   - **驛傳網絡重算(§1.19)**(城迴圈之前:自各國首都以腳程 3 跳、驛站換馬 BFS → 每城「政令所及」四檔,乘進本季貪腐/民心/隱戶)
   - **軍器 tick(§1.18)**(郡國鐵官+存鐵 → 甲胄;耗損隨駐軍)+ **傷兵療傷(§4.11)**(歸伍/廢疾還籍/死於創,耗藥材)+ **兵制(§4.8)**(軍餉自城庫發、欠餉逃亡、耗糧與練度漂移)
   - 領土補給壓力(衛星格陷敵 → 城受損)
   - 附庸進貢 + 軍俸發放(府庫不足 → 忠誠 −2)
   ...
   - **流民安置(§8.6)**(依糴政式的三檔政策納入/驅逐;招撫之城當季併入疫病高風險名單)
   ...
   - **常平倉糴糶(§1.16)** → **米市商旅轉輸**(價差 ≥1.22 開路、買方限府庫六成、商稅入首都)→ **糴政/錢法之商業漂移(§1.16/§1.17)**
   - **賞不逾時(§4.10)**:功高不賞則怨(全勢力)+ **AI 每季結一筆軍功帳**
   - 外交 tick + 合縱聯盟壓力 + **諸國一騎定和(§6.13 對稱)**
...
| **敗亡**(5 變體) | 失去所有城池。**依君主當時境遇分五種輓歌**(同屬 `defeat`,只換敘事):**社稷為墟**(君主身死亂軍)/**階下之囚**(君主被擒 imprisoned)/**奉璽出降**(君主尚存且 263 年後,樂不思蜀式)/**流亡天涯**(君主尚存而年代尚早,劉備數失城郭式)/**敗亡**(無主可考的通用兜底) |

> 每旬結算後檢查;達成即彈結局。多結局 = 不必非得吞下天下才算贏。敗亡也不再只有一句話 —— 輸的方式,對稱六種勝利,各有其文。

- **戰役落幕(`scenarioVerdicts.ts`,2026-08)**:上表那五段敗亡輓歌是**全域**的(只看君主是死是囚、年份過沒過 263)。分盤分家的落幕文本另放一張表,`checkEndings` 取 `scenarioId + playerForceId`:
  - **敗亡變體** —— 有寫就整段蓋掉通用輓歌(標題也換)。黃巾之亂五家各有其敗法:漢室「四百年,盡於此」/ 黃巾「蒼天未死,黃天不立」/ 皇甫嵩「功高而身危」/ 朱儁「扶漢者,漢先仆」/ 董卓「檻車徵詣廷尉」。
  - **史官論曰** —— 不蓋任何東西,附在正文之後,勝敗各一段(體例仿《後漢書》論曰)。
  - **共享敗亡層(`SHARED_DEFEATS`,2026-08-09)**:每家逐盤寫的話是 540 段,而清點下來缺口高度集中在**周邊勢力** —— 馬騰缺 15 張盤、士燮 16、劉璋 14、劉表 13、張魯 12、鮮卑 12、南蠻 10。而周邊勢力的敗亡在哪一張盤上都是同一件事(士燮亡於交州自治的終結,不管那年是 204 還是 220),逐盤重寫十四遍不會更準,只會更淡。所以加一層按 **(路線, 勢力)** 索引的共享表:`ws:`戰國 / `ch:`楚漢 / `st:`隋唐 / `sg:`三國(含假想盤)。**盤上寫過的專屬 `defeat` 永遠優先**,共享層只補沒寫過的;**論曰不共享**(那一段是對「這一張盤的這一年」下判斷的,搬過去就不成立)。29 段共享文本把覆蓋率從 241/540 推到 469/540、全覆蓋的盤從 18 推到 54/86。
  - ⚠ **分路線是必要條件,不是裝飾**:`qi` / `zhao` / `wei` / `chu` 這四個 forceId **戰國與楚漢兩條線都在用**,而戰國的齊(田氏之齊)跟楚漢的齊(田榮田橫)不是同一個國家。只按 forceId 共享會把「田橫五百人蹈海」貼到齊王建身上。同理 `ch:chu` 寫的是項羽的西楚,而大澤鄉那張盤的 `chu` 是**項梁** —— 敗法完全不同,那一張要逐盤寫,不吃共享層。
  - **哪些能共享、哪些必須逐盤**:判準是「這一家是不是只活在同一個十年裡」。袁紹/袁術/孔融/陶謙/呂布/馬超/公孫康 **能**共享(他們在哪張盤上敗亡都是同一件事);曹/孫/劉備/司馬**不能** —— 尤其 `sun` 這個 forceId,189 是孫堅、197–199 是孫策、204–214 是孫權、赤壁後是吳,**同一個 id 四個人**,共享會張冠李戴。
  - **2026-08-09 補滿:540/540、86 張盤全覆蓋。** 逐盤 + 共享兩層合計。`scenarioVerdicts.test.ts` 有一條測試釘住這個數字 —— **加新盤或給既有盤加一家而忘了寫敗亡,它會紅**;紅了就選一條路補:逐盤寫,或(周邊勢力)加進共享層。配角家的體例是 `defeat` + 敗方論曰兩段,勝方論曰留給那張盤真正在講的那幾家。
  - 沒寫過的盤照舊走通用結局 —— 這條退路仍在,但**現在用不到了**:69 張歷史盤已全部寫完,**17 張假想盤也在 2026-08-07 補齊**,`content-audit.ts` 四項覆蓋率(歷史/假想 × 開局外交/落幕文本)全部列印為 0。開場白那一端(`scenarioPrologues.ts`)早就是分勢力的,這是補上對稱的另一端。
  - **假想盤的體例多一條支點**:讀者知道原本會發生什麼,所以「論曰」要拿**真實的結局**來對照,而不是憑空發揮 —— 「郭奉孝在,不使孤至此」之後接「而此局要問的是:他在,又如何?」;袁術那段整段引他真正的死法(問廚下惟有麥屑三十斛、欲得蜜漿而無蜜),再說此局把糧給了他而沒把人給他。唯一沒有史實可對照的是群雄集結(它本來就無史),改用蒿里行整首當敗文。
  - ⚠ **體檢腳本原本只數歷史盤**(`kind !== 'whatif'`),於是歷史線歸零之後看起來很乾淨,而假想盤那一側十七張全空 —— 兩兩中立、沒有落幕。假想盤最靠開局外交撐前提(「呂布據徐州」的呂曹死仇、「馬超得關中」的馬韓不疑),不寫進關係表就只是換了幾座城的主人。腳本已改成兩側都數。
  - **寫作體例**(照這個寫,新盤才不會歪):每盤挑 1–4 家「這張盤真正在講的人」,不必每家都寫。每家三段 —— `defeat`(正文,引正史原話,不加轉述)、`verdictZh/En`(勝方論曰,一句可帶走的判斷)、`verdictLostZh/En`(敗方論曰,通常留給一個具體場景或一句原話)。判斷句用 `**…**` 標粗,一段最多一處 —— 那是這一盤要留下的話。
  - ⚠ **兩個踩過的坑**:① 勢力 id 一定要對著那張盤查(三國後期是 `cao`/`liu-bei`/`sun`,不是 `wei`/`shu`/`wu`;戰國/楚漢的 `han` 分別是韓與漢)—— `scenarioVerdicts.test.ts` 會擋,但外交那一側不會。② `verdictLostZh` 後面的鍵是 `verdictLostEn`,打成 `verdictEn` 會變成同名鍵 —— **vitest 全綠而 `tsc -b` 才報 TS1117**,所以這個檔改完一定要 `npm run build`。
  - ⚠ **修了一個一直沒人發現的接線洞**:`EndingsModal` 原本只在 `victoryStatus === 'victory'` 掛載,於是那五段敗亡輓歌**一次也沒被玩家看過** —— 亡國時看到的只有 VictoryModal 硬寫死的一行「爾之霸業,就此而止」。現在勝敗都掛(z-index 990 蓋在 VictoryModal 之上,關掉後底下就是本局戰史年表);另補 `endingDismissed` 旗標,否則敗亡時彈窗關不掉。UI 端重算也補齊了 `victoryGoal` / `scenarioId`,先前與 store 端算的不是同一件事。
...
- **開局設定(三国志/全面戰争式)**:新局精靈第三步「開局設定」與遊戲內「設定」分區呈現 —— **難度**、**AI 強度 / AI 起始兵力 / 戰鬥難度**(見第十章)、**起始國力**(`startHandicap` 我方起始金/糧/兵 ×0.7/1/1.4,於 loadScenario 套用)、**經濟**(`startTaxRate` 起始稅率 輕/正常/重 + `startInflation` 起始通脹 無/輕 15/重 35,於 loadScenario 種入)、**勝利條件**、**遊戲模式**(輪流/一代記/演義/Roguelike/**鐵人**)、**生死規則**(武將壽命/壽命長短/不會戰死/起死回生,見 §2.4)、**世界規則**(武將與名品位置 歷史/隨機、戰霧、**在野登場**、**單挑頻率**、**天災頻率**、**新武將登場**、**虛構人才庫**)、**外交**(**初始外交**)、**歷代名將**。
  ...
  - **開局姿態**(`ForcePosture`,`buildInitialCities` 第二參數,2026-08):按勢力給城池初值乘數/增減 —— `troops` / `food` / `gold` 乘數 + `defense` / `loyalty` 絕對增減(後兩者夾在 0–100)。用來讓**序章寫的東西與數字對得上**:黃巾的序章寫「你沒有的:甲仗、糧道、能守城的人」,姿態就給 兵 ×1.30、糧 ×0.10、金 ×0.35、城防 −22、民忠 −18。184 盤五家見 `scenarios.ts`。
    - **旋鈕要量得出來才算數**(2026-08-04):糧原本是 ×0.40,而逐季追下來糧兵比開局 2.19、四十五回合沒跌破 2、缺糧逃兵零次、體檢斷糧欄 0/12 —— 五個旋鈕裡這一個是**空的**。尺度使然:每兵每季耗糧 0.25,而鄴、汝南、平原都是大郡,四成倉存仍夠吃八個季,秋收再滿回去。壓到 ×0.10 後糧兵比 0.73(約三季),第九回合出現第一次缺糧逃兵,斷糧欄 24/24(兩輪 12 場對照)。主目標達成率同時也動了(漢室 92%→71%、皇甫嵩 83%→67%、朱儁 92%→83%、董卓 17%→29%),但兩輪之間漢室自己就從 7/12 跳到 10/12 —— n=12 的二元結果雜訊就這麼大,**那一項沒有歸因**。
    - ⚠ **糧不是可用的長期旋鈕** —— 秋收會抹掉一切持續壓力,「無糧道」要一直咬人得動 `agriculture`(等於改城池本身)。它能表達的只有「起事之初那個窗口」,而那恰好就是史書講的那件事。
...
- **`break-force`(擊破,2026-08-04)**:把某一家壓到只剩 `maxCities` 座城以內即算數 —— 問的是**亂平了沒**,不是他死了沒,而且**不在乎是誰打的**。
  - 為什麼要有這一種:184 年漢室的主目標本是「於187年前徹底擊潰黃巾」,自走五輪 0/5 —— 而且**照史書也不會發生**(餘部入太行號黑山、散為青州黃巾,活過了這個王朝;這張盤自己的「黑山之聚」就寫著「自此亂非一戰可平,而成積年之患」)。
  - 它同時解掉**聯軍歸屬**:皇甫嵩軍與朱儁軍在盤上是獨立勢力,城是他們打下來的、記在他們名下(體檢實測舊黃巾十七城終歸皇甫嵩五至八座),於是任何 `hold-cities` 寫法對漢室都判不對。
- **`retain-officer`(留人,2026-08-23)**:到 `byYear` 那一年他還在你麾下且活著即成功;**中途死了或叛走當場判敗**,不等期限 ——「田豐之言」的戲就在他會被殺,那一刻就是這條目標的結局。
  - 為什麼要有這一種:全庫十三條次要目標寫的是這件事(呂布「公臺不去」、劉璋「孝直不叛」、袁紹「田豐之言」、朱儁「織席販履之徒」、皇甫嵩「孟德在幕」……),句句都在說「他現在是你的,別弄丟了」,而它們一律寫成 `recruit-officer` —— 那型問的是**他歸你了沒**,人本來就在麾下,於是**第 0 旬就成功**,十三條全是白送的分。
  - 怎麼發現的:`objective-ownership-audit` 原本**只掃主目標**,而全庫 972 條目標裡有 432 條是次要的,從來沒被掃過。把「第 0 旬就成立」那幾類硬性檢查擴到次要目標,一跑就是這十三條加上漢中王的「跨有荊益」(開局全據成都、江陵而且沒有期限)。
...
**試玩探測**(`e2e/playtest184.spec.ts`,2026-08-02):自走回答不了「人手裡是什麼感覺」。這支扮黃巾打二十回合、記每回合家底與事件、留截圖,是探測不是回歸(**必須 `--headed`**:React.lazy 彈窗在 headless 下不 resolve,城內畫面在 SwiftShader 下也看不見;截圖要給短超時,視窗被蓋住時 rAF 停擺會讓 `page.screenshot()` 一路等到逾時)。它抓到的兩件事都不是自走看得見的:**行為事件的門檻用絕對值**(黃巾十七城共 14,735 金、每城 866 全盤最窮,卻在第二回合彈「倉廩盈溢」),以及**晉牌封賞對太平道教主用朝堂封賞的配圖**。

`scenario-report.ts` 除城數/兵力/目標達成率外,另報**天命、府庫最低點、府庫見底輪數、斷糧輪數**(2026-08-02)—— 城數只說了「打得贏嗎」,說不了「活得下去嗎」。黃巾之亂的開局姿態把黃巾府庫壓到 ×0.35,是這幾欄確認了他們一路貼著三千過活而從未見底。同一份輸出也暴露了漢室的府庫最低點是其餘四家的三十倍,而它的序章寫的是「州郡的倉是空的」——序章與數字對不上,就是姿態該改的信號。

⚠ **量測本身壞過一次,而它壞的方式很安靜**(2026-08-06)。`scenario-report.ts` 開頭一直是 `loadScenario(scenario, forces[0].id)`,註解還寫著「我們從不下令,所以每一家都是 AI 驅動」—— 那句話是錯的:`planAITurn` 用 `isHuman()` **跳過玩家勢力**,於是 `forces[0]` 從頭到尾一兵不出,只被別人吃。而 `forces[0]` 在多數盤上正是那張盤的主角。

- 代價量得出來:208 赤壁 200 回合 ×3 輪,舊法曹操 48 → 11 城、孫權 14 → 36;新法(`observeScenario`)曹操 48 → 42、孫權 14 → 18。
- 據此得出的「曹操在 195/200/208 都會結構性崩潰、模擬撐不起大帝國」**是工具的產物,不是模擬的性質**,已作廢。
- 連帶作廢的還有一整批「照這個數字調過」的目標與姿態:184 朱儁的宛城從 1/6 調回 6/6(漢室一旦真的會動就會搶先拿走)、190 襄陽具名調硬(孫堅 4/4 / 劉表 0/4 → 反過來)。
- 正確入口是 store 的 `observeScenario`(`playerForceId: null` + `victoryStatus: 'observing'`,`ai-watch.ts` 一直在用)。**別改成傳一個假的 id**:玩家勢力 0 城會讓 `victoryStatus` 立刻變 `defeat`,整盤停住,實測 200 回合零易主。

⚠ **回合數要蓋過所有期限,否則 0/N 只是窗口沒到**。190 盤的目標期限落在 195–198,我先跑 150 回合(只到 194),十一家裡七家 0/6;跑到 340 回合(199 年)之後只剩三家 0/4,而那三家才是真正該修的。**看到一整排 0 先查期限,再查機制。**

另有兩種「目標是死的」值得單獨記,兩種都不會有任何測試報錯:
- **要 AI 走到它不會去的地方**:董卓的 `control-province: liang` 在涼州從九城變十二城之後,等於要求 AI 走到敦煌、張掖、酒泉 —— 體檢五輪峰值一律 9/12,那三座沙漠城一次都沒去過。
- **要 AI 打它打不下的地方**:公孫瓚的「攻取鄴」四輪 0 中,而鄴是袁紹治所,史實上他也一次沒進去。照本專案的準則(**主目標寫他真正做到的事,次要寫他沒做到的**),這種該降為次要。

與 `scripts/scenario-audit.ts` 互補 —— 一支查靜態一致性,一支查動態可玩性,別用其中一支代替另一支。

**體檢腳本自己抄了一份判定,而且抄漏了(2026-08-02 修)**:它原先手寫主目標的達成判斷,①不認得 `control-province`(董卓改用之後永遠報未達成),②只在**終局**判一次 —— 於是「188 年集齊涼州九郡、189 年因民變掉了兩郡」被記成沒達成,實測正是這樣。改成呼叫引擎的 `evaluateGoal` 並**逐回合鎖存**(與 store.ts 的 `newAchieved` 一致)。少一份會漂走的手抄本 —— 這是 `MarchPicker` 自抄城壁倍率那個坑的同一種。

**拖曳大地圖會把地圖畫壞 —— 根因與修法(2026-08-03 回報,08-04 定位)**

使用者回報「拉拽地圖變成一層層疊上去、看不出原來的地圖」。`e2e/mapDragSmear.spec.ts` 重現並量化:往北拖兩次,地圖區的**逐列平均絕對差**從 13 升到 58(乾淨 ≈13,壞掉 50–67,中間沒有灰帶)。

**根因**:大地圖的 `<Canvas>` 開了 `preserveDrawingBuffer: true`(📷 天下大勢那顆鈕要讀回畫面),意思是「合成之後不要丟棄繪圖緩衝」—— 於是**沒有任何人**會在下一幀開始時清它。走一般渲染路徑看不出來:three 的 `autoClear` 每次 `render()` 都會清。但 **EffectComposer 接管之後,畫面是靠最後一個 pass 的全螢幕四邊形貼上去的,那條路徑從頭到尾沒有「清除」這個動作** —— 沒被蓋到的像素就留著上一幀,平移時每幀留下一層位移過的殘影,一層層疊起來。

**修法**:`StrategicMap3D` 的 `<Canvas>` 直屬層掛一個 `ClearFrame`,`useFrame(({ gl }) => gl.clear(true,true,true), 0)`。後處理因此**恢復預設開啟**,四時之色/泛光/暗角都回來了,截圖鈕也不受影響。

⚠ **位置敏感**:同一段程式放進 `ScenePostFx`(與 `<EffectComposer>` 同層)完全無效,12 跑全壞;補 `setRenderTarget(null)` 也救不回來。放在 `<Canvas>` 直屬層、掛在場景之前才有效,推測與 `useFrame` 在同一 priority 內的訂閱順序有關。要動它請先跑 `--repeat-each=6`。

單挑/舌戰的 Arena 也開著 `preserveDrawingBuffer`,但它們不走 `ScenePostFx`,沒有 composer 就沒有這個症狀 —— **日後若替它們加後處理,必須一併帶上這一層**。

**定位過程中三個方法上的教訓,比這個 bug 本身重要:**

1. **Playwright 跑的是 `vite preview` 服務的 `dist/`,不是原始碼。** 我改完原始碼只跑 `tsc -b` 就測,連做七次「二分」—— 那七次跑的是同一份 dist,clean/broken 全是雜訊,結論(先怪 N8AO、再怪光暈、再怪分級)全錯。改原始碼做 e2e 二分,**每次都要 `npm run build`,而且要確認它沒有因未使用匯入而失敗**。
2. **間歇性的 bug 不能用單跑判定。** 漏檢率約六分之一:單跑「乾淨」有六分之一純屬運氣 —— 我一度就是這樣得出「關陰影就好了」的錯誤結論。
3. **用眼睛看截圖不是判準。** 十二張截圖分不出來,而一個逐列差異的數字一眼分開(13 vs 58)。**先做出判準,再開始二分。** 後續幾個關鍵證據也都靠它:強制改視窗大小會把 corruption 抹掉(→ 是舊像素不是渲染錯誤)、三幀指紋相同(→ 靜態,不是雜訊)、沿螢幕高度分帶量測(→ 近景最嚴重)。

**⚠ 五輪的變異很大。****⚠ 五輪的變異很大。** 同一組設定,朱儁的主目標達成率在 3/5、4/5、5/5 之間跳。用它判「這條目標是不是自動達成」可以,用它分辨 3/5 與 5/5 則是在調雜訊。要下結論就加輪數,或改用逐回合的專項探針(如「朱儁第幾回合取宛城」)。

**目標的期限語意(2026-08-02)**:「取得型」目標(開局不據有那座城)原本只有達成與進行中兩態,期限寫了等於沒寫 —— 黃巾的「186 年前攻取洛陽」打到 200 年仍是進行中。現在與 `defeat-force`/`recruit-officer` 一致:`year > byYear` 即失敗(期限**當年**仍在期限之內)。守成型的判法不變。釘在 `objectives.test.ts` 的「目標的期限」。

**皇甫嵩的地盤原本在徐州(2026-08-03 修)**:盤上他據有彭城、下邳(鄰近填充連帶琅琊、小沛)—— 而他 184 年是左中郎將,一支野戰軍:五月長社(潁川)火攻破波才,轉東郡倉亭擊卜己,十月代董卓攻廣宗斬張梁,十一月下曲陽斬張寶。**他一生沒到過徐州**;亂平之後領的是冀州牧。184 年的徐州該是朝廷刺史部(陶謙 188 年才出任徐州刺史)。

更硬的證據在事件表裡:「長社之火」的 `chooserRulerId` 寫的是 `huangfu-song`,而它「堅壁待援」那一選項給的是**許昌**的城防 —— 長社屬潁川,盤上以許昌代表。**事件早就預設許昌在他手裡,而歸屬表把許昌給了漢室。**

改成許昌(長社)+ 陳留,鄰近填充順勢帶上白馬、延津、官渡 —— 洛陽到鄴的走廊,他北上廣宗要走的路。城數 7 → 5、兵 55,675 → 41,575,少而精,合乎一支「有兵無土」的中郎將部隊;徐州四城歸還漢室(38 → 40)。

**歷史修正順帶修好了平衡**:他的主目標(取鄴與信都)從 0/5 變 2/5,而黃巾終局城數從 7 掉到 3 —— 因為白馬、延津本來就與鄴相鄰,把他放回該在的位置,他就真的會北上。這比再調一次數字誠實。

同批修掉序章的內部矛盾:皇甫嵩那篇寫「長社一戰,你順風縱火……朝廷因此把北方交給你」,而長社之火是第 9–10 回合才觸發的**局內事件**(chooser 正是他)—— 序章先把玩家的第一場仗講完了。「督冀州」也早了半年。改成前瞻語氣,與朱儁、董卓兩篇一致。

**用「抉擇者 × 城池效果」交叉檢查事件(2026-08-03)**:許昌那條不是我看出來的,是資料兩處互相打臉。把它變成可重跑的檢查 —— 凡是同時有 `chooserRulerId` 與指名城池效果的事件(全庫 4 條),核對那座城開局屬不屬於抉擇者的勢力。結果:

| 事件 | 城 | 判讀 |
|---|---|---|
| 長社之火 | 許昌 | ✔(已修) |
| 笮融建浮屠祠 | 下邳 | ✔ 陶謙自己的城 |
| 決泗沂之水灌下邳 | 下邳 | 「不屬」但**正確** —— 效果全是損害(城防 −40、兵 ×0.8、糧 −8000、民忠 −10),那是敵城 |
| 宛城之圍 | 宛城 | 「不屬」而且**有問題** |

所以判準不能只看歸屬,要看效果是**增益還是損害**:增益落在別人城上才是 bug。`宛城之圍` 的「納降」給宛城 +10 民忠 —— 那只有在宛城已經是朱儁的時候才說得通;而它的敘述與兩個選項都在講攻下宛城,效果裡卻沒有任何一項把城交給他。兩個毛病指向同一個答案:它本來就是「攻下之後」的善後抉擇,只是條件漏寫了。條件表裡正好有 `city-owner-ruler`(註解寫著 conquest payoffs),補上即可。刻意**不**讓選項直接授城 —— 那會讓朱儁的主目標白送。

**補完之後崩盤垮了,而這正好暴露了平衡的真正支點。** 黃巾從 17→3 變成 17→**19**(反而長大)—— 原來崩盤有一大半重量壓在那條無條件觸發的事件上。史實上瓦解這場運動的是**張角之死**,不是宛城:「太平道一教之眾,所信者一人耳。」把重量移回張角病死(反正 0.40 → 0.50、兵 ×0.55 → ×0.45)與黑山之聚(0.30 → 0.35)之後,黃巾 17 → 6(1–9),而宛城的大反正成了玩家自己掙來的戰功。

**184 年少了它最重大的政治事件(2026-08-03)**:補這條之前,整個 184 年是純軍事的 —— 長社、廣宗、宛城,全是打仗。而史實上這一年真正改變走向的一道詔是**解黨禁**:皇甫嵩上言「宜解黨禁,益出中藏錢、西園廄馬以班軍士」,中常侍呂彊也說「黨錮久積,人情怨憤,若不赦宥,輕與張角合謀,為變滋大,悔之無救」—— 設黨錮的正是他們這些人。靈帝懼而從之,士大夫因此沒有倒向太平道。這是整場亂事最大的一個「差一點」,所以它該是抉擇而不是背景設定。

`evt-yt-dangjin`,春季(詔在三月,長社在五月,順序不能顛倒),抉擇者為漢室。解:出中藏錢 1,800、漢室天命 +10、黃巾天命 −10,何顒還鄉出仕。不解:黃巾天命 +10、兵 ×1.08(郡國失意士人各結部曲,而黃旗是現成的旗)。體檢五輪皆於**第 1 回合**觸發 —— 玩家的第一個決定是政治的,不是軍事的。

**查了但沒有補的:黃巾諸帥的腳本死亡。** 十二人裡只有張角有死亡事件,其餘十一人史實上多半死於 184 年。但實測三輪跑到 189 年:張角/張寶/張梁/張曼成/趙弘/韓忠/孫夏/馬元義**三跑全死**(城破時的俘殺系統自己做到了),而管亥、裴元紹三跑全活 —— 那兩位正是史實上活過 184 年的(管亥 193 年圍北海、裴元紹到官渡前後)。湧現已經對了,補腳本是多餘的。
...
1. 第一版用「全形，？！」當機翻標記,漏了三條(`dlg-orphan-petition` 等);真正可靠的是**檔案裡的區塊界線** —— 舊批是連續的一段,新批有自己的 banner 註解。
2. **掃到 0 只代表「對我這張表乾淨」。** 第一版字表是按部首手寫的,掃描報 0 之後,教學浮層裡仍留著「世界**视**角」「**拦**江鎖」「**横**江」。補洞的辦法要留在流程裡:用 OpenCC 逐字反查「src 裡還在、表上沒有、也不在歧義名單」的字,第二輪就這樣補出 39 個字、又修掉 108 處。
...
- **演義觀測 `scripts/ai-watch.ts`(2026-07-26)**:`npx tsx scripts/ai-watch.ts <scenarioId> <turns>` —— 用演義模擬器(無玩家、全 AI)推 N 旬,統計城池易主/出兵數/圍城旬次/施設峰值,以及各系統在結算報告裡的關鍵詞次數。**單元測試問「這機制能不能觸發」,這支問「一場戰役下來它有沒有真的跑」** —— 兩個沉默的 bug(AI 長圍完全失效、黃天教在楚漢盤起義)都是它撈出來的。關鍵詞分 common/rare,只有 common 掛 0 才標警告;陷城不列關鍵詞(無單一文案,改看精確的易主計數)。**加關鍵詞前務必對照實際 textZh 校過** —— 誤報的體檢工具比沒有更糟。
- **CI(`.github/workflows/ci.yml`,2026-07-26)**:兩個 job —— **check**(`tsc -b` / vitest / build)與 **e2e**(chromium + Playwright,失敗上傳 report)。Node 釘 24 對齊本機。**Lint 刻意 `continue-on-error`**:樹上約 300 個既有 findings,今天就讓它擋 build 只會換來一條永遠是紅的 CI,那比沒有更糟;等積壓清掉再拿掉這行。
- **`resolution.test.ts`(2026-07-26)**:resolution.ts 是全專案最大的檔(約 4.9k 行)、單一匯出,此前只被七個主題測試與十二個 integration 側面掃到 —— AI 長圍那個 bug 就活在這條縫裡。新測試寫的是**契約**:輸出可被 store 套用(帳目非 NaN/非負、民忠 0–100)、**不得改動傳入的 cities**、固定 rng 可重現、跨旬存活(carried 指令必有活著的武將與對應的 army、**同一武將不得在 carried 裡出現兩次**)、餵回自己連跑十旬不漂。
...
- **劇本目標 objectives.ts(86/86 全覆蓋,370 條)**:每個劇本各勢力都有**專屬目標**(`findObjectiveFor(scenarioId, forceId)` 按玩家所選勢力查,ObjectivePanel 顯示進度;`primary` 主目標 + 可選 `secondary` 次目標)。2026-07-25 由 9 個劇本擴至**全部 86 個**:
  - ⚠ **「86/86 全覆蓋」量的是盤,不是家。**2026-08-07 逐家數了一次:540 家勢力裡有 **147 家沒有主目標**(52 張盤)—— 而選勢力那一頁**不篩**,任何一家都選得到,選到就是一張空的目標卡。已全部補上,`0 家`。
  - 補的方式:每個勢力身分寫一段自己的框架(士燮「兄弟並為列郡守,四十餘年疆場無事」、公孫瓚「邊地之兵,慣與胡騎相馳」、趙「胡服騎射以教百姓」…),城則取**該盤上他真正握著的治所 + 次大城**,期限統一 start+6(三國線的中位窗口)。文字不寫「你的兩座主城」這種話 —— 目標卡上必須看得到城名。
  - ⚠ 第一版生成時把城取成「人口前二」,於是齊的旗幟寫著臨淄而目標查的是彭城下邳。**治所要排第一** —— 那是這一家的身分城。
  - **序章那一側也補齊了(2026-08-07,80 → 0)** —— 全部手寫,沒有用生成的。體例是**照人寫、不照盤寫**:同一家在不同盤上講的是同一個人的一段人生,每一段接著上一段。張魯 204→215 五段(從「置義舍,米肉懸於道」到陽平關夜裡的麋鹿與那一封庫)、士燮 204→221 八段(從「你今年六十七,你會活到九十歲」到「五年後你會死,六年後你的家族會滅門」)、馬騰 194→207 六段、鮮卑八段(軻比能:「最後殺死你的不是軍隊,是一個刺客」)、南蠻七段(收在七擒之後)。
  - ⚠ 這一批踩了三個同型的坑,都是**沒有先查那張盤的實際 id/內容**:① 207 本來就有劉璋序章,我照一份截斷的清單多寫一段 → TS1117 重覆鍵;② 「這家已經有了嗎」的檢查範圍越界掃進下一張盤,八家全被判為已有而靜靜跳過(輸出「補上 0 家」看起來一切正常);③ **同一個勢力在不同盤上的 id 不一樣** —— 197 的劉璋軍是 `liu-yan`、204 的公孫康軍是 `gongsun`,寫錯了 `scenarioPrologues.test.ts` 會擋(它擋住了)。
  - **三國正史線 42 本**(189 十常侍 → 280 晉滅吳):主目標寫該勢力的史實抱負,副目標給一條**翻案線**,並標明史書原本給了多久 —— 公孫瓚易京守到 202(史書只到 199)、袁術壽春活過 199、劉表不獻荊州、諸葛誕守住壽春、劉禪存續至 270。
  - **假想 17 本**:目標即前提 —— 關羽守住荊州→隆中對兩路北伐成立;郭嘉不死→「郭奉孝在,不使孤至此」;周瑜不死→其生前取蜀二分天下之策;曹爽先發制人→挾天子幸許昌。女傑時代七勢力各按其人立目標。
  - **戰國 14 / 楚漢 8 / 隋唐 5**:這些劇本共用三國地圖與紀年(開局皆 178 年),故 `byYear` 用**遊戲內年份**;城池代換見 scenarioPrologues 各本 intro 末尾。
  - **靜默失效防護**:`findObjectiveFor` 以 (scenarioId, forceId) 查表,任一 id 配錯**不報錯只是面板空白**,goal 指向不在該劇本地圖上的城/武將亦然。`objectives.test.ts` 逐條驗 scenarioId / forceId / cityIds / officerId / provinceId 是否在**它所屬的劇本**裡解析得到,外加 id 唯一、期限晚於開局、中英描述齊備。它上線即抓到一條既有失效:反董卓聯軍(scn-190)裡並無 `liu-bei` 勢力(此時劉備在公孫瓚麾下),那條目標從寫下起從未生效 —— 已改掛該劇本真實存在的 gongsun。

- **劇本序章 scenarioPrologues.ts / PrologueModal(86/86)**:選完劇本不再直接掉進地圖。每本一段 `intro`(天下大勢,人人可見)+ 逐勢力一段 `forces[forceId]`(只有你這一家聽得到的話:你是誰、難在哪、此刻要做的那個決定);31 本備有逐勢力段落。序章之後接**「所圖」**,把該勢力主/副目標連同描述一併列出 —— 上面那 370 條目標的副目標與 `descriptionZh` 在角落 HUD 面板**沒有位置顯示**,開局這一頁正是它們被讀到的地方。
  - 接線:`loadScenario` 依劇本有無序章設 `prologueOpen`(舊存檔無此欄位 → falsy → 不彈);演義模擬器(observeScenario)強制關閉;序章開著時不彈事件。**刻意不動 Scenario 型別/runtime/存檔**(同 cityLore 的做法),沒有條目的劇本就是沒有序章。
...
#### 11.4.3 戰術層 AI 體檢(scripts/tactical-watch.ts,2026-07-26)

`ai-watch.ts` 觀測戰略層,戰術層一直沒有對應物 —— 一個戰場行為可以在所有棋盤上都是死碼而無人知曉,因為單元測試只證明它**能**觸發。新腳本跑 N 場 AI 對 AI 戰術戰鬥,以戰報關鍵字統計**它到底做了什麼**:

    npx tsx scripts/tactical-watch.ts [場數] [每場回合上限]

首跑就抓到兩件事,而**其中一件是腳本自己的缺陷** —— 值得記下來:

- **雲梯 0 次是我的錯**:第一版讓一方三個單位同兵種,於是帶攻城械的一方沒有步兵、帶步兵的一方沒有器械,雲梯登城在設定上不可能發生。改成混編後 0→9,破牆場次也從 8/20 升到 12/20。**觀測工具的樣本設定本身會製造假陰性。**
- **搶修 0 次是真的,而且有兩層原因**。第一層:守衛寫成「三格內無敵」,但牆之所以被砸正因器械貼在牆外,內側守軍與它恰好相距 2 —— 「有損毀可修」與「安靜可修」幾乎互斥。第二層(放寬守衛後仍為 0,插探針才看見):分支被進入 **635 次而隔壁從來沒有受損牆** —— **AI 守軍根本不會走到被砸的那段牆旁邊**。兩層都補上(守衛改為「未被貼身接戰」+ 新增 `赴缺` 行為:未接戰且鄰無損毀牆時,最近的閒置守軍走向**最弱**那段牆)後,60 場中搶修 5 次,破牆場次不變。

  → 教訓:**放寬條件之前先確認前提會發生**。我第一次改對了守衛卻沒解決問題,是因為假設了「守軍本來就在牆邊」。

##### 熱座 PvP 戰術戰鬥

雙人對戰若打到戰場上,原本有一個與戰略層**同形**的洞:戰場畫面看 `playerForceId` 決定聽誰的令,而 `BattleAIDriver` 只要 `activeSide !== playerSide` 就開打 —— 於是回合一交到另一個真人手上,AI 就替他下令。

- `isPvpBattle(攻方, 守方, 座位表)`:兩邊都在座位表裡才算。AI driver 遇此**直接退場**。
- **鍵盤跟著回合走**:`playerForceId` 在全 app 的意思都是「正在看這個畫面的人」,所以戰場也照這個意思辦 —— `activeSide` 一變就把它移到該側的勢力並升起換手卡。戰場畫面因此**完全不需要知道有兩個玩家**,它照舊只聽 `playerForceId`。
- 換手卡改渲染在 MapScreen **最後**(原本在戰場螢幕之前,會被戰場蓋掉;戰場最高 z-index 1020,卡片 9000,但 DOM 順序仍要對)。

#### 11.4.2 浸泡 fuzzer 當武器用(2026-07-26)

`soak.integration.test.ts` **完全沒有種子** —— 它跑真實 48 旬並逐旬斷言不變量。這代表**偶發失敗是真的邊緣案例,不是 flake**;懷疑 flake 時應連跑 10 次以上,跑不出來才叫 flake。這一輪用它抓到五個真 bug。

**方法**:比對「它斷言了什麼」與「它沒斷言什麼」。原本只查 officer 的 `status` 合法,卻不查忠誠範圍、不查軍隊統帥是否在世、不查一將一營。**每加一條真不變量就是一次抓 bug 的機會。**

新增九族不變量:一將一營(結構上 `army.id === officerId`)、統帥在世、部伍有人、軍屬活勢力、歸師不圍城、追擊之的須存在、忠誠/五圍為數且有界、死者不仕、城池府庫非負/腐敗有界/屬活勢力、君不為屍。

抓到並修掉的:

1. **紮營之軍同時在避戰** —— `evading` 註釋寫著「Cleared on hold」,玩家切換也防呆,但**只有 AI 會設它**,而 resolution 三處紮營轉換一處都沒清。
2. **追擊一個幽靈** —— 追擊驗證拿「本回合開始時」的行軍表比對,獵物若在同回合中途抵達庇護或被殲滅,追兵就整季指著不存在的目標,並因 `pursueTargetId` 仍在而**跳過重新選敵、白費一季**。
3. **屍體領軍** —— `deriveArmy` 的守衛只測 `cmdr?.forceId`,而多數死亡路徑會清 forceId、**但有七處不清**(人質處決、傷重不治、暗殺、密謀、政治婚姻…),於是死人通得過守衛繼續行軍作戰。
4. **兵力歸零的軍隊留在圖上** —— 同一個守衛缺口。
5. **勢力由屍體統治** —— `applySuccession` 在 tick 中段跑,但君主會在那之後才死(晚期戰鬥、老死、當季事件、死諫),於是死君主的魅力仍在招賢、性格仍在驅動 AI、威名仍在威懾,整整一季。

**修法一律在「唯一接縫」而非各發生點**:①②③④ 修在 `deriveArmy`(每支軍隊誕生的唯一入口)與 `endSeason` 提交前的軍籍淨化;⑤ 修在提交前的最後補位。理由與 §2.9b 相同 —— 死亡/紮營的路徑至少七處,逐一補丁等於邀請第八處漏掉。

另加 **長程浸泡(240 旬)**:48 旬只走到 179 年、7 勢力、2 場攻城,整個晚期(統一、繼承、大規模叛附、朝堂、承平之亂)從沒被跑到。上面 ③④⑤ 全是長程才浮現的。

**唯一刻意不修的**:全部武將皆亡或被俘的勢力仍由屍體統治 —— 那是真的沒人可繼,屬「realm 已無人」而非帳務疏漏,不變量對此明文放行。

**第二批不變量(2026-07-27)**:死者不得持有官職/軍團/州牧、官爵互斥(`CivicTitle.excludes`)、一職一人(非太守職一勢力只能一人)、一人一職不重複、兵籍相鄰旬不跳變(比值須在 0.5–2,抓的是憑空增減而非正常徵募與傷亡)。

這批**順帶量了一遍「每條斷言在浸泡中到底咬不咬得到東西」**,結果一半沒有 —— 這件事比不變量本身更值得記:

- `appointments` 最多 76、`provinceGovernors` 最多 5 → 官爵那兩條真的在測。
- `cityDelegations` 與 `legions` 全程為 **0** → 旁觀者不會委任也不會建軍團,那兩條空轉。
- **死亡武將全程為 0** → 三條「死者不持有」全部空轉。

第三點差點被當成「老死系統壞了」報出去。查了才知道是對的:`deathChance()` 對有史實卒年的武將,在該年之前一律回傳 0,而**所有劇本都開在遊戲紀年 178** —— 178 到 188 年歷史上本來就幾乎沒有名將亡故。系統沒壞,是這段年份就這樣。

空轉的斷言留著(玩家的真存檔、以及委任/建軍團的戰役都會咬合),但註釋裡寫明「目前不觸發、原因是什麼、不要靠放寬來『修』它」。**一條永遠不執行的斷言看起來像保護,其實什麼都沒擋。**

> **⚠ 上段的「死者不持有」結論是錯的,已更正。** 我的覆蓋率探針只數了**老死**,於是報告 240 旬零死亡,我便斷定那三條是空轉。但**武將會戰死** —— 48 旬的浸泡裡就有人陣亡,斷言確實會咬,而且第一次咬就抓到真 bug:
>
> **州牧死了,職位永遠不會空出來。** `appointments` 有 `pruneStaleAppointments` 每季清掉死亡/被擒/叛去/失城的任命,`provinceGovernors` **沒有任何對等物**。州牧一死,那個州就永遠「有人治理」:活人再也拿不到位子,所有州牧驅動的效果繼續記在屍體頭上。修在與 appointments 同一個接縫(endSeason 提交前),且讀 `result.provinceGovernors` 而非 state 舊值 —— 否則會把 AI 本季剛任命的州牧一起清掉。
>
> 教訓:**量一條斷言能否觸發時,要窮舉所有讓它成立的路徑,不能只想到第一條。**

#### 11.4.6 平衡回歸鎖(balanceLock.integration.test.ts,2026-07-27)

§11.4.4 的種子接縫落地後才寫得出來的測試 —— 在那之前,任何跑真實世界的平衡鎖都注定間歇性失敗然後被靜音。

鎖的是**區間與形狀**,不是精確數值。釘死「某某第 24 旬有 7 座城」會在每次無關的內容改動時爆掉,教會所有人不讀內容就改數字。實際斷言:天下沒被一家吞掉(<75%)、群雄仍在(≥3 家)、版圖確實有變動(AI 真的在打)、民生沒崩(人口 >60%)也沒暴漲(<300%)、兵籍在 0.3–2 倍之間、民心在可玩區間、沒有 NaN 或負值。三個種子各跑 240 旬,最後一條反過來確認三個種子產生的世界**彼此不同** —— 否則上面所有區間都是空話。

量到的基線(2026-07-27,供日後比對):十年兵籍約淨減 33%(104 萬 → 70 萬),逐旬平滑無跳變。

兩個踩過的坑:

- **`endSeason` 是一旬一次,一季九旬。** 我按「40 季」寫,實際只跑了 4.4 季,三個種子的版圖當然紋絲不動 —— 差點把這當成「AI 十年不打仗」的 bug 報出去。
- **每個種子跑了兩遍**(describe 的 beforeAll 一次,最後那條「不同種子」又重跑三次)。整套跑時超過預設 5 秒逾時,而 vitest 的反應是報一個失敗**外加整個 describe 的 9 條被 SKIP**。綠底下的 skip 比紅字更糟。

#### 11.4.4 戰役隨機源 —— 一局可重播(campaignRng.ts,2026-07-27)

在此之前,**整個模擬層都在種子之外**。表面症狀是「seeded 平衡鎖三跑兩炸」與「ai-watch 跑兩次比對不起來」,真因不是散落的裸 `Math.random`,而是兩個呼叫:

- `endSeason` 呼叫 `resolveSeason` 與 `planAITurn` 時**一個 rng 都沒傳**。這兩個模組各自都有 `input.rng` 接縫,而且內部確實在用 —— 但接縫的預設值是 `Math.random`,呼叫端不傳就自動退回裸隨機。於是 4,943 行的季結算主體(人口、稅收、野戰、俘獲、攻城)與整個 AI 決策層,全都不受種子控制。
- `endSeason` 內另有 41 處寫成 `rng: Math.random` —— 系統明明開了接縫,store 硬是餵裸隨機進去。

**機制**:`campaignRng(state)` 是 (種子, 日期) 的純函式 —— 同一狀態必得同一流(這就是可重播),日期推進自動換流(否則每旬都會擲出一模一樣的結果)。種子存在 `GameState.rngSeed`,`loadScenario` 鑄一顆並持久化進存檔(不持久化的話重載就換流,跨場次的重播就斷了)。

**刻意不種子化的**:玩家主動觸發的 30 處(執行計略、鑑定、精煉、攻打關隘…)。兩個理由 —— 玩家自己按下去的那一下就是那個骰子;而且若拿狀態當種子,一次「失敗且不改變狀態」的嘗試會被凍成永遠失敗。同樣理由,`loadScenario` 自身的隨機(虛構武將、開局天候)也不種子化。

**抓漏的方法比修法值得記**:比對「兩次跑的世界指紋」不夠 —— 低頻事件在有限旬數內可能根本不觸發。真正把最後四個洞挖出來的是**攔截 `Math.random` 本身並記錄呼叫棧**,一跑就指名 `duel.ts` / `wordWar.ts` / `aiParley.ts`(seasonBouts 呼叫 `resolveDuel` / `resolveWordWar` / `tickAIPersuasions` 時全是裸的)。這招已留成常設測試(`seasonReplay.integration.test.ts` 第 4 條),比指紋比對更能擋回歸。

**現況**:240 旬逐旬比對零分歧,且 `endSeason` 期間裸 `Math.random` 呼叫數為 **0**。

#### 11.4.5 存檔遷移 —— 停止「改 key 換版本」(saveMigration.ts,2026-07-27)

存檔 key 曾是 `tkm-save-v26`,而版本是靠**改這個字串**來換的(v25 → v26 → …)。zustand 只查設定的那一個 key,所以 key 一改,所有進行中的戰役當場變成找不到 —— 具名存檔槽存在別處所以活著,但玩家正在玩的那局自動存檔會安靜消失。這不是遷移,是加了幾道手續的刪除。persist 選項裡當時既沒有 `version` 也沒有 `migrate`。

現在:

1. **key 凍結在 `tkm-save`,再也不准改**(測試釘住這個字串,想改會先撞 CI)。schema 變動改用 `SAVE_VERSION`。
...
`endSeason` 是**同步**的,內部提交約 27 次(每個 `set()` 都是一次 zustand commit),而 persist 在每次 commit 後都把整份約 690 KB 的存檔序列化並寫入一次。實測開局劇本:

| | 合併前 | 合併後 |
|---|---|---|
| storage 寫入 | 1.9 次/旬 | **1.0 次/旬** |
| `JSON.stringify`(大物件) | 約 2 次/旬 | **1.1 次/旬** |
| 60 旬序列化總量 | 44 MB | **23 MB** |

那 1.9 次裡除了最後一次,寫的全是**存活數微秒、誰也載不到的中間態世界**。桌面上只是浪費;但真正的目標平台是 iOS —— 每次寫入是一次整份 stringify 加一次 IndexedDB 交易。
...
**但「包哪一層」是關鍵,第一版包錯了。** 最初的寫法是包一個 `StateStorage`(字串層)墊在 `createJSONStorage` **底下** —— 結果只合併掉 IndexedDB 寫入,序列化一次都沒省:persist 在每個 commit 都先 stringify,只有最後那個字串進到緩衝區。量出來一目了然 —— **60 旬 60 次寫入,卻有 121 次 `JSON.stringify`**。貴的那一半根本沒被合併。改成直接實作 `PersistStorage`(物件層)、把 stringify/parse 收進模組自己做,才真的推遲到落盤那一刻:同樣 60 旬,stringify 降到 66 次。

> 教訓:**量「你以為省掉的那件事」本身,不要量它的代理指標。**當時我數的是 `localStorage.setItem` 的位元組數,它確實減半了 —— 但那衡量的是寫入量,不是序列化量,而我把結論寫成了後者。
...
#### 11.4.9 `endSeason` 拆到哪裡為止(seasonEpilogue.ts,2026-07-28)

`store.ts` 17,998 行,其中 **`endSeason` 一個 action 就佔 5,459 行(30%)**。391 個 action 裡第二長的 `applyTacticalResolution` 才 728 行 —— 這不是「平均都長」,是一頭獨大。

抽出 **`seasonEpilogue.ts`(375 行)**:得將開卡、名場面音效、圍城/伏擊功業、成套之禮、歷戰勳章、天下懸賞、切磋雙修、巔峰入冊、史官年鑑、武評前席、軍師點撥。這些塊共用一個形狀 —— **世界已經結算完了,它們讀「結算前 vs 結算後」這一對狀態,決定要告訴玩家什麼**。沒有一塊會改變本季的結果,只改變玩家看見的版本。契約與 `seasonBouts.ts` 相同:只透過 store 自己的 `get`/`set` 讀寫,行為與 inline 時完全一致。

順帶把 `stripPaint`(亡國褪色)從 store.ts 移進 `systems/hexPaint.ts` —— 它是操作 `HexPaint` 的純函式,本來就該在那裡,留在 store 只會逼新模組去 import store 而造成值層級的循環相依(型別 import 不會,值會)。

**然後就停了,這是刻意的。** 逐塊量過「引用了多少個 `endSeason` 的中間變數」之後:

| 段落 | 行數 | 外部依賴 |
|---|---|---|
| 季末落幕(已抽) | 375 | 0 |
| C+H 器物共鳴 / P12 婚姻相性 | 173 | 1(`postOfficers`) |
| 傷兵恢復 | 153 | 3 |
| 大工 §1.15 | 227 | 5 |
| 入主建國 | 371 | **14** |
| Academy training | 653 | **17** |
| §7.1-deep 盟約 | 514 | **28** |
| 最終提交(事件簿) | 494 | **281** |

`postCities` / `postOfficers` / `postForces` / `planned` / `espResult` 這些中間態**在各階段之間流動並且被就地改寫**(`postOfficers = resonated`),這正是 `endSeason` 不能被機械切開的原因 —— 它不是二十個獨立步驟,是一條有狀態的長流水線。硬把中段抽出去只會得到一個要傳十幾個參數、還得回傳一包改寫結果的函式,比留在原地更難讀。真要拆得動,得先把中間態顯式化成一個 `SeasonContext` 物件,那是另一次專門的重構,不該混在別的批次裡順手做。

**方法論的坑**:第一版依賴分析器報「Academy training 有 501 個外部依賴」,裡面是 `above`、`academy`、`active` —— **註釋裡的英文單字**。本專案註釋是密集的英文散文,掃識別字前**必須先剝掉註釋與字串字面量**。剝完仍有一層噪音:物件字面量的 key(`cityId:`、`kind:`、`text:`)和型別欄位名會被當成變數引用,要再濾一次 `(?<![.\w])\w+\s*:`。兩層都濾掉,數字才從 501 收斂到 17。

#### 11.4.10 大地圖為何會卡 —— 是 draw call,不是三角形(2026-07-28)

玩家回報大地圖「卡到不能用」。實測**靜置**(什麼都不做)於預設視角:

| | 修復前 | 修復後 |
|---|---|---|
| 每幀 draw call | **17,279** | **~5,000–6,900** |
| 場景可繪物件 | 5,633 | 5,633 |
| 放大率(call ÷ 物件) | **3.07×** | 0.89×(視錐剔除後) |
| FPS(headless 軟體渲染) | **19** | **47–60** |
| 拖曳每步 | 228 ms | 101 ms |

**先排除了兩個嫌疑,再動手。**存檔嫌疑最大(當天剛改過),但探針顯示靜置、拖曳、滑過全程 **零序列化** —— 存檔與地圖流暢度無關。CPU profile 則全部落在 `renderBufferDirect` / `uniformMatrix4fv` / `bindVertexArray` / `drawElements`,那是**每個 draw call 一次**的開銷,不是幾何量的開銷:整張地圖只有約 130 萬個三角形,現代 GPU 一幀畫幾千萬都不喘。**病因是提交次數,不是幾何複雜度。**

兩個修法,各自對應一個數字:

1. **城池細部分級(`ZoomLODCtx` 加 `mid` 層)**。每座城是二三十個獨立 mesh(城牆、門樓、塔的每一層、角樓、側殿、一圈市坊民居),而預設相機高度 `MAP_D×0.9` ≈ 135 之下,**一座大城在螢幕上不到四個像素寬**。門檻按「城池值不值得畫」定:`near`(<45,城約 11px)畫全套,`mid` 起只留輪廓。預設視角落在 `mid` —— 這是刻意的:**貴的那一層是你主動湊近才進去的,不是你一開局就待著的。**
2. **陰影隨之關閉**。陰影 pass 把每個投影物件**再畫一遍進 shadow map**,所以它是**乘法**而非加法 —— 3.07× 的放大率就是它。而四個像素的城池,其陰影根本不可解析。`castShadow={zoomLod === 'near'}`,一行。
...
- **風物志上圖(2026-07-25)**:大地圖懸停一座城,快覽卡除城名/歸屬/兵/糧外再帶一行**風物志首句**(`cityLoreBrief`,中文切「。」英文切「. 」,另有字數上限)。**刻意不受戰爭迷霧限制** —— 風物志是地理與舊事而非軍情,誰都知道赤壁是什麼地方;副作用是迷霧中的敵城不再只剩「情報不足」。全文仍在城市面板總覽頁。
- **3D 三圖的防護對齊(2026-07-25)**:兩套防護原本各只裝在一張圖上,現各補到缺的圖上。
  - **WebGL 上下文丟失恢復**(`hooks/useGLRecovery.ts`):iOS WKWebView 在記憶體壓力下會丟掉 GL 上下文,且**不保證**發 `webglcontextrestored` —— 不處理就是永久黑屏、只能重啟 app。hook 在寬限期(1.8s)內等不到恢復就 bump epoch、以全新上下文重掛 `<Canvas>`(模組級快取紋理自動重傳)。原本只有大地圖有,現在城內 3D 與戰場 3D 同樣掛上 —— 戰場那張最重(後處理棧 + instanced 千軍萬馬)且執行時大地圖的上下文還活著,最容易被丟。
  - **FPS 自適應**(`components/FrameRateWatch.tsx`):畫質檔(auto/low/high)在模組載入時就決定,無從得知這一局有三百支縱隊在野、或手機已因發熱降頻。此元件在場景內量測幀率,連續 3 秒低於 26fps 即通知宿主降級(**單向**,不來回開關以免閃爍)。原本只有戰場有,現補到大地圖與城內。
  - **大地圖的降級是分層的**(`GfxDegradedCtx`):水面鏡面(最貴,正是當年壓爆 WKWebView 顯存那層)退回廉價水材質 → Bloom 全螢幕後處理關 → 雲影(純裝飾)關 → 降水粒子 2000/1500 **減量到** 700/500 而非關閉(天候是可讀的遊戲狀態,不是裝飾)。
...
- **Esc 關閉(2026-07-28)**:兩層問題疊在一起,面板按 Esc 關不掉。(a)「武將」`OfficersTab` 與「群雄」`ForcesOverview` 是 53 個接 `onClose` 的面板裡唯二**從未進過 escape stack** 的 —— 它們早於共用 `<Modal>`,自己手寫 backdrop,所以兩條註冊路徑都沒走到;連同 `OfficersTab` 疊上去的 `OfficerDetail` 一併補上 `useEscapeKey`(不補的話 Esc 會跳過詳情、直接把底下整張名冊收掉)。(b) 補了還是關不掉,真因在 `YearbookModal`:它整局都掛在 MapScreen 上(沒有年鑑時 render null),卻寫成 `useEscapeKey(chronicle ? dismiss : () => {})` —— 沒有年鑑時等於往堆疊塞一個**什麼都不做**的處理器,而 `() => {}` 每次 render 都是新身分,effect 因此重跑、把它推回**堆疊頂端**;Esc 只呼叫最上層,底下開什麼都關不掉(實測堆疊 `["onClose", "()=>{}"]`)。改用 hook 本來就有的 `enabled` 參數:`useEscapeKey(dismiss, !!chronicle)`。**要停用就用 `enabled`,不要註冊空函式**。這一條在 `npm run dev` 看不出來 —— StrictMode 把 effect 跑兩遍,順序剛好把面板頂回最上層;只有生產構建會重現,守衛因此放在跑 `vite preview` 的 `e2e/escapeCloses.spec.ts`。
...
- **畫面管線(2026-07-26)**:三張 3D 圖此前只有戰場長出了完整的現代管線,大地圖與城內僅有一顆 Bloom。現統一於 `components/ScenePostFx.tsx`:
  - **N8AO 環境光遮蔽**(大地圖 radius 2.6 / 城內 1.4 / 戰場 1.2)—— 單項提升最明顯,城池、山峰、屋簷、牆根終於坐在地上而非浮著;**AgX 色調映射 / SMAA / 暗角** 補齊另兩張圖。
  - **四時之色**(`seasonGrade`):春青/夏濃/秋金/冬冷/夜藍的整體分級,作為資料而非烘進燈光。
  - **四時色溫(2026-07-30,`seasonTone` + `ColorGradeEffect`)**:飽和/對比說不出「冬天是鋼青、秋天是琥珀」,補一組色溫/色調/提黑/增益 —— 等同 LUT 的效果但零貼圖(±0.5 只推通道約 ±0.04,再大就讀成濾鏡)。三圖共用。
  - **熱浪扭曲(`HeatHazeEffect`)**:旱災(大地圖/城內)與戰場地火(依火點數 0.18→0.6)讓地表上方的空氣折射;遮罩只作用在畫面下三分之二,天際線歸零。
  - **體積光 God rays(戰場)**:光源是 `SkyBody` 的太陽圓盤本體(callback ref 提上去),夜戰不開(月光射線看起來不對)。**刻意不用 `<GodRays>` 元件** —— 它的 memo deps 是自己的 props 物件,等於每次 render 都 `new GodRaysEffect`(連同它的 render target)。
  - **鏡頭光暈(大地圖)**:自寫 `SunFlareEffect`(光斑+橫向拉絲+五顆鬼影+光環),太陽世界座標由 CPU 每幀投影成 UV、背向鏡頭與貼邊時淡出。**不能用 `<LensFlare>`** —— 它由 library 的泛型 props 工廠產生,memo key 是 `JSON.stringify(props)`,而我們的位置源自場景物件,`children → parent` 一定成環:`Converting circular structure to JSON`,開局直接整個 app 掛掉(2026-07-29 二分法確認:tone/heatHaze 乾淨、flare 致命)。
  - **描邊高亮(戰場)**:手上那支部隊描金邊(`xRay` 穿透前排)。腳下的選取環在密集戰列裡打不贏 —— 它貼地、被前排擋住。用 `<Selection>`/`<Select>`,只有被選中的子樹進描邊緩衝,成本是幾個 draw 而非一趟場景。`<Selection>` 必須同時包住 composer 與所有 `<Select>`。
  - **地表天候(`InstancedTilePrisms` 的 `skin`)**:雨後濕潤(壓暗+降 roughness 反光)、積雪(提白+柔化);底色乘在 per-instance 地形色上,所以一個材質就染完整盤。戰場吃 `battle.weather`,城內吃 `weatherKind` 或冬季。
  - **CSM 串接陰影:評估後不做(2026-07-30)** —— 大地圖目前**根本沒有陰影**(見下方「近景也暫時關掉」:拿掉 SoftShadows 後真正畫出來的陰影會讓地形冒出整片純黑六角格,成因未明)。在那個 bug 解掉之前談級聯是本末倒置;城內與戰場的場景尺度單張 2048 陰影圖就夠。
  ...
  - **程序化法線**(`ui/textures/proceduralMaps.ts`,2026-07-26):在小張 canvas 上畫高度場、Sobel 轉法線、模組級快取,五種接縫可平鋪的紋理 —— 瓦壟(半圓筒+逐皮錯縫)、磚縫(順砌+凹灰縫)、木紋(拉長噪聲+年輪)、織紋、夯土。整個遊戲幾百 KB 顯存,零素材。`surfaceRelief(kind, repeat)` 在材質譜之上掛法線,`repeat` 控制花紋在該網格 UV 上鋪幾次;**回傳整個 props 物件並快取**(它從 render 呼叫,不能每幀新建 Vector2)。全域強度旋鈕 `RELIEF_STRENGTH = 0.35`,**調 0 即一次關閉所有程序法線**。目前套用於屋簷/屋身(瓦壟)、城牆(磚縫 repeat 4)、民居石基與夯土牆;兵器甲冑改吃 SURFACE.iron/bronze。
  - **成本門控**:`RENDER_HI` 單獨不夠(現代 iPhone 也判 high),AO/DoF 另疊 `!IS_MOBILE`;整棧在 FrameRateWatch 降級時整個卸掉。**大地圖刻意不加景深** —— 它是資訊場景,把遠處城名糊掉是拿可讀性換氛圍。
  - **PCSS 柔影已移除(2026-07-28)**:drei 的 `<SoftShadows>` 把 three 的 shadowmap chunk 換成呼叫 `unpackRGBAToDepth(texture2D(shadowMap, uv))` 的 PCSS,而 three 0.184 的陰影圖宣告為 `sampler2DShadow` —— 沒有這個多載,片段著色器直接連結失敗。**它從來沒有柔化過任何一道影子**,只是安靜地砸掉整批著色器程式。它是**全域**改寫 `THREE.ShaderChunk`,所以任何一張圖掛上它,整個 app 的著色器一起壞:城內 27 個 program 陣亡、每幀 3,366 次 draw call 畫不出來,**進城看到的是一片天空**,牆、地、建築全部消失;大地圖也每次開局賠掉 11 個 program 與約兩萬五千次 draw。三張圖一併拿掉,普通 PCF 陰影照常。drei 10.7.7(當前最新)仍帶著這段程式碼,**升 drei 前不要加回來**。守衛在 `e2e/cityRenderBudget.spec.ts`(斷言 LINK_STATUS 全數成功)。
  - **陰影型別(2026-07-29)**:城內與戰場改寫成 `shadows={RENDER_HI ? 'percentage' : false}` —— r3f 預設是 PCFSoftShadowMap,three 0.184 已棄用並在每次開局印警告。VSM(`'variance'`)量過後**否決**:漏光把城內地面陰影沖平(暗部 7.06%→7.35%、中間調 49.95%→46.11%)。大地圖維持預設不動。
  - **環境球模糊(2026-07-29)**:`pmrem.fromScene(envScene, 0.06)` → `0.035`。three 的模糊上限是 20 個取樣,0.06 要求 30,所以每次開局都印 `sigmaRadians is too large and will clip` 然後照樣給你 20 取樣的結果。0.035 是落在上限內的最大值 —— 結果與原本相同,少一條警告。
  - **遠景城市不投影(2026-07-29)**:`useCityCastShadow` —— 過了 `near` 檔,城市網格不再 castShadow(投影者 4,488 → 1,163)。預設視角的 draw call 不變(光源自己的陰影相機本來就把那些城裁掉了),但**遠景靜置 36 → 46 FPS、拉遠 41 → 47 FPS**,因為拉遠之後那些城真的落回陰影相機的範圍。`receiveShadow` 全程保留 —— 接收不增加 draw call,而地形的影子落在城上正是立體感的來源。
  - **⚠ 遠景再減面:試過,回退了(2026-07-29)**。`mid`/`far` 檔的城市即使砍過一輪仍有 27–33 個網格:十二個雉堞(每個寬 `radius * 0.18`,在不到十一像素寬的城上是五分之一像素)加一整座八網格的城門樓。把雉堞、城門洞與飛檐一併略去,效果很好 —— **drawable 5,634 → 3,450、預設視角 draw call 5,020 → 3,138、靜置 52 → 60 FPS、遠景靜置 36 → 60 FPS**。但地形上會冒出**整片純黑的六角格**。固定鏡頭(回都城+釘死天氣日期)量近黑像素三組對照:只開投影門控 → 0.15%(乾淨);只開細節裁減 → 4.01%;兩個都開 → 4.01%。原因未明 —— 被砍的是城市網格,黑掉的是離城很遠的地形格,兩者沒有明顯關聯,但因果穩定可重現。**所以細節裁減整批回退,只留投影門控。** 要再試這條路,務必用同一套釘死鏡頭的量法(隨機開局的鏡頭落點會讓目測完全不可比,我為此連續誤判三輪),並且先解釋清楚黑格的成因再動手。
  - **畫風切換 — 寫實 / 絹本設色(2026-07-30,`SilkPaintingEffect`)**:亮度 Sobel 取墨線 + 褪色壓向暖絹底 + 靜態紙紋,一個 post pass 同時作用於三張 3D 圖(換材質要改 584 處,而且新場景不會自動跟上)。墨線刻意用**顏色**緩衝而非深度/法線 —— 多一趟場景 pass 比整幀其他東西加起來還貴(見下方 NormalPass)。此偏好**不凍結**,設定裡切換即時生效:它改的是好不好看,不是花多少錢。
  - **天漢(銀河)**:原本的星空是均勻散布,讀起來是「有一些星星」而不是一片天;沿一條傾斜大圓再鋪一層較密的點雲(高斯衰減出柔邊),一次 draw。牛郎織女隔河相望的天在這個題材裡是有份量的。
  - **雨後虹霓(`Rainbow3D`)**:只在**雨轉晴且非夜間**的那一刻觸發 —— 彩虹要同時有雨有日,而那正是地圖此前完全沒有表達的轉折。手寫光譜的環形幾何,兩秒張開、停留、五秒淡出,約 18 秒後自行卸掉。刻意短暫:它標記的是「變化」,常駐就什麼也沒說。
  - **名場面就位(`EventFocusFly` 擴充)**:歷史事件觸發時,鏡頭飛向事件在 `effects`(城防/糧秣…)或 `requires`(`officer-in-city`)裡點名的城 —— 關掉卡片後,你正看著它發生的地方。**刻意不延後卡片**:為了鏡頭而在「發生了事」與「告訴你是什麼事」之間插兩秒是本末倒置。
  - **⚠ 滅國波浪:評估後不做** —— 目前沒有「某勢力覆滅」的 store 訊號(領土是由 `hexPaint` 的亡國褪色直接重繪的),要做連鎖換色的波浪等於為純裝飾新增資料模型欄位。
  - **月相(2026-07-30)**:曆法本有上/中/下旬,夜空卻三旬同一顆滿月。一顆偏移的暗球在月前方切出弦月(幾何畫明暗界線,零貼圖):上旬左弦、中旬滿、下旬右弦。
  - **刀光(單挑)**:只在斬/劈/掃的瞬間點亮;常駐拖尾會在待機晃動時拖出殘影,並且整場帶著一條 MeshLine 的幾何。
  - **戰場遺留**:遠程齊射在落點留插地之矢(順彈道傾斜)、部隊覆滅處散落頭盔 —— 兩個 InstancedMesh 共兩次 draw,列表設上限(長仗不能無限成長)。
  - **城內起居**:燈籠改為由橫木垂吊擺盪(含流蘇)、炊煙分時辰(深夜只剩兩戶)、雨天簷下滴水簾、屯田旁的雞群與羊(疫城/焦土沒有)。
  - **開卡彩紙**:僅金/虹閃(12%/3%)落彩紙 —— 每包都撒等於沒撒。
  - **大地圖陰影修好了 —— 壞的一直是 frustum,不是陰影 pass(2026-07-30,`map3d/MapSunShadow.tsx`)**:舊設定把**整個世界**(±MAP_W × ±MAP_D ≈ 416×300 世界單位)塞進一張 2048 陰影圖 = **0.20 單位/texel**,而一個六角柱才約一單位寬 —— 深度比較幾乎處處失準,症狀就是整片六角格純黑。新的 rig 每幀把光源重新瞄準「鏡頭視線與地面的交點」,並依鏡頭高度把正交盒收到 ±10…26 單位(約 0.025 單位/texel,**八倍**),近黑像素 4.01% → **0.00%**。數字是走出來的:±45 + normalBias 0.03 仍有 2.12%;±26 + bias −0.0009 / normalBias **0.15** 才乾淨 —— **normalBias 的單位是世界單位**,當初試的 0.06 在這種尺度上根本咬不到,那一輪因此誤判成「pass 自己壞了」。順帶:收緊的陰影相機把絕大多數投影者裁掉,這個 pass 現在比世界級 frustum 便宜得多。守衛在 `e2e/mapShadows.spec.ts`(近黑像素 + rig 自報的 casting/extent —— 只量像素分不出「修好了」與「根本沒開」,第一版 spec 就是這樣自欺通過的)。**要遠景陰影請做 CSM(多個各自收緊的相機),不要把盒子放大。**
  - **四時之地(2026-07-30,`Terrain3D.useTerrainSkin`)**:地形貼圖是**整個 session 烘一次**的(數百萬次 `sampleTerrain`),所以季節換了它換不了 —— 天空/樹/分級都在動,地面十二個月穿同一件衣服。改成 patch 標準材質的著色器(`onBeforeCompile`,**零 draw call、零重烘**):雪線依真實世界高度(位移已經在幾何裡)在冬季壓到丘陵、其餘季節只蓋真峰頂;秋季低地鋪暖琥珀、旱災鋪褪色土黃。季節之間用每幀緩動而非瞬切。**要記得給 `customProgramCacheKey`**,否則 three 會把同 feature set 的未 patch 程式拿來重用,skin 靜靜地沒作用。
  - **同名事件必須互斥(2026-08-01,`eventExclusivity.test.ts`)**:155 個歷史事件裡有五組同名的,成因一致 —— 先寫了獨立事件,後來又寫了帶選項的加強版/鏈版,**兩份都留在檔案裡**。街亭那組當初處理得對(舊版加 `flag-unset: jieting-chain-started`,註解寫明 superseded),其餘三組沒有:同一局會在虎牢關前打兩次三英、斬兩次顏良、躍兩次檀溪。**最嚴重的是三顧茅廬** —— 簡述版寫成 `flag-set: maolu-abandoned`,意思剛好相反:玩家在鏈裡明白選了「罷了,天下何處無賢才」,系統反手把諸葛亮送上門,**把玩家的選擇整個抹掉**。測試釘的不是「不准同名」(簡述版對沒走過該鏈的盤有用),而是**同名的任兩個之間必須有旗標互斥**;判定會往上追一跳鏈(街亭是靠鏈的起手旗標互斥的,直接比對看不出來)。
  - **戰役覆蓋率的棘輪(2026-08-01,`scenarioCoverage.test.ts`)**:骨架早就齊了(86 個盤全有序章、封面、目標),缺的是**深度** —— 選了非主角的勢力就掉進空洞。實測 **271 個空洞散在 60 個盤**上(黃巾補完後),形狀很一致:缺的都是周邊勢力(劉表、劉焉/劉璋、馬騰、公孫、孔融、士燮、烏桓)。一次補完不是一個 commit 該做的事,而「先讓測試全紅慢慢修」等於整套長期是紅的,所以鎖的是**空洞數不得增加**:補一個就把 `HOLE_BUDGET` 調低,新加的盤沒寫齊會立刻頂破。另有一份 `COMPLETE` 清單逐盤硬鎖 —— 補齊的盤不准退回去。全部補完後把預算降到 0、改成硬性斷言。
  - **分代風物志(2026-08-01,`game/data/cityLoreEras.ts`)**:`cityLore.ts` 整份是照三國寫的,拿到戰國盤上就穿幫 —— 玩秦國看長安,風物志在講「李傕郭汜之亂」,早了四百年。改成 `cityLore(cityId, scenarioId)`:跨代盤(戰國/楚漢/隋唐)讀分代表,**該代沒寫就顯示空白,絕不回退三國文本**(守衛 `cityLoreEras.test.ts` 對每個跨代盤 × 每座城斷言不得回退)。目前先寫齊各代**盤上真正當都城的城**(戰國 11、楚漢 9、隋唐 7)—— 玩家開局第一眼看的就是自己的都城,那裡空白最刺眼;其餘隨戰役逐盤補齊,另有一條測試釘住「每個跨代盤的每個都城都必須有」。
  - **漢末風物志(2026-08-02,同檔 `LATE_HAN` + `POST_184_MARKERS`)**:分代表管的是「別在戰國盤講三國」,而**三國盤自己也有早晚**。2026-08-02 試玩黃巾之亂,開局點開鄴,風物志寫著「曹操破之而營之,築銅雀、金鳳、冰井三臺」—— 曹操 204 年才取鄴、210 年才築臺,而這是 184 年的盤。128 城裡這樣的有 **103 座**。
    修法不是逐城標年份,而是**讓文本自己說**:三國文本裡出現後世人事的詞(`POST_184_MARKERS`,詞 → 該事最早成立的年份),就表示它要到那一年才說得通。`cityLore(cityId, scenarioId, year)` 多吃一個**盤面當下年份**,未到年限就改出漢末版。於是同一座鄴,184 年講西門豹投巫鑿十二渠與太平道三十六方,打到 210 年再點開,講的就是銅雀臺 —— **方志隨著歷史推進而改寫**,不是靜態的兩套文本。
    三條測試釘住:①有年限閘的城必須有漢末版;②**漢末版自己不准帶劇透**(反向自檢,寫那 103 段時當場抓到「趙充國屯田湟中」被誤判);③跨代盤不受這條線影響。
    詞表只收**事件與人物**,不收地名 —— 赤壁、街亭、陳倉、合肥、南中在 184 年都已經是地名,拿地名當標記會把該城自己的名字掃成劇透(初版就用「南中」掃中了九真的「今越南中部」)。掃不出來的靠 `LORE_GATE_OVERRIDES` 具名處理並寫理由(郿塢、易京、樂浪的現代考古視角)。
    **教訓:詞表第一版只收了最顯眼的幾個,漏了「魏之宗室」「孫權築塢」「趙雲為太守」這一類,從 62 座補到 103 座。** 靠人看名單看不齊,要按城逐一翻舊文。
  - **紀年顯示(2026-08-01,`game/data/era.ts`)**:引擎跑的是**一條共用的正數時間軸**,而且那是刻意的 —— 歷代名將的 `birthYear` 統一設在 ~150,好讓每個人在 184 年的三國盤上都是 25–45 歲,跨代盤(戰國/楚漢/隋唐)因此一律開在內部年 178。代價是**顯示騙人**:玩「戰國·長平之戰」,標題畫面與頂欄都寫「178 AD」,而長平是公元前 260 年。修法是加一層**純顯示的紀年偏移**(`SCENARIO_ERA_OFFSET`:史實年 = 內部年 + 偏移),因為是加法,打十年顯示也跟著走(前 260 → 前 250);引擎一個數字都不用改,負年份的兩顆地雷(`abs % 4` 在 JS 對負數回傳負值、`year − birthYear` 變負歲數)全部避開。`useEraYear()` 供遊戲中的顯示點,`formatScenarioYear()` 供標題畫面那種手上就有劇本的地方。**偏移用史實慣例**(−260 就是前 260 年,不是天文紀年的 −260,那是前 261 年)—— 兩套差一年,我第一版混用,測試當場咬到。守衛在 `era.test.ts`:格式、隨年推進、三國盤偏移必須為 0、**每個跨代盤都必須登記**(加了新盤忘了登記會被抓)、換算年份要落在該時代區間。順帶:標題畫面的劇本清單原本照內部年份排,跨代盤全是 178 等於沒排 —— 改成照史實年份排。
  - **教學開場白不再寫死「三國」**:同一套教學會出現在戰國/楚漢/隋唐盤上,在長平之戰開場說「歡迎來到三國」是當場穿幫。
  - **⚠⚠ 城內開著陰影卻一道都沒畫過 —— 已修(2026-07-31 目視驗收發現)**:城內那盞 `directionalLight` 開著 `castShadow`、每幀付一張 2048² 陰影圖,但只設了 `shadow-mapSize` —— three 的**預設陰影相機是 ±5 且對準世界原點**,而城中心在 `(W·COL/2, H·ROW/2) ≈ (13, 10)`,整座城完全落在框外,所以成本照付、影子一道也沒有。修法(`CitySun`):把光源平移到城中心之上、target 釘在城中心(`light.target` 預設不在場景圖裡,r3f 的 `target-position` 只寫值不會有人更新它的矩陣,**必須自己 `updateMatrixWorld()`**),正交盒取城長邊的 0.62 倍,並補 bias/normalBias。城內每幀 draw call 2,330 → 2,921(多出來的是**真的在畫**的陰影通道),牆、府衙、民居、市集攤位終於坐在地上。戰場那盞一直是對的(±24 + bias),可以當範本。
  - **⚠⚠ 大地圖的後處理從來沒有顯示過 —— 已修(2026-07-30 目視驗收才發現)**:海面的 drei `<MeshReflectorMaterial>` 每幀自己呼叫 `renderer.render()`,只要它掛著,`<EffectComposer>` 的輸出**整片不會上畫面**。查證是逐步排除到只剩一個可能:效果有進 composer(印得出 pass 清單)、合成後的 shader 含它的原始碼、`update()` 每幀有跑、`composer.render()` 每秒 26 次、最後一個 pass `renderToScreen=true`、renderer 的 canvas 就是 DOM 那一張 —— 然後在 `composer.render()` 之後把預設 framebuffer 清成紅色,畫面**仍是地圖**;關掉反射,同一份程式立刻整片變紅。所以這張圖的 Bloom / AgX / 四時之色 / 暗角 / SMAA 一直都是白做的。**修法**:`GFX.reflections` 的 auto 改為關 —— 鏡面只影響海面一個表面,後處理影響整幀;想要倒影的人可在 設定 → 畫面 → 水面鏡面 明確打開,代價是那張圖沒有後處理。順帶把鏡面那趟場景重繪也省了,**預設視角 draw call 6,002 → 5,345**。
  - **⚠⚠ HueSaturation 會把畫面弄出整格純黑 —— 分級必須跑在色調映射之後(2026-07-30)**:把後處理換回來之後,地圖上冒出成片純黑六角格。逐項排除 N8AO、陰影、戰霧、Bloom、AgX、SMAA、BrightnessContrast 之後,兇手是 **`HueSaturation`**:它的 `hue-saturation.frag` 最後一行是 `outputColor = vec4(min(color, 1.0), inputColor.a)` —— **只夾上限、不夾下限**;而飽和公式在 saturation = 0.10 時是 `color' = 1.10988·color − 0.10988·average`,任何低於該像素平均值約 10% 的通道會直接變**負值**,負值進 AgX 就是黑。六角格是純色平面,一整格同時跨過門檻,所以壞掉的形狀正好是整格整格的黑。**它跑在線性 HDR 空間更是雪上加霜**(飽和暗部最弱的通道本來就只有平均值的百分之幾)。修法:自寫 `ColorGradeEffect` 一次做完飽和/對比/亮度/色溫/提黑/增益並 `clamp(0,1)`,而且**整組分級移到 `ToneMapping` 之後**(顯示參照 0..1,對比以 0.5 為樞軸才有意義)。ScenePostFx 現在的順序是:HDR 段(AO/體積光/光暈/鏡頭光暈/描邊/景深)→ **AgX** → 分級 → 熱浪 → 絹本 → 暗角 → SMAA。
  - **⚠ 盲調的參數在真的上畫面後全都要重調(2026-07-30)**:後處理一直沒顯示,所以它的參數是憑想像填的。實際看到之後,夜間 bloom(大地圖 0.5/0.9、城內 0.55/0.7)會把一個烽火加滿城燈火糊成一團紅光,夜間分級(contrast 0.16 / brightness −0.02)則把本來就暗的夜城壓成全黑。已改為 bloom 門檻 0.85、強度 0.4-0.42,夜間分級 contrast 0.02 / brightness +0.05 / lift 0.025 —— **夜色靠色溫與燈火表達,不靠把暗部推更暗**。
  - **FrameRateWatch 的暖機期(2026-07-30)**:它從第一幀就取樣,而 3D 場景**開頭幾秒必然最慢**(貼圖上傳、著色器編譯、合批第一次烘)—— 大地圖實測開局 3.8 → 7.9 → 15.4 fps,三秒連續低於門檻正好被這段打滿,於是**後處理整棧在開局就被永久卸掉**(降級是單向的)。加 6 秒暖機期,期間完全不計;單幀貢獻另夾在 0.1s 內,免得一次長停頓跳完暖機。
  - **上下文丟失時不掛後處理(2026-07-30)**:postprocessing 的 `addPass` 會讀 `getContext().getContextAttributes().alpha`,上下文丟失時那是 `null`,**整個 renderer 進程會掛掉**;而 composer 每次 render 都重掛所有 pass。`useContextAlive` 在 `webglcontextlost` 時讓 ScenePostFx 整個回傳 null。守衛:`e2e/glRecovery.spec.ts`。
  - **NormalPass 是白付的一趟場景重繪(2026-07-30)**:`<EffectComposer enableNormalPass={wantAO}>` 從 2026-07-26 加上就一直在**把整個場景再畫一遍**(城內實測 +1,494 次 draw/幀,比合批後的整座城還貴)。而全專案唯一的 AO 是 **N8AO,它是獨立 Pass**(`new N8AOPostPass(scene, camera)` 自己算深度與法線),從來沒讀過那張法線圖。改 `enableNormalPass={false}`,城內 3,790 → 2,296。要加回來的前提是改用真的吃它的效果(SSAO / DepthDownsampling)。
  - **⚠ 城內 draw call 有兩個穩定值,別拿它當斷言**:降級(FrameRateWatch 卸掉後處理)約 1,600、未降級(N8AO 還在)約 2,300;而「合批失效+已降級」是 4,147 —— 任何單一門檻都被兩種狀態橫跨。`cityRenderBudget.spec.ts` 因此改為斷言 **StaticBatch 自己報的普查行**(藏了幾個 mesh → 產出幾個合併 draw),它與幀率/畫質檔/降級全部無關;draw call 只留 5,000 的粗放煙霧線。**我在寫這條守衛時自己踩了 `mapDrawCalls.spec.ts` 早就記過的同一個坑。**
  - **靜態合批(2026-07-29,`components/StaticBatch.tsx`)**:城內一幀 4,144 次 draw call 的大頭是物件數而非三角形 —— 64 棟民居 × 每棟 6–8 個 mesh、街磚、城牆段、樓塔擺件,每個 mesh 一次 draw(開陰影再乘二)。StaticBatch 在 React 提交後把標記 `userData.batchStatic` 的 mesh 依材質簽名分組、烤世界矩陣、**把材質色寫進頂點色**(顏色各異因此能同批),合成每組一個大 mesh:實測 **2,523 mesh → 28 draw,城內 4,144 → 1,598/幀(−61%)**。互動子樹(onClick 巡查卡)靠 r3f `__r3f.eventCount` 自動排除;動態島(旗幟/火焰)標 `batchSkip` 留在原地。**deps 是契約**:會增減/換色標記物件的每個輸入都得在列,漏了就是畫面停舊態。合批失敗**靜默回退**原樣,所以 `cityRenderBudget.spec.ts` 加了 `< 3,500` 的數字鎖 —— 它壞了必須有測試會叫。
  - **畫質細項(2026-07-29,`renderQuality.ts` GFX)**:流暢/精緻一刀切拆成四個真實成本 —— 陰影/後處理/水面鏡面/天候粒子密度,設定 → 畫面 各自 自動/開/關(粒子:自動/全量/減量),`auto` 完全等同舊行為。與 RENDER_HI 同約定:模組載入時凍結,改了重啟生效;FrameRateWatch 的運行時降級疊在其上、只降不升。
  - **遠城 impostor:評估後不做(2026-07-29)** —— 大地圖遠景城的 LOD 檔位跟著鏡頭距離逐城變化,合批/替身都會在鏡頭移動時觸發重烤風暴;且上次「遠景細節裁減」留下過黑格懸案(見上)。大地圖預設視角 5,011/幀在 9,000 預算內,先不動。**貼圖圖集**同樣不做:本作材質幾乎全是純色無貼圖,成本在物件數不在貼圖切換,頂點色合批已是等效解。
  - **進城時大地圖停轉(2026-07-28)**:城邑地圖是全螢幕覆蓋,但 `StrategicMap3D` 的 `<Canvas>` 在後面照畫不誤 —— 實測每幀 5,019 次 draw call,而城內自己才 4,147:**站在城裡有一半以上的成本花在沒人看得見的世界地圖上**,城內因此只有 25 FPS(停掉後 48)。戰場那條路是直接卸載地圖(`MapScreen`:`!battleScreenUp && <StrategicMap3D/>`),但進出城太頻繁,重掛會重建整個場景並丟失鏡頭,所以改用 `frameloop={cityMapOpen ? 'never' : 'always'}` 把繪製迴圈停住、場景與鏡頭原封不動。**停了就必須會恢復** —— 同一支 e2e 一併斷言出城後大地圖重新開始繪製。
