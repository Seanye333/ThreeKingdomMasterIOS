...
flowchart TD
  ...
  B --> C["回合開始:將旗統率/寡不敵眾 ±士氣 · 孤軍 −士氣 · 陷亂自動重整;潰兵自動逃向己方邊緣"]
  C --> D["逐單位行動:移動(渡河致亂) / 普攻 / 個人戰法 / 詐敗誘敵 / 陣前挑將(車輪戰)"]
  D --> E["傷害結算"]
  E --> E1["× 地形修正(隘口/牆/林/丘…)"]
  E1 --> E2["× 政策 combatPolicyEffects(攻/守/火/射)"]
  E2 --> E3["× 精兵 × 技能 × 名戰★ × 戰法組合"]
  E3 --> E4["× 士氣轉戰力 × 衝鋒蓄力 × 追擊掩殺 × 兵裝相剋 × 圍殲(困獸/甕中) × 陷亂(潰兵/槍兵立防/詐敗回馬皆在此結算)"]
  E4 --> F["士氣檢定:見底=潰走(不能攻·被追擊·鄰主將可收攏)"]
  F --> G{"一方潰滅/盡潰 / 主將被擒被斬?"}
  G -->|否| C
