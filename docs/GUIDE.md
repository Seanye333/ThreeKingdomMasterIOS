...
**大地圖後處理預設關閉(2026-08-03)** —— 開著會在平移後把地圖畫壞。使用者回報「拉拽地圖變成一層層疊上去、看不出原來的地圖」,`e2e/mapDragSmear.spec.ts` 重現並量化:往北拖兩次,地圖區的**逐列平均絕對差**從 13 升到 58(乾淨 ≈ 13,壞掉 55–67,中間沒有灰帶),放大看是地形被壓成一列列碎塊。基準六跑五壞。

已排除的(每項六樣本、每次重建 dist):陰影、N8AO、Bloom、鏡頭光暈、四時之色分級、AgX 色調映射、SMAA、多重取樣(0→4)、相機近平面(0.5→3)、海面高度、離屏目標格式(HalfFloat→UnsignedByte)。**連完全清空 effects 的 composer 都照壞**;而相機在三個階段的俯角與距離全程恆定(52.1°、130),開關後處理的終點座標一模一樣。所以不是相機、不是任何單一效果,是**走離屏目標這條路徑本身**。根因尚未找到。

取捨與「水面鏡面」那條同型(見 renderQuality.ts):整幀壞掉比少一層色調重要得多。想要的人可到 設定 → 畫面 → 後處理 明確打開。城內與戰場不受影響。

**方法上的教訓,比這個 bug 本身重要:**

1. **Playwright 跑的是 `vite preview` 服務的 `dist/`,不是原始碼。** 我改完原始碼只跑 `tsc -b` 就測,連跑七次「二分」—— 那七次跑的是同一份 dist,clean/broken 全是雜訊,結論(先怪 N8AO、再怪光暈、再怪分級)全錯。改原始碼做 e2e 二分,**每次都要 `npm run build`,而且要確認它沒有因為未使用匯入而失敗**。
2. **間歇性的 bug 不能用單跑判定。** 基準的壞掉率是 5/6:單跑「乾淨」有六分之一機率純屬運氣。要 `--repeat-each` 取樣。
3. **用眼睛看截圖不是判準。** 十二張截圖分不出來,而一個逐列差異的數字一眼就分開了(13 vs 58)。先做出判準,再開始二分。
