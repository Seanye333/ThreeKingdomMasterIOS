...
### 2.3 性格 traits(personality.ts,共 200 條定義;另有 8 種君主 AI 性格 `RulerPersonality`)

- 每人 0~3 條性格,驅動 AI 傾向、事件鉤子與各系統加成。**機制中樞為 `traitEffects.ts`** —— 各系統呼叫它的小函式讀取性格加成,新增性格→把它加進對應的 Set 即可。約 **152 條接入機制、48 條純風味**(`isWiredTrait`/`isFlavorOnlyTrait`);風味性格在武將面板的性格 tooltip 標注「(風味·無機制效果)」,且**不會發給程序生成的新秀**(`officerGen` 只抽接入機制的性格;史實/捏人名冊不受限)。
...
- **舌戰**(`wordWarProwessMul`):雄辯/善說 ×1.12、寡言鋒 ×1.10、機智/風趣 ×1.06、沈着/含蓄 ×1.05、寡黙 ×0.92。**舌戰崩潰**:性烈者(**暴怒**/傲慢/虛榮/剛愎/急躁)氣勢被打穿時可被「罵死」(見第六章。註:早期誤用不存在的 `hot-tempered` id 致此機制從未觸發,現已更正為 `wrathful`)。
