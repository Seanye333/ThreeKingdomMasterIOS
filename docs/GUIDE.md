...
- **畫面管線(2026-07-26)**:三張 3D 圖此前只有戰場長出了完整的現代管線,大地圖與城內僅有一顆 Bloom。現統一於 `components/ScenePostFx.tsx`:
  ...
  - **⚠⚠ 大地圖的後處理從來沒有顯示過(2026-07-30 目視驗收才發現)**:海面的 drei `<MeshReflectorMaterial>` 每幀自己呼叫 `renderer.render()`,只要它掛著,`<EffectComposer>` 的輸出**整片不會上畫面**。查證方式是逐步排除到只剩一個可能:效果有掛進 composer(`PassProbe` 印出 pass 清單含 `SilkPaintingEffect`)、合成後的 shader 含它的原始碼、`update()` 每幀有跑、`composer.render()` 每秒 26 次、最後一個 pass `renderToScreen=true`、renderer 的 canvas 就是 DOM 裡那一張 —— 然後在 `composer.render()` 之後把預設 framebuffer 清成紅色,畫面**仍然是地圖**;把反射關掉,同一份程式立刻整片變紅。所以這張圖的 **Bloom / AgX 色調映射 / 四時之色 / 暗角 / SMAA 一直都是白做的**,城內與戰場則正常(它們沒有鏡面)。
    - **為什麼還沒改預設**:把鏡面關掉、讓後處理現身之後,地圖會冒出**成片純黑的六角格**。已逐項排除 N8AO(關掉仍在)、陰影(關掉仍在)、戰霧疊層(關掉仍在),成因未明 —— 與 2026-07-29 那樁「遠景細節裁減出黑格」是否同源尚不確定。在查清楚前維持現狀(桌機仍開鏡面),寧可畫面不變也不要為了分級而讓地圖破圖。設定 → 畫面 → 水面鏡面 的說明已寫明兩者互斥。
    - **教訓**:這批的前幾輪我一直在「往大地圖加後處理」,而它們**根本沒有機會上場**。凡是加了看不出差別的畫面效果,先證明它有沒有被畫出來,再去調參數。
  - **FrameRateWatch 拿開場幾秒當樣本(2026-07-30 修)**:它從第一幀就取樣,而 3D 場景**開頭幾秒必然最慢**(貼圖上傳、著色器編譯、合批第一次烘)—— 大地圖實測開局是 3.8 → 7.9 → 15.4 fps,三秒連續低於門檻正好被這段打滿,於是**後處理整棧在開局就被永久卸掉**(降級是單向的)。加了 6 秒暖機期,期間完全不計;單幀貢獻另外夾在 0.1s 以內,免得一次長停頓直接跳完暖機。
