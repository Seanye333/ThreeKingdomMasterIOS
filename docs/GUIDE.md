...
| 弓矢有限(§5.1) | 矢:弓4·攻城/水3;矢雨耗1,矢盡不能射;糧車/補給格相鄰每回合補1 |
...
| 衝鋒蓄力(§5.1) | 本回合淨衝距≥2 且敵原距≥2:每格 騎+12%(封頂+45)/步+5%(封頂+18);據守且正對的槍兵立防→衝勢×0.7 且反擊×1.6 |
| 將旗統率(§5.1) | 主將旗下半徑 R=2+統率/40 每回合 +3 士氣;半徑外又無鄰友軍=孤軍 −6 士氣 |
| 陣前單挑(§5.1) | 相鄰敵將可挑將(沿用演武場勝負);車輪戰 每戰每場 −5 武力(封頂−20);敗將部曲奪氣/被挑落即潰走;委託指揮時自動裁決 |
| 兵裝相剋(§5.1/§5.9) | 10 兵裝接戰:戟制騎×1.15·弩破甲×1.13·騎踏弓弩軍師×1.12·劍走側背×1.10·刀破輕×1.07·襲書生×1.12/欺徒手×1.08;每條≤×1.15、夾[0.85,1.4] |
...
| 射擊縱深(§5.1) | 基礎遠程攻擊 射程 弓3/弩4/攻城4/水軍3;射界被牆/門/高山擋(矢雨拋射越過);掩體 林×0.8·人擋×0.85;射程外無反擊;每射耗1矢;矢雨齊發=主滿傷+鄰半傷濺射 |
| 戰局氣勢(§5.1) | momentum −100..+100(正偏攻方);殺敵+9·斬將+22,每回合×0.85回落;順勢 傷害至多+8%·回士氣,頹勢 傷害−·掉士氣 |
...
**弓矢有限・就地補給**——弓弩/攻城/水軍開戰各帶**矢數**(`ammo`:弓 4、攻城/水軍 3);每次**矢雨齊發**耗 1 矢,**矢盡則不能再射**(「矢盡待補給」,逼近身肉搏)。**糧車**(`isSupply`)或補給格相鄰,每回合補 1 矢。遠程火力自此掛鉤後勤。AI 矢盡亦不空放。
...
**衝鋒蓄力(蓄勢突陣)**——單位記錄本回合的**淨衝距**(`charge`,從起步格到當前格的真實六角距離,折返不算)。當**淨衝距 ≥2 且目標在起步時 ≥2 格外**(真衝鋒,非貼身挪動),近戰挾**衝鋒之勢**:每格**騎兵 +12%(封頂 +45%)、步兵 +5%(封頂 +18%)**,攻城/水軍無此勢。**剋制**:目標為**據守(防御)且正面朝向衝來者**的**槍兵**,則**立防拒馬**——衝勢反挫 ×0.7,且槍兵**反擊 ×1.6**人馬交摧。騎兵蓄力突陣、長槍立陣相迎成為新博弈;蓄力命中彈「衝」字、立防彈「折」字。
...
**射擊縱深(射程・射界・掩體・齊射)**——遠程不再只能靠計謀:**弓弩/攻城/水軍有了基礎遠程攻擊**(`attackRange`:弓 3、弩 4、攻城 4、水軍 3;近戰兵 1),選中即可在射程內點敵放箭。**射界(視線)**:**牆/城門/高山**會擋住直射(`hasLineOfSight`,矢雨拋射則越過);**掩體**:目標在**林中** ×0.8、有**人擋箭** ×0.85。從射程外射來的箭,**白刃夠不著者無從反擊**(只有射程涵蓋的對手才回射);每射**耗一矢**,矢盡須補給。**矢雨齊發**升級為**拋射覆蓋**:主目標滿傷、貼鄰之敵**半傷**濺射。弓弩終於有了「站位、射界、彈藥」的真戰術。

**戰局氣勢(順勢 ↔ 頹勢)**——一條全域**氣勢**主線(`momentum` −100..+100,正偏攻方):**殺敵 +9、斬將 +22**,取勢者得勢;每回合**向 0 回落 ×0.85**。**順勢**之師全軍傷害微增(至多 +8%)、回合**回士氣**;**頹勢**之師傷害微減、**掉士氣**。把原本散落的局部士氣縫成一條「勝負之勢」,HUD 頂條一目了然。
...
- **傷害預估**(forecastAttack):攻擊前預覽傷害——**已校準**:把 §5.1 全套乘子(兵裝相剋/側背/居高/夾擊/氣勢/士氣/衝鋒/追擊/圍殲/陷亂/遠程掩體/疲乏…)全折進預覽,**射程外的近戰敵不顯反擊**;不再只算粗剋+地形。AI 的選敵打分亦改用同一套精算 forecast,故 AI **會避開拒馬槍陣、挑兵裝佔優的目標、銜尾追潰兵**。
- **戰後復盤**(battleRecap):戰損比、最堅韌、中流砥柱、計謀次數;另列**戰局轉折**(從戰報萃取斬將/潰走/接掌帥旗/甕中/衝鋒…的關鍵時刻,標回合)與**終局氣勢**。
- **委託指揮**:小仗一鍵交戰術 AI 代打。
- **戰鬥錄影**:🎬 導出 WebM。
...
- **兵裝相剋(已接戰,見 §5.1)**:兵裝不再純展示 —— 近戰時 attacker/defender 的兵裝相互作用,在 6 兵種粗剋之上再加一層細修(戟制騎、弩破甲、騎踏弓弩軍師、劍走側背、刀破輕、襲書生/欺徒手),每條 ≤×1.15 並夾於 [0.85,1.4]。傷害彈會標出當下最強的一條(如「戟制騎」)。
...
成算看軍師智力,敵對越深越易得手;施計**勝負都扣金**。officer 級「離間」在諜報。**智 ≥72 的軍師會主動在§4.5 錦囊裡獻此計**(見兩強接壤獻二虎競食、遠方強鄰獻遠交近攻),一鍵照辦即施。
