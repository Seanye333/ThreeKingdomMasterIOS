...
| 11 | 每張盤實際演得到幾條戲 | `node --import tsx scripts/event-coverage.ts` | — | 零事件 0 張(診斷,全庫 20–30 分) |
...
8. ~~**③ 戰役專屬事件鏈 32/86 是現在最短的一格**~~ —— **判準本身是錯的,
   ...
   兩批共 32 條補完後,戰國 14 張全部 11–13 條、楚漢 9–10 條、
   安史盤 5 條且**不外漏到唐初三張盤**。新增 §1 第 11 條的體檢腳本量這件事。
