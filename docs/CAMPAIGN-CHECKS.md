...
假想盤仍缺的只剩一項:**敗亡變體 24/129**(2026-08-08 起補:
「若袁紹勝官渡」曹操/孫權、「若郭嘉不死」劉備/孫權、
「若董卓未亡」曹操/袁紹/孔融、「若呂布割據徐州」袁術/公孫瓚、「若孫策不死」曹操/袁紹、「若袁術稱帝成」呂布/孫策、「英雄集結」劉備/董卓/士燮/劉表/張魯/公孫度)。
...
4. **每家敗亡變體缺 302/540**(全庫)。`scenarioVerdicts.ts` 的檔頭明寫這是
   選填(沒寫的走通用輓歌,而通用那一支已按君主死/囚/降/流亡分了四種),
   所以不是壞掉;但選了周邊勢力的玩家讀到的是通用文本。
   目前的節奏是**每深挖一張盤就補那張盤缺的幾家** —— 硬要一次寫完 317 段,
   質量守不住。缺最多的:英雄集結 16、若董卓未亡 10、若呂布割據徐州 8、
   若袁術稱帝成 8。
5. **傳國玉璽是個死旗標,而袁術因此永遠稱不了帝**(2026-08-08 查到)。
