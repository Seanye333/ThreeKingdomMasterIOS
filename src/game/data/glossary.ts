...
export const GLOSSARY: GlossaryCategory[] = [
  {
    ...
    terms: [
      ...
      { zh: '掩殺收降', en: 'Ride down a rout', descZh: '潰軍無力再戰:敵行軍縱隊撞上(或潰軍路過敵城,守軍≥4000 出城)即成「掩殺」— 基準斬獲 55%(獵方有「追擊」技 +15%),斬獲三成就地收降入己軍/守城;潰軍餘部 <300 即覆滅,敵將各 35% 就擒(成就「追亡逐北」)。「邀擊」潰軍是主動獵殺之法。', descEn: 'Routs cannot fight back: any hostile column or garrison that catches one cuts it down — 55% slain per strike (+15% with the Pursuit skill), 30% of the kill pressed into your ranks, officers captured if the rout is wiped out. Use INTERCEPT to hunt one actively.' },
      ...
      { zh: '避戰迂迴', en: 'Evade stance', descZh: '在途縱隊可下「避戰」(在途部隊面板):遇敵縱隊/守軍出擊時以智鬥智擲脫離(我智−敵智定成敗,緩進加成、急行減成;對匿伏之敵減半 — 看不見的埋伏繞不開),成則兵不血刃各自去遠;被抓則倉皇接戰(×0.85)。代價:避戰行軍不奪土不佔色帶。偷渡陰平之計自此可行。', descEn: 'Set a marching column to EVADE: it rolls (wits vs wits) to slip hostile contacts and garrison sallies; caught = fights strung out. It claims no territory while slinking by back roads.' },
      { zh: '師老兵疲', en: 'Campaign fatigue', descZh: '縱隊跨季累積疲勞(0–100):行軍 +8/季(急行 +14、緩進 +4)、紮營休整 −8、圍城營 +4。疲勞削野戰戰力(百疲 ×0.75)並壓開戰士氣(至 −13)。連年遠征須擇地休整 — 久暴師則鈍兵挫銳。', descEn: 'Columns accrue fatigue season over season (marching +8, forced +14, siege camps +4; rest camps −8). It saps field power (×0.75 at 100) and opening morale. Long campaigns need rest stops.' },
      { zh: '冬季行軍', en: 'Winter campaigning', descZh: '冬季在途縱隊苦寒折兵 3%/季(緩進減半);行至深山每季五成「大雪封山」寸步難進;圍城營冬季凍損 4%/季。而冬季河面結冰可渡(行程反而可能更短)— 冬征是「走冰面近道 vs 凍餒交加」的取捨。', descEn: 'Winter marches shed 3% a season to the cold, deep-mountain columns risk being snowed in, and siege camps freeze (−4%/season) — but frozen rivers open shortcuts. A real trade-off.' },
      { zh: '野戰繳獲', en: 'Field spoils', descZh: '野戰/攔截/拔寨獲勝即剝取敗軍輜重:糧 = 敵折損 ×1.5(直補勝方隨軍糧;未帶糧則歸源城)、金 = ×0.04、馬 = ×0.015、鐵 = ×0.025 歸源城(受城庫上限);拔寨奪其積聚再 ×1.5。守軍出擊獲勝繳獲入城。繳來的馬鐵還餵鍛造與騎兵。', descEn: "Win a field clash and you strip the loser's baggage: grain (straight into your column's train), coin, loose mounts and dropped arms (horses/iron feed your forges and cavalry); storming a camp yields its stores on top." },
    ],
  },
  {
