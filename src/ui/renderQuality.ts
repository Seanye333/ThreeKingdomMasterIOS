...
function resolveGfx(p: GfxPrefs, hi: boolean): GfxFlags {
  ...
  return {
     ...
     *
     * 取捨很清楚:鏡面只影響海面**一個表面**,後處理影響**整幀**(色調映射、
     * 四時之色、光暈、暗角、抗鋸齒、畫風)。所以 auto = 關,把整幀換回來;
     * 想要海面倒影的人可以在 設定 → 畫面 → 水面鏡面 明確打開,代價是那張圖
     * 沒有後處理。城內與戰場沒有鏡面,它們一直是正常的。
     *
     * (順帶:一開始換回後處理時地圖會冒成片純黑六角格,那是 library 的
     * HueSaturation 在線性 HDR 上不夾下限所致 —— 見 ColorGradeEffect。
     * 先修好那個,才有這個預設。)
     */
    reflections: onOff(p.reflections, false),
  };
}
