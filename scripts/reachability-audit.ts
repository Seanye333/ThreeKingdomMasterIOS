 ...
 *
 * ## ⚠ 高壓力**不等於**打得下來(2026-08-16 學到的)
 *
 * 這條門檻是**必要條件,不是充分條件**,而那個差距比想像中大。
 * 實例:張魯的「師君治漢中」被改成取葭萌,理由是壓力算出來 2.73–3.87
 * (看起來輕鬆),而重掃結果是三張盤全部 0/3 —— 一次也沒拿下來過。
 *
 * 原因是這支算的是 `pickForceTarget` 的**入選門檻**(≥1.05,只決定那座城
 * 進不進候選名單),而真正要不要出兵是 `decideCommand` 的**攻擊門檻**:
 * 逐城比 `effectiveDefender / attacker ≤ 0.6`(再乘君主性格/天候/守勢/
 * 多線收縮等一串係數),嚴格得多。中間還隔著:守方會增援、會建軍、
 * 城防會漲,而攻方只派得出本城七成的兵。
 *
 * **判準**:壓力低 → 幾乎一定死(信這個);壓力高 → 只是「沒被這一關擋掉」,
 * 要不要寫進主目標仍然得用 `objective-sweep.ts` 實跑。
 * 拿這支的數字直接改主目標,就會像張魯那三條一樣,改完照樣是 0。
 *
