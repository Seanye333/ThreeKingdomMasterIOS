 ...
 * ## ⚠ 它只告訴你方向,不夠準到拿來定期限
 *
 * 實測過一次對不上:英雄集結的袁紹,這支報「鄴 +1年 3/6、+3年 3/6」,
 * 而逐旬探針是 **201 年 4/4 還在、203 年 0/4**(全數丟失)。兩邊差很多。
 *
 * 所以判準是:
 *  - **拿它看方向**(他往哪邊推得動、哪幾座城明顯留不住)—— 這一層是可靠的,
 *    「一次也沒有在手上」「每個檢查點都已失去」這種全 0 的訊號尤其可靠。
 *  - **不要拿它定 byYear**。改完的每一條仍然要用 `objective-sweep.ts` 實跑
 *    驗證,而那才是判死活的那把尺。
 *  - 輪數少時 50% 上下那一格是雜訊 —— 同 objective-sweep 檔頭那條
 *    「二值判定裁決不了 AI 改動」。
 *
 * Run:
