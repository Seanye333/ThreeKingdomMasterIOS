 ...
 *
 * ## ⚠ 這支**裁決不了 AI 行為的改動**(2026-08-16)
 *
 * 它的輸出是二值的:一條目標三輪裡中 0 次就進榜。而一條剛好卡在門檻上的
 * 目標,三輪中 0 次還是 1 次基本是擲骰子 —— 所以頭條數字對 AI 改動極不敏感。
 *
 * 實測:同一天用三個 AI 版本各跑一次(17 張假想盤 × 3 輪),頭條是
 * 31 / 34 / 28,看起來差很多;但把「從未達成」逐條 diff,**23 條裡有 15 條
 * 翻了面**(7 條新死、8 條救回)。總數幾乎沒動,名單換掉三分之二 ——
 * 那三個數字全在噪聲裡。
 *
 * **判準**:量**目標資料的修訂**有效(52 → 31 那種量級,改動是定向的);
 * 量 **AI 行為**要改用連續量(`scripts/big-power-ab.ts` 的城留存/兵留存
 * 中位數 + 區間),不要用「達成/沒達成」這種二值判定去比。
 *
 * ## 兩個一定要記住的前提
 *
...
const BY_DESIGN: Array<{ scenario: string; force: string; why: string }> = [
  ...
  /*
   * 合肥之戰(215)的張魯:開局只有漢中與武都兩城,而**兩座在第一個檢查點就已失去**,
   * 中位 0。215 年正是曹操出散關取漢中、張魯奔巴中而後降的那一年 ——
   * 「五斗自守」要他撐到 216,而這張盤的開局就是他的末日。
   * 同型於下邳的呂布、易京的公孫瓚。落鳳坡與入主西川那兩張的張魯**沒有**進來:
